...
void loop()
{
    ...
    if (SerialSW.available()) {       // Jika ada data yang diterima dari sensor
        data = SerialSW.readString(); // data yang diterima dari sensor berawalan
        // tanda * dan diakhiri tanda #, contoh *1#
        a = data.indexOf("*"); // a adalah index tanda *
        b = data.indexOf("#"); // b adalah index tanda #
        s_angin = data.substring(a + 1,
                                 b); // membuang tanda * dan # sehingga di dapat nilai dari arah angin
