/*
SoftwareSerial.h - Implementation of the Arduino software serial for
ESP8266/ESP32. Copyright (c) 2015-2016 Peter Lerup. All rights reserved.
...
public:
    static constexpr bool isValidPin(int8_t pin)
    {
...
#elif defined(ESP32)
        // Remove the strapping pins as defined in the datasheets, they affect
        // bootup and other critical operations Remove the flash memory pins on
        // related devices, since using these causes memory access issues.
        ...
        return (pin == 1) || (pin >= 3 && pin <= 5) || (pin >= 12 && pin <= 15) ||
               (!psramFound() && pin >= 16 && pin <= 17) || (pin >= 18 && pin <= 19) || (pin >= 21 && pin <= 23) ||
        ...
        // Pinout
        // https://docs.espressif.com/projects/esp-idf/en/latest/esp32c3/_images/esp32-c3-devkitm-1-v1-pinout.jpg
...
/// It also has optional input buffer capacity arguments for byte buffer and ISR
/// bit buffer. Bitrates up to at least 115200 can be used.
...
public:
    ...
    /// @param rxPin -1 or default: either no RX pin, or keeps the rxPin set in
    /// the ctor
    /// @param txPin -1 or default: either no TX pin (onewire), or keeps the txPin
    /// set in the ctor
    /// @param invert true: uses invert line level logic
    /// @param bufCapacity the capacity for the received bytes buffer
    /// @param isrBufCapacity 0: derived from bufCapacity. The capacity of the
    /// internal asynchronous
    ///        bit receive buffer, a suggested size is bufCapacity times the sum
    ///        of start, data, parity and stop bit count.
    ...
    int peek() override;
    int read() override;
    /// @returns The verbatim parity bit associated with the last successful
    /// read() or peek() call
    bool readParity()
    {
    ...
    /// The read(buffer, size) functions are non-blocking, the same as readBytes
    /// but without timeout
    ...
    /// @returns The number of bytes read into buffer, up to size. Times out if
    /// the limit set through
    ...
    [[deprecated("function removed; semantics of onReceive() changed; check the "
                 "header file.")]] void
    perform_work();
...
private:
    ...
    int8_t m_rxLastBit; // 0 thru (m_pduBits - m_stopBits - 1): data/parity bits.
    // -1: start bit. (m_pduBits - 1): stop bit.
...
#endif
    // the ISR stores the relative bit times in the buffer. The inversion
    // corrected level is used as sign bit (2's complement): 1 = positive
    // including 0, 0 = negative.
    std::unique_ptr<circular_queue<uint32_t, UARTBase *>> m_isrBuffer;
    const Delegate < void(uint32_t &&), UARTBase * > m_isrBufferForEachDel{
        [](UARTBase * self, uint32_t &&isrTick) { self->rxBits(isrTick); }, this};
...
public:
    ...
    /// @param rxPin the GPIO pin used for RX
    /// @param txPin -1 for onewire protocol, GPIO pin used for twowire TX
    BasicUART(int8_t rxPin, int8_t txPin = -1, bool invert = false) : UARTBase(rxPin, txPin, invert)
    {
    }
    ...
    ///        bit receive buffer, a suggested size is bufCapacity times the sum
    ///        of start, data, parity and stop bit count.
    void begin(uint32_t baud, Config config, int8_t rxPin, int8_t txPin, bool invert, int bufCapacity = 64,
    ...
    void begin(uint32_t baud, Config config, int8_t rxPin)
    {
    ...
    void setTransmitEnablePin(int8_t txEnablePin)
    {
        UARTBase::setTransmitEnablePin(GpioCapabilities::isValidOutputPin(txEnablePin) ? txEnablePin : -1);
    }
};
...
#if __GNUC__ < 12
// The template member functions below must be in IRAM, but due to a bug GCC
// doesn't currently honor the attribute. Instead, it is possible to do explicit
// specialization and adorn these with the IRAM attribute: Delegate<>::operator
// (), circular_queue<>::available, circular_queue<>::available_for_push,
// circular_queue<>::push_peek, circular_queue<>::push
