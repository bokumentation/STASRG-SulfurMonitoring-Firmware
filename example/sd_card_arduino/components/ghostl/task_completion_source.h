#pragma once
/*
task_completion_source.h - Implementation of a C++20 async coroutines task
completion source. Copyright (c) 2023 Dirk O. Kaar. All rights reserved.
...
namespace ghostl {
template <typename T = void> struct task_completion_source {
    ...
    auto set_value(T &&val) const -> void
    {
        ...
        if (auto handle = state->coroutine.load(); handle && !handle.done()) {
            handle.resume();
        }
    }
...
private:
    ...
    struct awaiter final {
        ...
        {
            return state->ready.exchange(true);
        }
...
template <> struct task_completion_source<void> {
    task_completion_source() = default;
    task_completion_source(const task_completion_source &other) noexcept : state(other.state)
    {
    }
