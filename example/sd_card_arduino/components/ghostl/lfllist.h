...
namespace ghostl {
template <typename T, class Allocator, typename ForEachArg> struct lfllist;

namespace detail {
template <typename T> struct lfllist_node_type {
...
struct lfllist {
    ...
    /// <summary>
    ///  Emplace an item at the list's front. Is safe for concurrency and
    ///  reentrance.
    /// </summary>
    ...
    /// </summary>
    /// <param name="node">A node (not nullptr) that must be a member of this
    /// list.</param>
    ...
    /// </summary>
    /// <param name="node">A node (not nullptr) that must be a member of this
    /// list.</param> <returns>True on success, false if there is competition on
    /// locking node.</returns>
    auto try_remove(node_type *const node) -> bool
    {
        auto _false = false;
        if (!node->remove_lock.compare_exchange_strong(_false, true)) {
            return false;
        }
    ...
    /// Caveat: for_each() or try_pop() may erase any node pointer.
    /// </summary>
    /// <param name="to_erase">An item (not nullptr) that must be a member of this
    /// list.</param>
    ...
    /// <param name="to_erase">An item (not nullptr) that must be a member of this
    /// list.</param> <returns>True on success, false if there is competition on
    /// locking to_erase.</returns>
    auto try_erase(node_type *const to_erase) -> bool
    {
        if (!try_remove(to_erase)) {
            return false;
        }
        destroy(to_erase);
        return true;
    ...
    {
        if (auto node = last_sentinel.pred.load(); node) {
            return node;
        }
        return nullptr;
    }
    ...
    /// <summary>
    /// Try to atomically get the item of and erase a node at the back of the
    /// list. Using try_pop(), full concurrency safety. Non-reentrant,
    /// non-concurrent with erase().
    /// </summary>
    /// <param name="item">An out argument that on success, receives the item at
    /// the back of this list.</param> <returns>True on success, false if the
    /// queue is empty or there is competition.</returns>
    ...
    {
        auto _false = false;
        if (!pop_guard.compare_exchange_strong(_false, true)) {
            return false;
        }
        auto has_node = false;
    ...
    /// The ownership of the item contained in that node is passed to parameter
    /// function. Non-reentrant, non-concurrent with erase(), try_pop(), and
    /// for_each().
    /// </summary>
    /// <param name="to_erase">A function this is invoked for each node of this
    /// list.</param>
