...
namespace ghostl {
template <typename T = void> struct when_all final {
    static auto continuation(T result, size_t pos, std::shared_ptr<std::vector<T>> results,
    ...
    template <typename C> explicit when_all(C &&_tasks)
    {
        ...
        for (ghostl::task<T> &task : tasks) {
            ...
            runner.continue_with(
            [pos, results, remaining, tcs](T result) {
                continuation(result, pos, results, remaining, tcs);
            });
...
private:
    ...
    std::shared_ptr<std::atomic<size_t>> remaining{std::make_shared<std::atomic<size_t>>(0)};
    ghostl::task_completion_source<std::vector<T>> tcs{};
};
template <> struct when_all<void> final {
    static auto continuation(std::shared_ptr<std::atomic<size_t>> remaining, ghostl::task_completion_source<> tcs)
    -> void
    {
        if (auto isremaining = --*remaining; !isremaining) {
            tcs.set_value();
    ...
    template <typename C> explicit when_all(C &&_tasks)
    {
        ...
        for (ghostl::task<> &task : tasks) {
            ...
            runner.continue_with([remaining, tcs]() {
                continuation(remaining, tcs);
            });
            continuations->emplace_back(std::move(runner));
        }
        if (continuations->empty()) {
...
private:
    std::shared_ptr<std::vector<ghostl::run_task<>>> continuations{std::make_shared<std::vector<ghostl::run_task<>>>()};
    std::shared_ptr<std::atomic<size_t>> remaining{std::make_shared<std::atomic<size_t>>(0)};
