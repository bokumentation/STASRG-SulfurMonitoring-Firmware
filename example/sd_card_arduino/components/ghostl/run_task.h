#pragma once
/*
run_task.h - Implementation of a C++20 async coroutines task runner featuring
continue_with. Copyright (c) 2023 Dirk O. Kaar. All rights reserved.
...
template <typename T = void> struct run_task {
    ...
    /// <param name="cont">The continuation function. Caveat: lambda captures can
    /// leak when the function is never invoked due to prior cancellation etc. of
    /// the task.</param>
    ...
    run_task(ghostl::run_task<T> &&other) noexcept
        : task(std::exchange(other.task, {})), continuation(std::exchange(other.continuation, nullptr)),
