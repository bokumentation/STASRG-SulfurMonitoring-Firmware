...
{
    uint8_t buffer[8] = {0, bin2bcd(dt.second()), bin2bcd(dt.minute()), bin2bcd(dt.hour()),
                         0, bin2bcd(dt.day()),    bin2bcd(dt.month()),  bin2bcd(dt.year() - 2000U)
                        };
...
DateTime RTC_DS1307::now()
{
    ...
    return DateTime(bcd2bin(buffer[6]) + 2000U, bcd2bin(buffer[5]), bcd2bin(buffer[4]), bcd2bin(buffer[2]),
