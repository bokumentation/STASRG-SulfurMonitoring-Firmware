...
void ssd1306_display_text_box1(SSD1306_t *dev, int page, int seg, const char *text, int box_width, int text_len,
...
void ssd1306_display_text_box2(SSD1306_t *dev, int page, int seg, const char *text, int box_width, int text_len,
                               bool invert, int delay)
{
    if (page >= dev->_pages) {
        return;
    ...
    for (int _height = 0; _height < height; _height++) {
        for (int index = 0; index < _width; index++) {
            for (int srcBits = 7; srcBits >= 0; srcBits--) {
                ...
                ESP_LOGD(__FUNCTION__, "index=%d offset=%d wk1=0x%x page=%d _seg=%d, wk2=%02x", index, offset, wk1,
                         page, _seg, wk2);
                if (_seg >= 128) {
