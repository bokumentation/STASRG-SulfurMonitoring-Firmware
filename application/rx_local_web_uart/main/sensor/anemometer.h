 ...
 * @param measurement_interval_sec The time window for pulse counting in
 * seconds.
 * @return anemometer_handle_t The handle to the initialized sensor context, or
 * NULL on failure.
 */
...
/**
 * @brief Polls the sensor pin and calculates wind speed if the time window is
 * met.
 *
 ...
 * @return bool True if a new measurement was calculated and updated, false
 * otherwise.
 */
bool anemometer_read_speed(anemometer_handle_t handle, anemometer_data_t *data);
