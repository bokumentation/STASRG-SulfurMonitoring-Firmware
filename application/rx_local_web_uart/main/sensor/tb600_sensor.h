 ...
 * @param data_out Pointer ke struktur tb600b_combined_data_t untuk menyimpan
 * data yang dibaca.
 * @note Fungsi ini mengirim perintah ke sensor, menunggu respons, dan mengurai
 * data yang diterima. Pastikan untuk memanggil fungsi ini dalam konteks task
 * FreeRTOS yang sesuai, karena fungsi ini dapat memblokir hingga data diterima.
 * @note Fungsi ini mengharapkan respons dalam format yang telah ditentukan oleh
 * protokol TB600B/C. Respons harus dimulai dengan header byte yang benar (0xFF,
 * 0x87) dan diakhiri dengan checksum yang valid.
 * @return void
 */
...
/**
 * @brief Sends command, reads and parses sensor data, returning the data struct
 * directly. This is the preferred, safe, high-level access function.
 ...
 * @return tb600b_combined_data_t A structure containing sensor data and a
 * success flag.
 */
tb600b_combined_data_t tb600b_get_data_safe(uart_port_t uart_num, const uint8_t *command, size_t commandSize);

/**
 * @brief Converts gas concentration from micrograms per cubic meter (ug/m³) to
 * parts per million (ppm), correcting for the ambient temperature.
