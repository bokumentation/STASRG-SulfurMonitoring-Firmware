...
{
    tb600b_combined_data_t data = {0, 0, 0, 0}; // Zero-initialize the entire structure
    // (T=0, H=0, Gas=0, Success=false)
    const char *tag =
        (uart_num == UART_NUM_1) ? "SENSOR_H2S_TAG" : "SENSOR_SO2_TAG"; // Placeholder for tag determination

    esp_err_t err = tb600b_read_combined_data(uart_num, command, commandSize, &data);

    if (err != ESP_OK) {
        ESP_LOGE(tag,
                 "Failed to read data from sensor (Error: 0x%X). Returning zeroed "
                 "structure.",
                 err);
    } else {
        data.success = true;
...
{
    // Constant: Molar Volume (Vm) at Standard Temperature (0°C / 273.15 K) and
    // Pressure (1 atm)
    ...
    // The final division by 1000 converts ug to mg (or ug/m³ to mg/m³) to match
    // the standard ppm relationship.
