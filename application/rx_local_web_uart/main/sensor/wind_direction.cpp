...
DESKRIPSI:
Cara kerja:
1. Inisialisasi komunikasi serial software pada pin RX=12, TX=33 dengan baudrate
9600
2. Pada fungsi loop, baca data dari serial software
3. Data yang diterima berbentuk string dengan format *x# dimana x adalah nilai
arah angin (1-8)
4. Konversi nilai x menjadi arah angin dalam bentuk string (utara, timur laut,
...
SoftwareSerial my_Windir_SWSerial(PIN_SENSOR_WIND_DIR_RX,
                                  PIN_SENSOR_WIND_DIR_TX); // RX=12, TX=33
...
void inisialisasiWinDir()
{
    ...
    while (1) {
        if (my_Windir_SWSerial.available()) {
            ...
            // kata lain: mengambil substring antara posisi a+1 dan b. Misal data=*3#,
            // maka diambil karakter ke 1 yaitu 3.
