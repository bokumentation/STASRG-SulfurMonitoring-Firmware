...
/*!
  \brief There was an unexpected, unknown error. If you see this, something went
  incredibly wrong. Your Arduino may be possessed, contact your local exorcist
...
/*!
  \brief Radio chip was not found during initialization. This can be caused by
  specifying wrong chip type in the constructor (i.e. calling SX1272 constructor
  for SX1278 chip) or by a fault in your wiring (incorrect slave select pin).
*/
#define RADIOLIB_ERR_CHIP_NOT_FOUND (-2)

/*!
  \brief Failed to allocate memory for temporary buffer. This can be cause by
  not enough RAM or by passing invalid pointer.
*/
...
/*!
  ...
  This means that the packet was damaged during transmission and should be sent
  again.
*/
...
/*!
  ...
  This means that there is some LoRa device currently transmitting in your
  channel.
*/
#define RADIOLIB_PREAMBLE_DETECTED (-14)

/*!
  \brief No LoRa preambles were detected during channel activity detection. Your
  channel is free.
*/
#define RADIOLIB_CHANNEL_FREE (-15)

/*!
  \brief Real value in SPI register does not match the expected one. This can be
  caused by faulty SPI wiring.
*/
...
/*!
  ...
  For example, this can happen when you try to change LoRa configuration when
  FSK modem is active.
*/
...
/*!
  \brief A `NULL` pointer has been encountered. If you see this, there may be a
...
/*!
  \brief The supplied bit rate to bandwidth ratio is invalid. See the module
...
/*!
  \brief Bit rate / bandwidth / frequency deviation ratio is invalid. See SX126x
...
/*!
  ...
  The specified sleep period is shorter than the time necessary to sleep and
  wake the hardware including TCXO delay, or longer than the maximum possible
*/
...
/*!
  ...
  The specified number of repeaters does not match number of repeater IDs or
  their callsigns.
*/
...
/*!
  \brief Unable to restore existing LoRaWAN session because this node did not
  join any network yet.
*/
...
/*!
  \brief Invalid LoRaWAN uplink port requested by user, or downlink received at
  invalid port.
*/
...
/*!
  \brief User requested to start uplink while still inside RX window or under
  dutycycle.
*/
...
/*!
  \brief Unable to join network because JoinNonce is not higher than saved
  value.
*/
...
/*!
  \brief Uplink payload length at this datarate exceeds the active dwell time
  limitations.
*/
...
/*!
  \brief No JoinAccept was received - check your keys, or otherwise likely a
  range issue!
*/
...
/*!
  \brief The supplied Nonces buffer is discarded as its activation information
  is invalid.
*/
#define RADIOLIB_ERR_NONCES_DISCARDED (-1119)

/*!
  \brief The supplied Session buffer is discarded as it doesn't match the
  Nonces.
*/
...
/*!
  \brief Type used for radio-agnostic IRQ flags. IRQ to enable corresponds to
  the bit index (RadioLibIrq_t). For example, if bit 0 is set, the module will
  enable its RADIOLIB_IRQ_TX_DONE (if it is supported).
*/
