...
void RadioLibBCH::begin(uint8_t n, uint8_t k, uint32_t poly)
{
     ...
     * lookup tables:  index->polynomial form   this->alphaTo[] contains
     * j=alpha**i; polynomial form -> index form  this->indexOf[j=alpha**i] = i
    ...
    for (ii = 2; ii <= rdncy; ii++) {
        ...
        for (jj = ii - 1; jj > 0; jj--) {
            if (this->generator[jj] != 0) {
                this->generator[jj] =
                    this->generator[jj - 1] ^ this->alphaTo[(this->indexOf[this->generator[jj]] + zeros[ii]) % this->n];
            } else {
