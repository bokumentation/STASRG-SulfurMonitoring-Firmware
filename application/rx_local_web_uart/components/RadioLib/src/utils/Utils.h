...
// macros to access bits in byte array, from
...
// the final bit (x^0 term in polynomial notation) is assumed to always be
// present
...
/*!
  \brief Function to scramble or descramble input using a linear feedback shift
  register (LFSR). \param data The input data to (de)scramble. \param len Number
  of input bytes. \param poly Polynomial to use for scrambling. \param init
  Initial LFSR value, sometimes called seed. \param scramble Whether to perform
  scrambling (true) or de-scrambling (false).
*/
...
*/
void rlb_hexdump(const char *level, const uint8_t *data, size_t len, uint32_t offset = 0, uint8_t width = 1,
                 bool be = false);
