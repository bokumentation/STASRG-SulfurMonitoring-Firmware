...
/*! \brief Global-scope function that returns timestamp since start (in
 * microseconds). */
...
public:
    ...
    // these tell RadioLib how are different logic states represented on a given
    // platform
    ...
    */
    ...
    /*!
      ...
      \param falling Value to be used as the "falling" GPIO level change
      direction.
    */
    RadioLibHal(const uint32_t input, const uint32_t output, const uint32_t low, const uint32_t high,
                const uint32_t rising, const uint32_t falling);
    ...
    */
    ...
    // pure virtual methods - these must be implemented by the hardware
    // abstraction for RadioLib to function
    ...
    */
    ...
    /*!
      ...
      \returns Pulse length in microseconds, or 0 if the pulse did not start
      before timeout.
    */
    ...
    */
    ...
    /*!
      \brief Method to produce a square-wave with 50% duty cycle ("tone") of a
      given frequency at some pin. \param pin Pin to be used as the output. \param
      frequency Frequency of the square wave. \param duration Duration of the tone
      in ms. When set to 0, the tone will be infinite.
    */
    ...
    */
    ...
    /*!
      \brief Yield method, called from long loops in multi-threaded environment
