...
int16_t RF69::begin(float freq, float br, float freqDev, float rxBw, int8_t pwr, uint8_t preambleLen)
{
    ...
    while ((i < 10) && !flagFound) {
        ...
        if (version == RADIOLIB_RF69_CHIP_VERSION) {
            ...
            RADIOLIB_DEBUG_BASIC_PRINTLN("RF69 not found! (%d of 10 tries) RADIOLIB_RF69_REG_VERSION == "
                                         "0x%04X, expected 0x0024",
                                         i + 1, version);
...
int16_t RF69::directMode()
{
    ...
    // set DIO mapping
    state = this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_DIO_MAPPING_1,
                                      RADIOLIB_RF69_DIO1_CONT_DCLK | RADIOLIB_RF69_DIO2_CONT_DATA, 5, 2);
    RADIOLIB_ASSERT(state);
...
void RF69::setDio1Action(void (*func)(void))
{
    ...
    this->mod->hal->attachInterrupt(this->mod->hal->pinToInterrupt(this->mod->getGpio()), func,
                                    this->mod->hal->GpioInterruptRising);
}
...
void RF69::setFifoEmptyAction(void (*func)(void))
{
    ...
    // we need to invert the logic here (as compared to setDio1Action), since we
    // are using the "FIFO not empty interrupt"
    this->mod->hal->attachInterrupt(this->mod->hal->pinToInterrupt(this->mod->getGpio()), func,
                                    this->mod->hal->GpioInterruptFalling);
}
...
int16_t RF69::startTransmit(const uint8_t *data, size_t len, uint8_t addr)
{
    ...
    if (len > RADIOLIB_RF69_MAX_PACKET_LENGTH) {
        state =
            this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_DIO_MAPPING_1, RADIOLIB_RF69_DIO1_PACK_FIFO_NOT_EMPTY, 5, 4);
    } else {
    ...
    if ((filter == RADIOLIB_RF69_ADDRESS_FILTERING_NODE) ||
            (filter == RADIOLIB_RF69_ADDRESS_FILTERING_NODE_BROADCAST)) {
...
int16_t RF69::setOOK(bool enable)
{
    ...
    // call setRxBandwidth again, since register values differ based on OOK mode
    // being enabled
...
{
    if ((type != RADIOLIB_RF69_OOK_THRESH_FIXED) && (type != RADIOLIB_RF69_OOK_THRESH_PEAK) &&
            (type != RADIOLIB_RF69_OOK_THRESH_AVERAGE)) {
...
int16_t RF69::setFrequency(float freq)
{
    ...
    if (!(((freq > 290.0f) && (freq < 340.0f)) || ((freq > 431.0f) && (freq < 510.0f)) ||
    ...
    // FRF(23:0) = freq / Fstep = freq * (1 / Fstep) = freq * (2^19 / 32.0) (pag.
    // 17 of datasheet)
...
int16_t RF69::setRxBandwidth(float rxBw)
{
    ...
    for (int8_t e = 7; e >= 0; e--) {
        for (int8_t m = 2; m >= 0; m--) {
            float point = (RADIOLIB_RF69_CRYSTAL_FREQ * 1000000.0f) /
                          (((4 * m) + 16) * ((uint32_t)1 << (e + (this->ookEnabled ? 3 : 2))));
...
int16_t RF69::getFrequencyDeviation(float *freqDev)
{
    ...
    *freqDev = (1000.0f * fdev * RADIOLIB_RF69_CRYSTAL_FREQ) / (uint32_t(1) << RADIOLIB_RF69_DIV_EXPONENT);
...
int16_t RF69::setOutputPower(int8_t pwr, bool highPower)
{
    ...
    if (highPower) {
        ...
        if (pwr <= 10) {
            ...
            state = this->mod->SPIsetRegValue(
                        RADIOLIB_RF69_REG_PA_LEVEL,
                        RADIOLIB_RF69_PA0_OFF | RADIOLIB_RF69_PA1_ON | RADIOLIB_RF69_PA2_OFF | (power + 18), 7, 0);
        } else if (pwr <= 17) {
            // 13 to 17 dBm, both PAs required
            state = this->mod->SPIsetRegValue(
                        RADIOLIB_RF69_REG_PA_LEVEL,
                        RADIOLIB_RF69_PA0_OFF | RADIOLIB_RF69_PA1_ON | RADIOLIB_RF69_PA2_ON | (power + 14), 7, 0);
        } else {
            // 18 - 20 dBm, both PAs and hig power settings required
            state = this->mod->SPIsetRegValue(
                        RADIOLIB_RF69_REG_PA_LEVEL,
                        RADIOLIB_RF69_PA0_OFF | RADIOLIB_RF69_PA1_ON | RADIOLIB_RF69_PA2_ON | (power + 11), 7, 0);
        }
        ...
        state = this->mod->SPIsetRegValue(
                    RADIOLIB_RF69_REG_PA_LEVEL,
                    RADIOLIB_RF69_PA0_ON | RADIOLIB_RF69_PA1_OFF | RADIOLIB_RF69_PA2_OFF | (power + 18), 7, 0);
    }
...
int16_t RF69::setNodeAddress(uint8_t nodeAddr)
{
    ...
    int16_t state =
        this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_PACKET_CONFIG_1, RADIOLIB_RF69_ADDRESS_FILTERING_NODE, 2, 1);
    RADIOLIB_ASSERT(state);
...
int16_t RF69::setBroadcastAddress(uint8_t broadAddr)
{
    ...
    int16_t state = this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_PACKET_CONFIG_1,
                                              RADIOLIB_RF69_ADDRESS_FILTERING_NODE_BROADCAST, 2, 1);
    RADIOLIB_ASSERT(state);
...
int16_t RF69::disableAddressFiltering()
{
    ...
    int16_t state =
        this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_PACKET_CONFIG_1, RADIOLIB_RF69_ADDRESS_FILTERING_OFF, 2, 1);
    RADIOLIB_ASSERT(state);
...
int16_t RF69::enableSyncWordFiltering(uint8_t maxErrBits)
{
    ...
    int16_t state = this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_SYNC_CONFIG,
                                              RADIOLIB_RF69_SYNC_ON | RADIOLIB_RF69_FIFO_FILL_CONDITION_SYNC, 7, 6);
    RADIOLIB_ASSERT(state);
...
int16_t RF69::disableSyncWordFiltering()
{
    ...
    return (this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_SYNC_CONFIG,
                                      RADIOLIB_RF69_SYNC_OFF | RADIOLIB_RF69_FIFO_FILL_CONDITION, 7, 6));
}

int16_t RF69::enableContinuousModeBitSync()
{
    int16_t state =
        this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_DATA_MODUL, RADIOLIB_RF69_CONTINUOUS_MODE_WITH_SYNC, 6, 5);
    if (state == RADIOLIB_ERR_NONE) {
        this->bitSync = true;
    }
...
int16_t RF69::config()
{
    ...
    // set operation modes
    state = this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_OP_MODE, RADIOLIB_RF69_SEQUENCER_ON | RADIOLIB_RF69_LISTEN_OFF,
                                      7, 6);
    RADIOLIB_ASSERT(state);
    ...
    state =
        this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_DATA_MODUL, RADIOLIB_RF69_PACKET_MODE | RADIOLIB_RF69_FSK, 6, 3);
    ...
    state = this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_PACKET_CONFIG_1,
                                      RADIOLIB_RF69_PACKET_FORMAT_VARIABLE | RADIOLIB_RF69_DC_FREE_NONE |
                                      RADIOLIB_RF69_CRC_ON | RADIOLIB_RF69_CRC_AUTOCLEAR_ON |
                                      RADIOLIB_RF69_ADDRESS_FILTERING_OFF,
                                      7, 1);
    state |= this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_PACKET_CONFIG_2, RADIOLIB_RF69_INTER_PACKET_RX_DELAY, 7, 4);
    state |= this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_PACKET_CONFIG_2,
                                       RADIOLIB_RF69_AUTO_RX_RESTART_ON | RADIOLIB_RF69_AES_OFF, 1, 0);
    RADIOLIB_ASSERT(state);
    ...
    // set FIFO threshold
    state =
        this->mod->SPIsetRegValue(RADIOLIB_RF69_REG_FIFO_THRESH,
                                  RADIOLIB_RF69_TX_START_CONDITION_FIFO_NOT_EMPTY | RADIOLIB_RF69_FIFO_THRESH, 7, 0);
    RADIOLIB_ASSERT(state);
