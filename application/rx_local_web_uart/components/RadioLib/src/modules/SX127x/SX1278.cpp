...
{
    // execute common part
    const uint8_t versions[] = {RADIOLIB_SX1278_CHIP_VERSION, RADIOLIB_SX1278_CHIP_VERSION_ALT,
                                RADIOLIB_SX1278_CHIP_VERSION_RFM9X
                               };
...
int16_t SX1278::beginFSK(float freq, float br, float freqDev, float rxBw, int8_t power, uint16_t preambleLength,
                         bool enableOOK)
{
    // execute common part
    const uint8_t versions[] = {RADIOLIB_SX1278_CHIP_VERSION, RADIOLIB_SX1278_CHIP_VERSION_ALT,
...
int16_t SX1278::setBandwidth(float bw)
{
    ...
    if (state == RADIOLIB_ERR_NONE) {
        ...
        // calculate symbol length and set low data rate optimization, if
        ...
        if (this->ldroAuto) {
            ...
            if (symbolLength >= 16.0f) {
                ...
                state =
                    mod->SPIsetRegValue(RADIOLIB_SX1278_REG_MODEM_CONFIG_3, RADIOLIB_SX1278_LOW_DATA_RATE_OPT_ON, 3, 3);
            } else {
                this->ldroEnabled = false;
                state = mod->SPIsetRegValue(RADIOLIB_SX1278_REG_MODEM_CONFIG_3, RADIOLIB_SX1278_LOW_DATA_RATE_OPT_OFF,
                                            3, 3);
            }
        }
    }
...
int16_t SX1278::setOutputPower(int8_t power, bool forceRfo)
{
    ...
    if (useRfo) {
        ...
        if (power != 20) {
            ...
            // power is 20 dBm, enable PA1 + PA2 on PA_BOOST and enable high power
            // control
            state |= mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PA_CONFIG, RADIOLIB_SX127X_PA_SELECT_BOOST, 7, 7);
...
int16_t SX1278::setCRC(bool enable, bool mode)
{
    ...
    if (getActiveModem() == RADIOLIB_SX127X_LORA) {
        ...
        if (mode) {
            return (
                       mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, RADIOLIB_SX127X_CRC_WHITENING_TYPE_IBM, 0, 0));
        } else {
            return (mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, RADIOLIB_SX127X_CRC_WHITENING_TYPE_CCITT,
                                        0, 0));
        }
    }
}
...
int16_t SX1278::setSpreadingFactorRaw(uint8_t newSpreadingFactor)
{
    ...
    if (newSpreadingFactor == RADIOLIB_SX127X_SF_6) {
        ...
        state |= mod->SPIsetRegValue(RADIOLIB_SX127X_REG_MODEM_CONFIG_2,
                                     RADIOLIB_SX127X_SF_6 | RADIOLIB_SX127X_TX_MODE_SINGLE, 7, 3);
        ...
        state |= mod->SPIsetRegValue(RADIOLIB_SX127X_REG_MODEM_CONFIG_2,
                                     newSpreadingFactor | RADIOLIB_SX127X_TX_MODE_SINGLE, 7, 3);
        state |=
            mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DETECT_OPTIMIZE, RADIOLIB_SX127X_DETECT_OPTIMIZE_SF_7_12, 2, 0);
        state |=
            mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DETECTION_THRESHOLD, RADIOLIB_SX127X_DETECTION_THRESHOLD_SF_7_12);
    }
    return (state);
}
...
void SX1278::errataFix(bool rx)
{
    ...
    // shift the freqency up when receiving, or restore the original when
    // transmitting
    if (rx) {
