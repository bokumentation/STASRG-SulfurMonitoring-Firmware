...
int16_t SX1272::beginFSK(float freq, float br, float freqDev, float rxBw, int8_t power, uint16_t preambleLength,
                         bool enableOOK)
{
    // execute common part
    uint8_t version = RADIOLIB_SX1272_CHIP_VERSION;
    ...
    if (state == RADIOLIB_ERR_NONE) {
        ...
        if (this->ldroAuto) {
            ...
            if (symbolLength >= 16.0f) {
                ...
                state =
                    mod->SPIsetRegValue(RADIOLIB_SX127X_REG_MODEM_CONFIG_1, RADIOLIB_SX1272_LOW_DATA_RATE_OPT_ON, 0, 0);
            } else {
                this->ldroEnabled = false;
                state = mod->SPIsetRegValue(RADIOLIB_SX127X_REG_MODEM_CONFIG_1, RADIOLIB_SX1272_LOW_DATA_RATE_OPT_OFF,
                                            0, 0);
            }
        }
    }
...
int16_t SX1272::setOutputPower(int8_t power, bool forceRfo)
{
    ...
    if (useRfo) {
        ...
        if (power <= 17) {
            ...
            // power is 18 - 20 dBm, enable PA1 + PA2 on PA_BOOST and enable high
            // power control
            state |= mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PA_CONFIG, RADIOLIB_SX127X_PA_SELECT_BOOST, 7, 7);
...
int16_t SX1272::setSpreadingFactorRaw(uint8_t newSpreadingFactor)
{
    ...
    if (newSpreadingFactor == RADIOLIB_SX127X_SF_6) {
        this->implicitHdr = true;
        state |= mod->SPIsetRegValue(
                     RADIOLIB_SX127X_REG_MODEM_CONFIG_1,
                     RADIOLIB_SX1272_HEADER_IMPL_MODE |
                     (SX127x::crcEnabled ? RADIOLIB_SX1272_RX_CRC_MODE_ON : RADIOLIB_SX1272_RX_CRC_MODE_OFF),
                     2, 1);
        ...
        state |= mod->SPIsetRegValue(
                     RADIOLIB_SX127X_REG_MODEM_CONFIG_1,
                     RADIOLIB_SX1272_HEADER_EXPL_MODE |
                     (SX127x::crcEnabled ? RADIOLIB_SX1272_RX_CRC_MODE_ON : RADIOLIB_SX1272_RX_CRC_MODE_OFF),
                     2, 1);
        state |= mod->SPIsetRegValue(RADIOLIB_SX127X_REG_MODEM_CONFIG_2,
