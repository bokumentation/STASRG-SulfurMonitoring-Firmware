...
#define RADIOLIB_SX127X_OUTPUT_POWER 0b00001111    //  3     0   output power: P_out = 2 + OUTPUT_POWER [dBm] for
//  PA_SELECT_BOOST
...
#define RADIOLIB_SX127X_PREAMBLE_LENGTH_MSB                                                                            \
    0b00000000 //  7     0   2-byte preamble length setting: l_P = PREAMBLE_LENGTH
//  + 4.25
...
#define RADIOLIB_SX127X_HOP_PERIOD_OFF 0b00000000 //  7     0   number of periods between frequency hops; 0 =
//  disabled
...
#define RADIOLIB_SX127X_CLEAR_IRQ_FLAG_CAD_DETECTED                                                                    \
    0b00000001 //  0     0   valid LoRa signal detected during CAD operation
...
#define RADIOLIB_SX127X_MASK_IRQ_FLAG_CAD_DETECTED                                                                     \
    0b11111110 //  0     0   valid LoRa signal detected during CAD operation
...
// NOTE: FSK register names that are conflicting with LoRa registers are marked
// with "_FSK" suffix
...
#define RADIOLIB_SX127X_BITRATE_MSB 0x1A //  7     0   bit rate setting: BitRate = F(XOSC)/(BITRATE +
//  BITRATE_FRAC/16)
...
#define RADIOLIB_SX127X_RESTART_RX_ON_COLLISION_OFF                                                                    \
    0b00000000 //  7     7   automatic receiver restart disabled (default)
#define RADIOLIB_SX127X_RESTART_RX_ON_COLLISION_ON                                                                     \
    0b10000000 //  7     7   automatically restart receiver if it gets saturated
//  or on packet collision
#define RADIOLIB_SX127X_RESTART_RX_WITHOUT_PLL_LOCK                                                                    \
    0b01000000 //  6     6   manually restart receiver without frequency change
#define RADIOLIB_SX127X_RESTART_RX_WITH_PLL_LOCK                                                                       \
    0b00100000                                     //  5     5   manually restart receiver with frequency change
...
#define RADIOLIB_SX127X_RX_TRIGGER_BOTH 0b00000111 //  2     0                        RSSI interrupt and preamble
//  detected
...
#define RADIOLIB_SX127X_RSSI_SMOOTHING_SAMPLES_8                                                                       \
    0b00000010 //  2     0                                       8 (default)
...
#define RADIOLIB_SX127X_RSSI_COLLISION_THRESHOLD                                                                       \
    0x0A //  7     0   RSSI threshold in dB that will be considered a collision,
//  default value: 10 dB

// RADIOLIB_SX127X_REG_RSSI_THRESH
#define RADIOLIB_SX127X_RSSI_THRESHOLD 0xFF //  7     0   RSSI threshold that will trigger RSSI interrupt,
//  RssiThreshold = RSSI_THRESHOLD / 2 [dBm]

// RADIOLIB_SX127X_REG_RX_BW
#define RADIOLIB_SX127X_RX_BW_MANT_16 0b00000000 //  4     3   channel filter bandwidth: RxBw = F(XOSC) / (RxBwMant
//  * 2^(RxBwExp + 2)) [kHz]
...
#define RADIOLIB_SX127X_BIT_SYNC_OFF 0b00000000 //  5     5   bit synchronizer disabled (not allowed in packet
//  mode)
...
#define RADIOLIB_SX127X_OOK_PEAK_THRESH_DEC_1_1_CHIP                                                                   \
    0b00000000 //  7     5   OOK demodulator step period: once per chip (default)
#define RADIOLIB_SX127X_OOK_PEAK_THRESH_DEC_1_2_CHIP                                                                   \
    0b00100000 //  7     5                                once every 2 chips
#define RADIOLIB_SX127X_OOK_PEAK_THRESH_DEC_1_4_CHIP                                                                   \
    0b01000000 //  7     5                                once every 4 chips
#define RADIOLIB_SX127X_OOK_PEAK_THRESH_DEC_1_8_CHIP                                                                   \
    0b01100000 //  7     5                                once every 8 chips
#define RADIOLIB_SX127X_OOK_PEAK_THRESH_DEC_2_1_CHIP                                                                   \
    0b10000000 //  7     5                                2 times per chip
#define RADIOLIB_SX127X_OOK_PEAK_THRESH_DEC_4_1_CHIP                                                                   \
    0b10100000 //  7     5                                4 times per chip
#define RADIOLIB_SX127X_OOK_PEAK_THRESH_DEC_8_1_CHIP                                                                   \
    0b11000000 //  7     5                                8 times per chip
#define RADIOLIB_SX127X_OOK_PEAK_THRESH_DEC_16_1_CHIP                                                                  \
    0b11100000                                             //  7     5                                16 times per chip
...
#define RADIOLIB_SX127X_OOK_AVG_THRESH_FILT_32_PI                                                                      \
    0b00000000 //  1     0   OOK average filter coefficient: chip rate / 32*pi
#define RADIOLIB_SX127X_OOK_AVG_THRESH_FILT_8_PI                                                                       \
    0b00000001 //  1     0                                   chip rate / 8*pi
#define RADIOLIB_SX127X_OOK_AVG_THRESH_FILT_4_PI                                                                       \
    0b00000010 //  1     0                                   chip rate / 4*pi
//  (default)
#define RADIOLIB_SX127X_OOK_AVG_THRESH_FILT_2_PI                                                                       \
    0b00000011 //  1     0                                   chip rate / 2*pi
...
#define RADIOLIB_SX127X_AFC_AUTO_CLEAR_OFF 0b00000000 //  0     0   AFC register will not be cleared at the start of AFC
//  (default)
...
#define RADIOLIB_SX127X_PREAMBLE_DETECTOR_TOL                                                                          \
    0x0A //  4     0   default number of tolerated errors per chip (4 chips per
//  bit)

// RADIOLIB_SX127X_REG_RX_TIMEOUT_1
#define RADIOLIB_SX127X_TIMEOUT_RX_RSSI_OFF                                                                            \
    0x00 //  7     0   disable receiver timeout when RSSI interrupt doesn't occur
//  (default)

// RADIOLIB_SX127X_REG_RX_TIMEOUT_2
#define RADIOLIB_SX127X_TIMEOUT_RX_PREAMBLE_OFF                                                                        \
    0x00 //  7     0   disable receiver timeout when preamble interrupt doesn't
//  occur (default)

// RADIOLIB_SX127X_REG_RX_TIMEOUT_3
#define RADIOLIB_SX127X_TIMEOUT_SIGNAL_SYNC_OFF 0x00 //  7     0   disable receiver timeout when sync address interrupt
//  doesn't occur (default)
...
#define RADIOLIB_SX127X_AUTO_RESTART_RX_MODE_OFF                                                                       \
    0b00000000 //  7     6   Rx mode restart after packet reception: disabled
#define RADIOLIB_SX127X_AUTO_RESTART_RX_MODE_NO_PLL                                                                    \
    0b01000000 //  7     6                                           enabled,
//  don't wait for PLL lock
#define RADIOLIB_SX127X_AUTO_RESTART_RX_MODE_PLL                                                                       \
    0b10000000 //  7     6                                           enabled, wait
//  for PLL lock (default)
...
#define RADIOLIB_SX127X_CRC_AUTOCLEAR_OFF 0b00001000  //  3     3   keep FIFO on CRC mismatch, issue payload ready
//  interrupt
#define RADIOLIB_SX127X_CRC_AUTOCLEAR_ON 0b00000000 //  3     3   clear FIFO on CRC mismatch, do not issue payload
//  ready interrupt
...
#define RADIOLIB_SX127X_CRC_WHITENING_TYPE_CCITT                                                                       \
    0b00000000 //  0     0   CRC and whitening algorithms: CCITT CRC with standard
//  whitening (default)
#define RADIOLIB_SX127X_CRC_WHITENING_TYPE_IBM                                                                         \
    0b00000001 //  0     0                                 IBM CRC with alternate
//  whitening
...
#define RADIOLIB_SX127X_TX_START_FIFO_LEVEL 0b00000000 //  7     7   start packet transmission when: number of bytes in
//  FIFO exceeds FIFO_THRESHOLD
#define RADIOLIB_SX127X_TX_START_FIFO_NOT_EMPTY                                                                        \
    0b10000000 //  7     7                                   at least one byte in
//  FIFO (default)
...
#define RADIOLIB_SX127X_IDLE_MODE_STANDBY 0b00000000 //  5     5   chip mode during sequencer idle mode: standby
//  (default)
#define RADIOLIB_SX127X_IDLE_MODE_SLEEP 0b00100000 //  5     5                                         sleep
#define RADIOLIB_SX127X_FROM_START_LP_SELECTION                                                                        \
    0b00000000 //  4     3   mode that will be set after starting sequencer: low
//  power selection (default)
...
#define RADIOLIB_SX127X_LP_SELECTION_SEQ_OFF 0b00000000 //  2     2   mode that will be set after exiting low power
//  selection: sequencer off (default)
#define RADIOLIB_SX127X_LP_SELECTION_IDLE 0b00000100  //  2     2 idle state
#define RADIOLIB_SX127X_FROM_IDLE_TRANSMIT 0b00000000 //  1     1   mode that will be set after exiting idle mode:
//  transmit (default)
#define RADIOLIB_SX127X_FROM_IDLE_RECEIVE 0b00000010 //  1     1 receive
#define RADIOLIB_SX127X_FROM_TRANSMIT_LP_SELECTION                                                                     \
    0b00000000 //  0     0   mode that will be set after exiting transmit mode:
//  low power selection (default)
...
#define RADIOLIB_SX127X_FROM_RECEIVE_PACKET_RECEIVED_PAYLOAD                                                           \
    0b00100000 //  7     5   mode that will be set after exiting receive mode:
//  packet received on payload ready interrupt (default)
#define RADIOLIB_SX127X_FROM_RECEIVE_LP_SELECTION                                                                      \
    0b01000000 //  7     5                                                     low
//  power selection
...
#define RADIOLIB_SX127X_FROM_RECEIVE_SEQ_OFF_PREAMBLE_DETECT                                                           \
    0b11000000                                             //  7     5 sequencer off on preamble detect interrupt
#define RADIOLIB_SX127X_FROM_RX_TIMEOUT_RECEIVE 0b00000000 //  4     3   mode that will be set after Rx timeout: receive
//  (default)
#define RADIOLIB_SX127X_FROM_RX_TIMEOUT_TRANSMIT                                                                       \
    0b00001000 //  4     3                                           transmit
#define RADIOLIB_SX127X_FROM_RX_TIMEOUT_LP_SELECTION                                                                   \
    0b00010000 //  4     3                                           low power
//  selection
#define RADIOLIB_SX127X_FROM_RX_TIMEOUT_SEQ_OFF                                                                        \
    0b00011000 //  4     3                                           sequencer off
#define RADIOLIB_SX127X_FROM_PACKET_RECEIVED_SEQ_OFF                                                                   \
    0b00000000 //  2     0   mode that will be set after packet received:
//  sequencer off (default)
#define RADIOLIB_SX127X_FROM_PACKET_RECEIVED_TRANSMIT                                                                  \
    0b00000001 //  2     0                                                transmit
#define RADIOLIB_SX127X_FROM_PACKET_RECEIVED_LP_SELECTION                                                              \
    0b00000010 //  2     0                                                low
//  power selection
#define RADIOLIB_SX127X_FROM_PACKET_RECEIVED_RECEIVE_FS                                                                \
    0b00000011 //  2     0                                                receive
//  via FS
#define RADIOLIB_SX127X_FROM_PACKET_RECEIVED_RECEIVE                                                                   \
    0b00000100 //  2     0                                                receive
...
#define RADIOLIB_SX127X_TEMP_CHANGED 0b00001000       //  3     3   temperature changed more than TEMP_THRESHOLD since
//  last calibration
#define RADIOLIB_SX127X_TEMP_THRESHOLD_5_DEG_C 0b00000000 //  2     1   temperature change threshold: 5 deg. C
#define RADIOLIB_SX127X_TEMP_THRESHOLD_10_DEG_C                                                                        \
    0b00000010 //  2     1                                 10 deg. C (default)
...
#define RADIOLIB_SX127X_FAST_HOP_OFF 0b00000000 //  7     7   carrier frequency validated when FRF registers are
//  written
#define RADIOLIB_SX127X_FAST_HOP_ON 0b10000000 //  7     7   carrier frequency validated when FS modes are
//  requested

// SX1272_REG_TCXO + SX1278_REG_TCXO
#define RADIOLIB_SX127X_TCXO_INPUT_EXTERNAL 0b00000000 //  4     4   use external crystal oscillator
#define RADIOLIB_SX127X_TCXO_INPUT_EXTERNAL_CLIPPED                                                                    \
    0b00010000 //  4     4   use external crystal oscillator clipped sine
//  connected to XTA pin
...
/*!
  \class SX127x
  \brief Base class for SX127x series. All derived classes for SX127x (e.g.
  SX1278 or SX1272) inherit from this base class. This class should not be
  instantiated directly from Arduino sketch, only from its derived classes.
*/
...
public:
    ...
    /*!
      \brief Default constructor. Called internally when creating new LoRa
      instance. \param mod Instance of Module that will be used to communicate
      with the %LoRa chip.
    */
    ...
    /*!
      \brief Initialization method. Will be called with appropriate parameters
      when calling initialization method from derived class. \param chipVersions
      Array of possible values in SPI version register. Used to verify the
      connection and hardware version. \param numVersions Number of possible chip
      versions. \param syncWord %LoRa sync word. \param preambleLength Length of
      %LoRa transmission preamble in symbols. \returns \ref status_codes
    */
    int16_t begin(const uint8_t *chipVersions, uint8_t numVersions, uint8_t syncWord, uint16_t preambleLength);

    /*!
      \brief Reset method. Will reset the chip to the default state using RST pin.
      Should be implemented in derived class since SX1272 and SX1278
      implementations differ.
    */
    virtual void reset();

    /*!
      \brief Initialization method for FSK modem. Will be called with appropriate
      parameters when calling FSK initialization method from derived class. \param
      chipVersions Array of possible values in SPI version register. Used to
      verify the connection and hardware version. \param numVersions Number of
      possible chip versions. \param freqDev Frequency deviation of the FSK
      transmission in kHz. \param rxBw Receiver bandwidth in kHz. \param
      preambleLength Length of FSK preamble in bits. \param enableOOK Flag to
      specify OOK mode. This modulation is similar to FSK. \returns \ref
      status_codes
    */
    int16_t beginFSK(const uint8_t *chipVersions, uint8_t numVersions, float freqDev, float rxBw,
                     uint16_t preambleLength, bool enableOOK);

    /*!
      \brief Binary transmit method. Will transmit arbitrary binary data up to 255
      bytes long using %LoRa or up to 63 bytes using FSK modem. For overloads to
      transmit Arduino String or C-string, see PhysicalLayer::transmit. \param
      data Binary data that will be transmitted. \param len Length of binary data
      to transmit (in bytes). \param addr Node address to transmit the packet to.
      Only used in FSK mode. \returns \ref status_codes
    */
    int16_t transmit(const uint8_t *data, size_t len, uint8_t addr = 0) override;

    /*!
      \brief Binary receive method. Will attempt to receive arbitrary binary data
      up to 255 bytes long using %LoRa or up to 63 bytes using FSK modem. For
      overloads to receive Arduino String, see PhysicalLayer::receive. \param data
      Pointer to array to save the received binary data. \param len Number of
      bytes that will be received. Must be known in advance for binary
      transmissions. \param timeout Reception timeout in milliseconds. If set to
      0, timeout period will be calculated automatically based on the radio
      configuration. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Sets the %LoRa module to sleep to save power. %Module will not be
      able to transmit or receive any data while in sleep mode. %Module will wake
      up automatically when methods like transmit or receive are called. \returns
      \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Sets the %LoRa module to standby.
      \param mode Standby mode to be used. No effect, implemented only for
      PhysicalLayer compatibility. \returns \ref status_codes
    */
    int16_t standby(uint8_t mode) override;

    /*!
      \brief Enables direct transmission mode on pins DIO1 (clock) and DIO2
      (data). While in direct mode, the module will not be able to transmit or
      receive packets. Can only be activated in FSK mode. \param frf 24-bit raw
      frequency value to start transmitting at. Required for quick frequency
      shifts in RTTY. \returns \ref status_codes
    */
    int16_t transmitDirect(uint32_t frf = 0) override;

    /*!
      \brief Enables direct reception mode on pins DIO1 (clock) and DIO2 (data).
      While in direct mode, the module will not be able to transmit or receive
      packets. Can only be activated in FSK mode. \returns \ref status_codes
    */
    int16_t receiveDirect() override;

    /*!
      \brief Disables direct mode and enables packet mode, allowing the module to
      receive packets. Can only be activated in FSK mode. \returns \ref
      status_codes
    */
    int16_t packetMode();
    ...
    */
    ...
    /*!
      \brief Sets interrupt service routine to call when a channel scan is
      finished. \param func ISR to call.
    */
    void setChannelScanAction(void (*func)(void)) override;

    /*!
      \brief Clears interrupt service routine to call when a channel scan is
      finished.
    */
    ...
    */
    ...
    /*!
      ...
      \param remLen Pointer to a counter holding the number of bytes that have
      been transmitted so far. \returns True when a complete packet is sent, false
      if more data is needed.
    */
    bool fifoAdd(uint8_t *data, int totalLen, int *remLen);

    /*!
      \brief Set interrupt service routine function to call when FIFO is
      sufficiently full to read. \param data Pointer to a buffer that stores the
      receive data. \param totalLen Total number of bytes to receive. \param
      rcvLen Pointer to a counter holding the number of bytes that have been
      received so far. \returns True when a complete packet is received, false if
      more data is needed.
    */
    ...
    */
    ...
    /*!
      \brief Reads data that was received after calling startReceive method. When
      the packet length is not known in advance, getPacketLength method must be
      called BEFORE calling readData! \param data Pointer to array to save the
      received binary data. \param len Number of bytes that will be read. When set
      to 0, the packet length will be retrieved automatically. When more bytes
      than received are requested, only the number of bytes requested will be
      returned. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Interrupt-driven channel activity detection method. DIO0 will be
      activated when LoRa preamble is detected. DIO1 will be activated if there's
      no preamble detected before timeout. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Sets current limit for over current protection at transmitter
      amplifier. Allowed values range from 45 to 120 mA in 5 mA steps and 120 to
      240 mA in 10 mA steps. \param currentLimit Current limit to be set (in mA).
      \returns \ref status_codes
    */
    int16_t setCurrentLimit(uint8_t currentLimit);

    /*!
      \brief Sets %LoRa or FSK preamble length. Allowed values range from 6 to
      65535 in %LoRa mode or 0 to 65535 in FSK mode. \param preambleLength
      Preamble length to be set (in symbols when in LoRa mode or bits in FSK
      mode). \returns \ref status_codes
    */
    int16_t setPreambleLength(size_t preambleLength) override;

    /*!
      \brief Invert FSK preamble polarity. The default (non-inverted) is 0x55, the
      inverted is 0xAA. \param enable Preamble polarity in FSK mode - 0xAA when
      true, 0x55 when false. \returns \ref status_codes
    */
    int16_t invertPreamble(bool enable);

    /*!
      \brief Gets frequency error of the latest received packet.
      \param autoCorrect When set to true, frequency will be automatically
      corrected. \returns Frequency error in Hz.
    */
    float getFrequencyError(bool autoCorrect = false);

    /*!
      \brief Gets current AFC error.
      \returns Frequency offset from RF in Hz if AFC is enabled and triggered,
      zero otherwise.
    */
    float getAFCError();

    /*!
      \brief Gets signal-to-noise ratio of the latest received packet. Only
      available in LoRa mode. \returns Last packet signal-to-noise ratio (SNR).
    */
    ...
    */
    ...
    /*!
      \brief Sets FSK frequency deviation from carrier frequency. Allowed values
      depend on bit rate setting and must be lower than 200 kHz. Only available in
      FSK mode. \param freqDev Frequency deviation to be set (in kHz). \returns
      \ref status_codes
    */
    int16_t setFrequencyDeviation(float freqDev) override;

    /*!
      \brief Sets FSK receiver bandwidth. Allowed values range from 2.6 to 250
      kHz. Only available in FSK mode. \param rxBw Receiver bandwidth to be set
      (in kHz). \returns \ref status_codes
    */
    int16_t setRxBandwidth(float rxBw);

    /*!
      \brief Sets FSK automatic frequency correction bandwidth. Allowed values
      range from 2.6 to 250 kHz. Only available in FSK mode. \param afcBw Receiver
      AFC bandwidth to be set (in kHz). \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param trigger one from SX127X_RX_TRIGGER_NONE,
      SX127X_RX_TRIGGER_RSSI_INTERRUPT, SX127X_RX_TRIGGER_PREAMBLE_DETECT,
      SX127X_RX_TRIGGER_BOTH \return \ref status_codes
    */
    int16_t setAFCAGCTrigger(uint8_t trigger);

    /*!
      \brief Sets FSK sync word. Allowed sync words are up to 8 bytes long and can
      not contain null bytes. Only available in FSK mode. \param syncWord Sync
      word array. \param len Sync word length (in bytes). \returns \ref
      status_codes
    */
    int16_t setSyncWord(uint8_t *syncWord, size_t len) override;

    /*!
      \brief Sets FSK node address. Calling this method will enable address
      filtering. Only available in FSK mode. \param nodeAddr Node address to be
      set. \returns \ref status_codes
    */
    int16_t setNodeAddress(uint8_t nodeAddr);

    /*!
      \brief Sets FSK broadcast address. Calling this method will enable address
      filtering. Only available in FSK mode. \param broadAddr Broadcast address to
      be set. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param type Threshold type: SX127X_OOK_THRESH_PEAK(default),
      SX127X_OOK_THRESH_FIXED, SX127X_OOK_THRESH_AVERAGE \returns \ref
      status_codes
    */
    ...
    */
    ...
    /*!
      \brief Fixed threshold for the Data Slicer in OOK mode or floor threshold
      for the Data Slicer in OOK when Peak mode is used. \param value Threshold
      level in steps of 0.5 dB. \returns \ref status_codes
    */
    ...
    /*!
      ...
      (default), RADIOLIB_SX127X_OOK_PEAK_THRESH_STEP_1_0_DB,
      RADIOLIB_SX127X_OOK_PEAK_THRESH_STEP_1_5_DB,
      RADIOLIB_SX127X_OOK_PEAK_THRESH_STEP_2_0_DB,
      RADIOLIB_SX127X_OOK_PEAK_THRESH_STEP_3_0_DB,
      RADIOLIB_SX127X_OOK_PEAK_THRESH_STEP_4_0_DB,
      RADIOLIB_SX127X_OOK_PEAK_THRESH_STEP_5_0_DB,
      RADIOLIB_SX127X_OOK_PEAK_THRESH_STEP_6_0_DB \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param update Update received packet length. Will return cached value when
      set to false. \returns Length of last received packet in bytes.
    */
    size_t getPacketLength(bool update = true) override;

    /*!
      \brief Get LoRa header information from last received packet. Only valid in
      explicit header mode. \param cr Pointer to variable to store the coding
      rate. \param hasCRC Pointer to variable to store the CRC status. \returns
      \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \returns The total number of LoRa symbols, including preamble, sync and
      possible header.
    */
    ...
    */
    ...
    /*!
      \brief Calculate the expected time-on-air for a given modem, data rate,
      packet configuration and payload size. \param modem Modem type. \param dr
      Data rate. \param pc Packet config. \param len Payload length in bytes.
      \returns Expected time-on-air in microseconds.
    */
    RadioLibTime_t calculateTimeOnAir(ModemType_t modem, DataRate_t dr, PacketConfig_t pc, size_t len) override;

    /*!
      \brief Calculate the timeout value for this specific module / series (in
      number of symbols or units of time) \param timeoutUs Timeout in microseconds
      to listen for \returns Timeout value in a unit that is specific for the used
      module
    */
    ...
    */
    ...
    /*!
      \brief Set interrupt on DIO1 to be sent on a specific IRQ bit (e.g.
      RxTimeout, CadDone). NOTE: Unlike other modules that support IRQ abstraction
      (SX126x, LR11x0, etc.), SX127x cannot configure multiple IRQs to signal
      using the same DIOx pin. This method tries to configure IRQs in a "best
      effort" approach, and will skip conflicting flags. RADIOLIB_ERR_INVALID_IRQ
      will be returned in this case. \param irq Module-specific IRQ flags.
      \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      numSamples = 2 ^ (1 + smoothingSamples), allowed values are in range 0 (2
      samples) - 7 (256 samples) \param offset Signed RSSI offset that will be
      automatically compensated. 1 dB per LSB, defaults to 0, allowed values are
      in range -16 dB to +15 dB. \returns \ref status_codes
    */
    ...
    /*!
      ...
      Allowed values are RADIOLIB_ENCODING_NRZ, RADIOLIB_ENCODING_MANCHESTER and
      RADIOLIB_ENCODING_WHITENING. \param encoding Encoding to be used. \returns
      \ref status_codes
    */
    int16_t setEncoding(uint8_t encoding) override;

    /*!
      \brief Reads currently active IRQ flags, can be used to check which event
      caused an interrupt. In LoRa mode, this is the content of
      SX127X_REG_IRQ_FLAGS register. In FSK mode, this is the contents of
      SX127X_REG_IRQ_FLAGS_2 (MSB) and SX127X_REG_IRQ_FLAGS_1 (LSB) registers.
      \returns IRQ flags.
    */
    ...
    */
    ...
    /*!
      \brief Reads uncalibrated temperature value. This function will change
      operating mode and should not be called during Tx, Rx or CAD. \returns
    ...
    */
    ...
    /*!
      \brief Read version SPI register. Should return SX1278_CHIP_VERSION (0x12)
      or SX1272_CHIP_VERSION (0x22) if SX127x is connected and working. \returns
      Version register contents or \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Set interrupt service routine function to call when data bit is
      received in direct mode. \param func Pointer to interrupt service routine.
    */
    ...
    */
    ...
    /*!
      \brief Configure DIO pin mapping to get a given signal on a DIO pin (if
      available). \param pin Pin number onto which a signal is to be placed.
      \param value The value that indicates which function to place on that pin.
      See chip datasheet for details. \returns \ref status_codes
    */
    int16_t setDIOMapping(uint32_t pin, uint32_t value) override;

    /*!
      \brief Configure DIO mapping to use RSSI or Preamble Detect for pins that
      support it. \param usePreambleDetect Whether to use PreambleDetect (true) or
      RSSI (false) on the pins that are mapped to this function. \returns \ref
      status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param level Battery threshold level (one of
      RADIOLIB_SX127X_LOW_BAT_THRESHOLD_*), or -1 to disable the detector.
      Disabled by default. Note that this will not attach any interrupts! \param
      pin DIO pin number which will be used to signal low battery. Only DIO0/4 can
      be used (in packet mode) or DIO3/4 (in continuous mode). Ignored when
      disabling the detector. \returns \ref status_codes
    */
...
private:
    ...
    float dataRate = 0;
    bool packetLengthQueried = false; // FSK packet length is the first byte in
    // FIFO, length can only be queried once
    ...
    void clearFIFO(size_t count); // used mostly to clear remaining bytes in FIFO
    // after a packet read
