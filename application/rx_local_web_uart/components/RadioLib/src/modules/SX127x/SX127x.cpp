...
int16_t SX127x::beginFSK(const uint8_t *chipVersions, uint8_t numVersions, float freqDev, float rxBw,
...
int16_t SX127x::receive(uint8_t *data, size_t len, RadioLibTime_t timeout)
{
    ...
    if (!timeoutInternal) {
        ...
        size_t maxLen = len;
        if (len == 0) {
            maxLen = RADIOLIB_SX127X_MAX_PACKET_LENGTH;
        }
...
{
    // set DIO1 to the FIFO empty event (the register setting is done in
    // startTransmit)
...
{
    // subtract first (this may be the first time we get to modify the remaining
    // length)
...
{
    // wait for at least 1 bit at the lowest possible bit rate before clearing IRQ
    // flags not doing this and clearing RADIOLIB_SX127X_FLAG_FIFO_OVERRUN will
    // dump the FIFO, which can lead to mangling of the last bit (#808)
...
int16_t SX127x::readData(uint8_t *data, size_t len)
{
    ...
    if (modem == RADIOLIB_SX127X_LORA) {
        ...
        if (this->crcEnabled && (state == RADIOLIB_ERR_NONE) &&
            ...
            // CRC is disabled according to packet header and enabled according to
            // user most likely damaged packet header
...
int16_t SX127x::startChannelScan()
{
    ...
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                  RADIOLIB_SX127X_DIO0_LORA_CAD_DONE | RADIOLIB_SX127X_DIO1_LORA_CAD_DETECTED, 7, 4);
    RADIOLIB_ASSERT(state);
...
{
    if ((this->getIRQFlags() & RADIOLIB_SX127X_CLEAR_IRQ_FLAG_CAD_DETECTED) ==
...
int16_t SX127x::setBitRateCommon(float br, uint8_t fracRegAddr)
{
    ...
    // datasheet says 1.2 kbps should be the smallest possible, but 0.512 works
    // fine
    if (ookEnabled) {
        RADIOLIB_CHECK_RANGE(br, 0.5f, 32.768002f,
                             RADIOLIB_ERR_INVALID_BIT_RATE); // Found that 32.768 is 32.768002
    } else {
...
int16_t SX127x::setNodeAddress(uint8_t nodeAddr)
{
    ...
    int16_t state =
        this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, RADIOLIB_SX127X_ADDRESS_FILTERING_NODE, 2, 1);
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::disableAddressFiltering()
{
    ...
    // disable address filtering
    int16_t state =
        this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, RADIOLIB_SX127X_ADDRESS_FILTERING_OFF, 2, 1);
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::setFrequencyRaw(float newFreq)
{
    ...
    // write registers
    // lsb needs to be written no matter what in order for the module to update
    // the frequency
...
int16_t SX127x::getLoRaRxHeaderInfo(uint8_t *cr, bool *hasCRC)
{
    ...
    if (cr) {
        *cr = this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_MODEM_STAT, 7, 5) >> 5;
    }
    if (hasCRC) {
        *hasCRC = this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_HOP_CHANNEL, 6, 6) != 0;
    }
...
float SX127x::getNumSymbols(size_t len, DataRate_t dr, PacketConfig_t pc)
{
    ...
    float n_pay = 8.0f + RADIOLIB_MAX(ceilf((8.0f * (float)len - 4.0f * (float)dr.lora.spreadingFactor + 28.0f +
                                             16.0f * crc - 20.0f * ih) /
                                            (4.0f * (float)dr.lora.spreadingFactor - 8.0f * de)) *
                                      (float)dr.lora.codingRate,
                                      0.0f);
...
RadioLibTime_t SX127x::calculateTimeOnAir(ModemType_t modem, DataRate_t dr, PacketConfig_t pc, size_t len)
{
    if (modem == RADIOLIB_MODEM_LORA) {
        ...
        // calculate time-on-air in us {[(length in bytes) * (8 bits / 1 byte)] /
        // [(Bit Rate in kbps) * (1000 bps / 1 kbps)]} * (1000000 us in 1 sec)
        return ((uint32_t)((((float)(pc.fsk.crcLength * 8) + pc.fsk.syncWordLength + pc.fsk.preambleLength +
                             (float)(len * 8)) /
...
RadioLibTime_t SX127x::getTimeOnAir(size_t len)
{
    ...
    case (RADIOLIB_SX127X_LORA): {
        ...
        // Get number of preamble symbols
        uint16_t n_pre = ((this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_MSB) << 8) |
    ...
    case (RADIOLIB_SX127X_FSK_OOK): {
        ...
        uint16_t n_pre = (uint16_t)((this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_MSB_FSK) << 8) |
                                    this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_LSB_FSK)) *
                         8;
...
RadioLibTime_t SX127x::calculateRxTimeout(RadioLibTime_t timeoutUs)
{
    ...
    if (getActiveModem() == RADIOLIB_SX127X_LORA) {
        ...
        // symbols is truncated to integer the order of operators is swapped here to
        // decrease the effects of this truncation error
        float symbolLength = (float)(uint32_t(1) << this->spreadingFactor) / (float)this->bandwidth;
...
{
    // this is a bit convoluted, but unfortunately SX127x IRQ flags are not used
    // to enable/disable that IRQ ... in addition, the configuration is often
    // mutually exclusive, so we iterate over the set bits in a loop
    ...
    for (uint8_t i = 0; i <= 31; i++) {
        ...
        // if not, decode it
        uint8_t dioNum = 0; // DIO pin number and register value to set (address and
        // MSB/LSB can be inferred)
        uint8_t regVal = 0;
        if (modem == RADIOLIB_SX127X_LORA) {
            switch (irqBit) {
...
int16_t SX127x::setEncoding(uint8_t encoding)
{
    ...
    case RADIOLIB_ENCODING_MANCHESTER:
        return (
                   this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, RADIOLIB_SX127X_DC_FREE_MANCHESTER, 6, 5));
    case RADIOLIB_ENCODING_WHITENING:
        return (
                   this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, RADIOLIB_SX127X_DC_FREE_WHITENING, 6, 5));
    default:
...
int16_t SX127x::configFSK()
{
    ...
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
...
bool SX127x::findChip(const uint8_t *vers, uint8_t num)
{
    ...
    while ((i < 10) && !flagFound) {
        ...
        if (!flagFound) {
            RADIOLIB_DEBUG_BASIC_PRINTLN("SX127x not found! (%d of 10 tries) "
                                         "RADIOLIB_SX127X_REG_VERSION == 0x%04X",
                                         i + 1, version);
            this->mod->hal->delay(10);
            i++;
        }
...
int16_t SX127x::setMode(uint8_t mode)
{
    ...
    if ((getActiveModem() == RADIOLIB_SX127X_FSK_OOK) && (mode == RADIOLIB_SX127X_RX)) {
        // disable checking of RX bit in FSK RX mode, as it sometimes seem to fail
        // (#276)
        checkMask = 0xFE;
    }
...
int16_t SX127x::invertIQ(bool enable)
{
    ...
    // Tx path inversion is swapped, because it seems that setting it according to
    // the datsheet will actually lead to the wrong inversion. See
    // https://github.com/jgromes/RadioLib/issues/778
    int16_t state;
    if (enable) {
...
int16_t SX127x::stageMode(RadioModeType_t mode, RadioModeConfig_t *cfg)
{
    ...
    case (RADIOLIB_RADIO_MODE_RX): {
        ...
        if (modem == RADIOLIB_SX127X_LORA) {
            ...
            if (cfg->receive.timeout != 0) {
                // for non-zero timeout value, change mode to Rx single and set the
                // timeout
            ...
            if (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_HOP_PERIOD) > RADIOLIB_SX127X_HOP_PERIOD_OFF) {
                state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                                  RADIOLIB_SX127X_DIO1_LORA_FHSS_CHANGE_CHANNEL, 5, 4);
            }
            ...
            // set FIFO pointers
            state |=
                this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_RX_BASE_ADDR, RADIOLIB_SX127X_FIFO_RX_BASE_ADDR_MAX);
            state |=
                this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_ADDR_PTR, RADIOLIB_SX127X_FIFO_RX_BASE_ADDR_MAX);
            RADIOLIB_ASSERT(state);
            ...
            // FSK modem does not actually distinguish between Rx single and
            // continuous mode, Rx single is emulated using timeout
    ...
    case (RADIOLIB_RADIO_MODE_TX): {
        ...
        if (modem == RADIOLIB_SX127X_LORA) {
            ...
            // set DIO mapping
            if (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_HOP_PERIOD) > RADIOLIB_SX127X_HOP_PERIOD_OFF) {
                this->mod->SPIsetRegValue(
                    RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                    RADIOLIB_SX127X_DIO0_LORA_TX_DONE | RADIOLIB_SX127X_DIO1_LORA_FHSS_CHANGE_CHANNEL, 7, 4);
            } else {
            ...
            // set FIFO pointers
            state |=
                this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_TX_BASE_ADDR, RADIOLIB_SX127X_FIFO_TX_BASE_ADDR_MAX);
            state |=
                this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_ADDR_PTR, RADIOLIB_SX127X_FIFO_TX_BASE_ADDR_MAX);
            ...
            if (cfg->transmit.len > RADIOLIB_SX127X_MAX_PACKET_LENGTH_FSK) {
                this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1, RADIOLIB_SX127X_DIO1_PACK_FIFO_EMPTY, 5,
                                          4);
            } else {
                this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1, RADIOLIB_SX127X_DIO0_PACK_PACKET_SENT, 7,
                                          6);
            }
...
{
    return this->mod->SPIsetRegValue(
               RADIOLIB_SX127X_REG_DIO_MAPPING_2,
               (usePreambleDetect) ? RADIOLIB_SX127X_DIO_MAP_PREAMBLE_DETECT : RADIOLIB_SX127X_DIO_MAP_RSSI, 0, 0);
}
...
float SX127x::getRSSI(bool packet, bool skipReceive, int16_t offset)
{
    if (getActiveModem() == RADIOLIB_SX127X_LORA) {
        if (packet) {
            ...
            // check last packet SNR and if it's less than 0, add it to reported RSSI
...
int16_t SX127x::setLowBatteryThreshold(int8_t level, uint32_t pin)
{
    ...
    case (0):
        return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                          RADIOLIB_SX127X_DIO0_PACK_TEMP_CHANGE_LOW_BAT, 7, 6));
    case (3):
        return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                          RADIOLIB_SX127X_DIO3_CONT_TEMP_CHANGE_LOW_BAT, 1, 0));
    case (4):
        return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_2,
                                          RADIOLIB_SX127X_DIO4_PACK_TEMP_CHANGE_LOW_BAT, 7, 6));
    }
    return (RADIOLIB_ERR_INVALID_DIO_PIN);
}
