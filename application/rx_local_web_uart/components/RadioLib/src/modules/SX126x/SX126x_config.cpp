...
int16_t SX126x::setPreambleLength(size_t preambleLength)
{
    ...
    if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
        ...
        return (setPacketParams(this->preambleLengthLoRa, this->crcTypeLoRa, this->implicitLen, this->headerType,
        ...
        // for details, see the note in SX1261 datasheet, Rev 2.1, section 6.2.2.1,
        // page 45
        ...
        return (setPacketParamsFSK(this->preambleLengthFSK, this->preambleDetLength, this->crcTypeFSK,
...
int16_t SX126x::setFrequencyDeviation(float freqDev)
{
    ...
    uint32_t freqDevRaw = (uint32_t)(((newFreqDev * 1000.0f) * (float)((uint32_t)(1) << 25)) /
                                     (RADIOLIB_SX126X_CRYSTAL_FREQ * 1000000.0f));
...
int16_t SX126x::setDataRate(DataRate_t dr, ModemType_t modem)
{
    ...
    if (modem == RADIOLIB_MODEM_FSK) {
        ...
        // set hopping grid
        this->lrFhssGridNonFcc =
            dr.lrFhss.narrowGrid ? RADIOLIB_SX126X_LR_FHSS_GRID_STEP_NON_FCC : RADIOLIB_SX126X_LR_FHSS_GRID_STEP_FCC;
    }
...
int16_t SX126x::setRxBoostedGainMode(bool rxbgm, bool persist)
{
    ...
    if (persist) {
        // values and registers below are specified in SX126x datasheet v2.1
        // section 9.6, just below table 9-3
        const uint8_t data[] = {0x01, (uint8_t)((RADIOLIB_SX126X_REG_RX_GAIN >> 8) & 0xFF),
                                (uint8_t)(RADIOLIB_SX126X_REG_RX_GAIN & 0xFF)
                               };
...
int16_t SX126x::setWhitening(bool enabled, uint16_t initial)
{
    ...
    if (!enabled) {
        ...
        // as per note on pg. 65 of datasheet v1.2: "The user should not change the
        // value of the 7 MSB's of this register"
        uint8_t data[2];
        // first read the actual value and mask 7 MSB which we can not change
        // if different value is written in 7 MSB, the Rx won't even work (tested on
        // HW)
...
int16_t SX126x::setPacketMode(uint8_t mode, uint8_t len)
{
    ...
    int16_t state =
        setPacketParamsFSK(this->preambleLengthFSK, this->preambleDetLength, this->crcTypeFSK, this->syncWordLength,
                           RADIOLIB_SX126X_GFSK_ADDRESS_FILT_OFF, this->whitening, mode, len);
    RADIOLIB_ASSERT(state);
...
int16_t SX126x::config(uint8_t modem)
{
    ...
    data[0] = this->standbyXOSC ? RADIOLIB_SX126X_RX_TX_FALLBACK_MODE_STDBY_XOSC
