...
public:
    /*!
      ...
      \param mod Instance of STM32WLx_Module that will be used to communicate with
      the radio.
    */
    ...
    */
    int16_t begin(float freq = 434.0, float bw = 125.0, uint8_t sf = 9, uint8_t cr = 7,
                  ...
                  float tcxoVoltage = 1.6, bool useRegulatorLDO = false) override;

    /*!
      \copydoc SX1262::beginFSK
    */
    int16_t beginFSK(float freq = 434.0, float br = 4.8, float freqDev = 5.0, float rxBw = 156.2, int8_t power = 10,
                     uint16_t preambleLength = 16, float tcxoVoltage = 1.6, bool useRegulatorLDO = false) override;
    ...
    /*!
      ...
      This automatically switches between the low-power (LP) and high-power (HP)
      amplifier.
      ...
      setRfSwitchTable()), HP is used, which supports -9 to +22dBm. If the LP is
      marked as unavailable, HP output will be used instead.
