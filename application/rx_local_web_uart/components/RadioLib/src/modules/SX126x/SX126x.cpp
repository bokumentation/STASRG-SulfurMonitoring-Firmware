...
{
    // BW in kHz and SF are required in order to calculate LDRO for
    // setModulationParams set the defaults, this will get overwritten later
    // anyway
    this->bandwidthKhz = 500.0;
    this->spreadingFactor = 9;

    // initialize configuration variables (will be overwritten during public
    // settings configuration)
    this->bandwidth = RADIOLIB_SX126X_LORA_BW_500_0; // initialized to 500 kHz, since lower
    // values will interfere with LLCC68
...
int16_t SX126x::beginFSK(float br, float freqDev, float rxBw, uint16_t preambleLength, float tcxoVoltage,
...
int16_t SX126x::setLrFhssConfig(uint8_t bw, uint8_t cr, uint8_t hdrCount, uint16_t hopSeqId)
{
    ...
    RADIOLIB_CHECK_RANGE((int8_t)cr, (int8_t)RADIOLIB_SX126X_LR_FHSS_CR_5_6, (int8_t)RADIOLIB_SX126X_LR_FHSS_CR_1_3,
                         RADIOLIB_ERR_INVALID_CODING_RATE);
    this->lrFhssCr = cr;
    RADIOLIB_CHECK_RANGE((int8_t)bw, (int8_t)RADIOLIB_SX126X_LR_FHSS_BW_39_06,
                         (int8_t)RADIOLIB_SX126X_LR_FHSS_BW_1574_2, RADIOLIB_ERR_INVALID_BANDWIDTH);
    this->lrFhssBw = bw;
...
int16_t SX126x::receive(uint8_t *data, size_t len, RadioLibTime_t timeout)
{
    ...
    if (!timeoutInternal) {
        ...
        size_t maxLen = len;
        if (len == 0) {
            maxLen = RADIOLIB_SX126X_MAX_PACKET_LENGTH;
        }
...
int16_t SX126x::transmitDirect(uint32_t frf)
{
    ...
    // direct mode activation intentionally skipped here, as it seems to lead to
    // much worse results
...
int16_t SX126x::directMode()
{
    ...
    // this is done exclusively by writing magic values to even more magic
    // registers
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_TX_BITBANG_ENABLE_1, RADIOLIB_SX126X_TX_BITBANG_1_ENABLED, 6, 4);
    RADIOLIB_ASSERT(state);
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_TX_BITBANG_ENABLE_0, RADIOLIB_SX126X_TX_BITBANG_0_ENABLED, 3, 0);
    RADIOLIB_ASSERT(state);
    ...
    // set preamble length to the maximum to prevent SX126x from exiting Tx mode
    // for a while
...
int16_t SX126x::packetMode()
{
    ...
    RADIOLIB_ASSERT(state);
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_TX_BITBANG_ENABLE_0, RADIOLIB_SX126X_TX_BITBANG_0_DISABLED, 3, 0);
    RADIOLIB_ASSERT(state);
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_TX_BITBANG_ENABLE_1, RADIOLIB_SX126X_TX_BITBANG_1_DISABLED, 6, 4);
    RADIOLIB_ASSERT(state);
...
{
    return (this->startReceive(RADIOLIB_SX126X_RX_TIMEOUT_INF, RADIOLIB_IRQ_RX_DEFAULT_FLAGS,
                               RADIOLIB_IRQ_RX_DEFAULT_MASK, 0));
}

int16_t SX126x::startReceiveDutyCycle(uint32_t rxPeriod, uint32_t sleepPeriod, RadioLibIrqFlags_t irqFlags,
                                      RadioLibIrqFlags_t irqMask)
{
    // datasheet claims time to go to sleep is ~500us, same to wake up, compensate
    // for that with 1 ms + TCXO delay
    ...
    // this check of the high byte also catches underflow when we subtracted
    // transitionTime
    ...
    const uint8_t data[6] = {(uint8_t)((rxPeriodRaw >> 16) & 0xFF), (uint8_t)((rxPeriodRaw >> 8) & 0xFF),
                             (uint8_t)(rxPeriodRaw & 0xFF), (uint8_t)((sleepPeriodRaw >> 16) & 0xFF),
                             (uint8_t)((sleepPeriodRaw >> 8) & 0xFF), (uint8_t)(sleepPeriodRaw & 0xFF)
                            };
...
int16_t SX126x::startReceiveDutyCycleAuto(uint16_t senderPreambleLength, uint16_t minSymbols,
                                          RadioLibIrqFlags_t irqFlags, RadioLibIrqFlags_t irqMask)
{
    if (senderPreambleLength == 0) {
        ...
        // the unit must be configured to expect a preamble length at least as long
        // as the sender is using
    ...
    // worst case is that the sender starts transmitting when we're just less than
    // minSymbols from going back to sleep. in this case, we don't catch
    // minSymbols before going to sleep, so we must be awake for at least that
    // long before the sender stops transmitting.
    ...
    // when the unit detects a preamble, it starts a timer that will timeout if it
    // doesn't receive a header in time. the duration is sleepPeriod + 2 *
    // wakePeriod. The sleepPeriod doesn't take into account shutdown and startup
    // time for the unit (~1ms) We need to ensure that the timeout is longer than
    // senderPreambleLength. So we must satisfy: wakePeriod > (preamblePeriod -
    // (sleepPeriod - 1000)) / 2. (A) we also need to ensure the unit is awake to
    // see at least minSymbols. (B)
    uint32_t wakePeriod = RADIOLIB_MAX((symbolLength * (senderPreambleLength + 1) - (sleepPeriod - 1000)) / 2, // (A)
                                       symbolLength * (minSymbols + 1));                                       //(B)
    RADIOLIB_DEBUG_BASIC_PRINTLN("Auto wake period: %lu", (long unsigned int)wakePeriod);

    // If our sleep period is shorter than our transition time, just use the
    // standard startReceive
...
int16_t SX126x::startReceiveCommon(uint32_t timeout, RadioLibIrqFlags_t irqFlags, RadioLibIrqFlags_t irqMask)
{
    ...
    if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
        state = setPacketParams(this->preambleLengthLoRa, this->crcTypeLoRa, this->implicitLen, this->headerType,
                                this->invertIQEnabled);
    } else if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
        state =
            setPacketParamsFSK(this->preambleLengthFSK, this->preambleDetLength, this->crcTypeFSK, this->syncWordLength,
...
int16_t SX126x::readData(uint8_t *data, size_t len)
{
    ...
    // Report CRC mismatch when there's a payload CRC error, or a header error and
    // no valid header (to avoid false alarm from previous packet)
    ...
    // check if CRC failed - this is done after reading data to give user the
    // option to keep them
...
size_t SX126x::getPacketLength(bool update, uint8_t *offset)
{
    ...
    if ((getPacketType() == RADIOLIB_SX126X_PACKET_TYPE_LORA) &&
            (this->headerType == RADIOLIB_SX126X_LORA_HEADER_IMPLICIT) && (!offset)) {
        return (this->implicitLen);
    }

    // if offset was requested, or in explicit mode, we always have to perform the
    // SPI transaction
...
int16_t SX126x::getLoRaRxHeaderInfo(uint8_t *cr, bool *hasCRC)
{
    ...
    if (cr) {
        *cr = this->mod->SPIgetRegValue(RADIOLIB_SX126X_REG_LORA_RX_CODING_RATE, 6, 4) >> 4;
    }
    if (hasCRC) {
        *hasCRC = (this->mod->SPIgetRegValue(RADIOLIB_SX126X_REG_FREQ_ERROR_RX_CRC, 4, 4) != 0);
    }
...
RadioLibTime_t SX126x::calculateTimeOnAir(ModemType_t modem, DataRate_t dr, PacketConfig_t pc, size_t len)
{
    ...
    case RADIOLIB_MODEM_LORA: {
        ...
        int16_t bitCount = (int16_t)8 * len + pc.lora.crcEnabled * bitsPerCrc - 4 * dr.lora.spreadingFactor + sfCoeff2 +
                           N_symbol_header;
        if (bitCount < 0) {
            bitCount = 0;
        }
        ...
        uint32_t nSymbol_x4 =
            (pc.lora.preambleLength + 8) * 4 + sfCoeff1_x4 + nPreCodedSymbols * dr.lora.codingRate * 4;

        return ((symbolLength_us * nSymbol_x4) / 4);
    }
    case RADIOLIB_MODEM_FSK: {
        return ((((float)(pc.fsk.crcLength * 8) + pc.fsk.syncWordLength + pc.fsk.preambleLength + (uint32_t)len * 8) /
...
RadioLibTime_t SX126x::getTimeOnAir(size_t len)
{
    ...
    if (type == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
        ...
        // We assume same calculation for short and long interleaving, so map CR
        // values 0-4 and 5-7 to the same values
        if (cr < 5) {
            cr = cr + 4;
        } else if (cr == 7) {
            cr = cr + 1;
        ...
        uint8_t crcLen = 0;
        if (this->crcTypeFSK == RADIOLIB_SX126X_GFSK_CRC_1_BYTE ||
                this->crcTypeFSK == RADIOLIB_SX126X_GFSK_CRC_1_BYTE_INV) {
            crcLen = 1;
        } else if (this->crcTypeFSK == RADIOLIB_SX126X_GFSK_CRC_2_BYTE ||
...
RadioLibTime_t SX126x::calculateRxTimeout(RadioLibTime_t timeoutUs)
{
    ...
    // the calling function should provide some extra width, as this number of
    // units is truncated to integer
...
int16_t SX126x::stageMode(RadioModeType_t mode, RadioModeConfig_t *cfg)
{
    ...
    case (RADIOLIB_RADIO_MODE_RX): {
        ...
        // otherwise we trust the user has previously set the payload length
        // manually
    ...
    case (RADIOLIB_RADIO_MODE_TX): {
        ...
        uint8_t modem = getPacketType();
        if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
            state = setPacketParams(this->preambleLengthLoRa, this->crcTypeLoRa, cfg->transmit.len, this->headerType,
                                    this->invertIQEnabled);

        } else if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
            state = setPacketParamsFSK(this->preambleLengthFSK, this->preambleDetLength, this->crcTypeFSK,
                                       this->syncWordLength, RADIOLIB_SX126X_GFSK_ADDRESS_FILT_OFF, this->whitening,
                                       this->packetType, cfg->transmit.len);
        ...
        if (modem != RADIOLIB_SX126X_PACKET_TYPE_LR_FHSS) {
            state =
                setDioIrqParams(RADIOLIB_SX126X_IRQ_TX_DONE | RADIOLIB_SX126X_IRQ_TIMEOUT, RADIOLIB_SX126X_IRQ_TX_DONE);
        } else {
            state = setDioIrqParams(RADIOLIB_SX126X_IRQ_TX_DONE | RADIOLIB_SX126X_IRQ_LR_FHSS_HOP,
        ...
        if (modem != RADIOLIB_SX126X_PACKET_TYPE_LR_FHSS) {
            ...
            this->lrFhssHopNum = 0;
            state = buildLRFHSSPacket(cfg->transmit.data, cfg->transmit.len, frame, &frameLen,
                                      &this->lrFhssFrameBitsRem, &this->lrFhssFrameHopsRem);
            RADIOLIB_ASSERT(state);
            ...
            // activate hopping
            const uint8_t hopCfg[] = {RADIOLIB_SX126X_HOPPING_ENABLED, (uint8_t)frameLen,
                                      (uint8_t)this->lrFhssFrameHopsRem
                                     };
...
int16_t SX126x::uploadPatch(const uint32_t *patch, size_t len, bool nonvolatile)
{
    ...
    for (uint32_t i = 0; i < len / sizeof(uint32_t); i++) {
        ...
        data[2] = (bin >> 8) & 0xFF;
        data[3] = bin & 0xFF;
        this->mod->SPIwriteRegisterBurst(RADIOLIB_SX126X_REG_PATCH_MEMORY_BASE + i * sizeof(uint32_t), data,
                                         sizeof(uint32_t));
    }
...
{
    // abort first - not sure if this is strictly needed, but the example code
    // does this
    spectralScanAbort();
...
int16_t SX126x::spectralScanGetResult(uint16_t *results)
{
    ...
    this->mod->SPIreadRegisterBurst(RADIOLIB_SX126X_REG_SPECTRAL_SCAN_RESULT,
                                    2 * RADIOLIB_SX126X_SPECTRAL_SCAN_RES_SIZE, data);
...
int16_t SX126x::fixSensitivity()
{
    ...
    // see SX1262/SX1268 datasheet, chapter 15 Known Limitations, section 15.1 for
    // details
...
int16_t SX126x::fixImplicitTimeout()
{
    ...
    // see SX1262/SX1268 datasheet, chapter 15 Known Limitations, section 15.3 for
    // details
...
int16_t SX126x::fixGFSK()
{
    ...
    // it in all other cases) this is not documented in the datasheet, only in
    // Semtech repositories for SX126x and LR11xx
...
int16_t SX126x::modSetup(float tcxoVoltage, bool useRegulatorLDO, uint8_t modem)
{
    ...
    // find the SX126x chip - this will also reset the module and verify the
    // module
    ...
    if (state != RADIOLIB_ERR_NONE) {
        ...
        // SPI command fail and oscillator start error flag indicate incorrectly set
        // oscillator
        if ((state == RADIOLIB_ERR_SPI_CMD_FAILED) && (errors & RADIOLIB_SX126X_XOSC_START_ERR)) {
            // typically users with XTAL devices will try to call the default begin
            // method disable TCXO and try to run config again
            this->XTAL = false;
...
#if RADIOLIB_DEBUG_BASIC
            RADIOLIB_DEBUG_BASIC_PRINTLN("SX126x not found! (%d of 10 tries) "
