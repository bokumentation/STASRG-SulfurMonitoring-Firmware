...
{
    const uint8_t data[] = {(uint8_t)((timeout >> 16) & 0xFF), (uint8_t)((timeout >> 8) & 0xFF),
...
{
    // default CAD parameters are selected according to recommendations on Semtech
    // DS.SX1261-2.W.APP rev. 1.1, page 92.
...
{
    const uint8_t data[] = {(uint8_t)((frf >> 24) & 0xFF), (uint8_t)((frf >> 16) & 0xFF), (uint8_t)((frf >> 8) & 0xFF),
                            (uint8_t)(frf & 0xFF)
                           };
...
int16_t SX126x::setModulationParamsFSK(uint32_t br, uint8_t sh, uint8_t rxBw, uint32_t freqDev)
{
    const uint8_t data[8] = {
        (uint8_t)((br >> 16) & 0xFF), (uint8_t)((br >> 8) & 0xFF), (uint8_t)(br & 0xFF),     sh, rxBw,
...
int16_t SX126x::setPacketParams(uint16_t preambleLen, uint8_t crcType, uint8_t payloadLen, uint8_t hdrType,
                                uint8_t invertIQ)
{
...
int16_t SX126x::setPacketParamsFSK(uint16_t preambleLen, uint8_t preambleDetectorLen, uint8_t crcType,
                                   uint8_t syncWordLen, uint8_t addrCmp, uint8_t whiten, uint8_t packType,
                                   uint8_t payloadLen)
{
    const uint8_t data[9] = {(uint8_t)((preambleLen >> 8) & 0xFF),
...
int16_t SX126x::setPacketParamsBPSK(uint8_t payloadLen, uint16_t rampUpDelay, uint16_t rampDownDelay,
                                    uint16_t payloadLenBits)
{
    const uint8_t data[] = {payloadLen,
                            ...
                            (uint8_t)((payloadLenBits >> 8) & 0xFF),
                            (uint8_t)(payloadLenBits & 0xFF)
                           };

    // this one is a bit different, it seems to be split into command transaction
    // and then a register write
