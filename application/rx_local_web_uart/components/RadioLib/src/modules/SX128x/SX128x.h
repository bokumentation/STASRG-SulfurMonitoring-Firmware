...
#define RADIOLIB_SX128X_SLEEP_DATA_RAM_FLUSH 0b00000000 //  0     0   data RAM (configuration) behavior in sleep mode:
//  flush
...
// RADIOLIB_SX128X_CMD_SET_TX + RADIOLIB_SX128X_CMD_SET_RX +
// RADIOLIB_SX128X_CMD_SET_RX_DUTY_CYCLE
...
#define RADIOLIB_SX128X_PACKET_STATUS_RX_PID                                                                           \
    0b11000000 //  7     6   packet status status byte: PID field of the received
//  packet
#define RADIOLIB_SX128X_PACKET_STATUS_NO_ACK 0b00100000 //  5     5                              NO_ACK field of the
//  received packet
...
/*!
  \class SX128x
  \brief Base class for %SX128x series. All derived classes for %SX128x (e.g.
  SX1280 or SX1281) inherit from this base class. This class should not be
...
public:
    ...
    */
    ...
    /*!
      ...
      Allowed values range from 4 to 8. Note that a value of 4 means no coding, is
      undocumented and not recommended without your own FEC. \param syncWord
      2-byte LoRa sync word. Defaults to RADIOLIB_SX128X_SYNC_WORD_PRIVATE (0x12).
      \param pwr Output power in dBm. Defaults to 10 dBm.
      \param preambleLength LoRa preamble length in symbols. Defaults to 12
      symbols. \returns \ref status_codes
    */
    int16_t begin(float freq = 2400.0, float bw = 812.5, uint8_t sf = 9, uint8_t cr = 7,
                  uint8_t syncWord = RADIOLIB_SX128X_SYNC_WORD_PRIVATE, int8_t pwr = 10, uint16_t preambleLength = 12);
    ...
    /*!
      ...
      to 400.0 kHz. \param pwr Output power in dBm. Defaults to 10 dBm. \param
      preambleLength FSK preamble length in bits. Defaults to 16 bits. \returns
      \ref status_codes
    */
    int16_t beginGFSK(float freq = 2400.0, uint16_t br = 800, float freqDev = 400.0, int8_t pwr = 10,
    ...
    /*!
      ...
      \param freqDev Frequency deviation from carrier frequency in kHz. Defaults
      to 400.0 kHz. \param pwr Output power in dBm. Defaults to 10 dBm. \param
      dataShaping Time-bandwidth product of the Gaussian filter to be used for
      shaping. Defaults to 0.5. \returns \ref status_codes
    */
    int16_t beginBLE(float freq = 2400.0, uint16_t br = 800, float freqDev = 400.0, int8_t pwr = 10,
    ...
    /*!
      ...
      \param dataShaping Time-bandwidth product of the Gaussian filter to be used
      for shaping. Defaults to 0.5. \returns \ref status_codes
    */
    int16_t beginFLRC(float freq = 2400.0, uint16_t br = 650, uint8_t cr = 3, int8_t pwr = 10,
                      uint16_t preambleLength = 16, uint8_t dataShaping = RADIOLIB_SHAPING_0_5);
    ...
    */
    ...
    /*!
      ...
      for binary transmissions. \param timeout Reception timeout in microseconds.
    ...
    */
    ...
    /*!
      ...
      compatibility, as %SX128x series does not support direct mode reception.
      Will always return RADIOLIB_ERR_UNKNOWN. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Sets the module to sleep mode. To wake the device up, call standby().
      \param retainConfig Set to true to retain configuration and data buffer or
      to false to discard current configuration and data buffer. Defaults to true.
      \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      RADIOLIB_SX128X_STANDBY_RC (13 MHz RC oscillator) or
      RADIOLIB_SX128X_STANDBY_XOSC (52 MHz external crystal oscillator). \param
    ...
    */
    ...
    /*!
      ...
      activated when LoRa preamble is detected, or upon timeout. \returns \ref
      status_codes
    */
    ...
    */
    ...
    /*!
      \brief Sets carrier frequency. Allowed values are in range from 2400.0 to
      2500.0 MHz. \param freq Carrier frequency to be set in MHz. \returns \ref
      status_codes
    */
    int16_t setFrequency(float freq) override;

    /*!
      \brief Sets LoRa bandwidth. Allowed values are 203.125, 406.25, 812.5 and
      1625.0 kHz. \param bw LoRa bandwidth to be set in kHz. \returns \ref
      status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param longInterleaving Whether to enable long interleaving mode. Not
      available for coding rate 4/7, defaults to false. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      module's range. \returns \ref status_codes
    */
    ...
    /*!
      ...
      to set - FSK, LoRa or LR-FHSS. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Sets preamble length for currently active modem. Allowed values range
      from 1 to 65535. \param preambleLength Preamble length to be set in symbols
      (LoRa) or bits (FSK/BLE/FLRC). \returns \ref status_codes
    */
    int16_t setPreambleLength(size_t preambleLength) override;

    /*!
      \brief Set data rate.
      \param dr Data rate struct. Interpretation depends on currently active modem
      (FSK or LoRa). \returns \ref status_codes
    */
    int16_t setDataRate(DataRate_t dr, ModemType_t modem = RADIOLIB_MODEM_NONE) override;

    /*!
      \brief Sets FSK or FLRC bit rate. Allowed values are 125, 250, 400, 500,
      800, 1000, 1600 and 2000 kbps (for FSK modem) or 260, 325, 520, 650, 1000
      and 1300 (for FLRC modem). \param br FSK/FLRC bit rate to be set in kbps.
      \returns \ref status_codes
    */
    int16_t setBitRate(float br) override;

    /*!
      \brief Sets FSK frequency deviation. Allowed values range from 0.0 to 3200.0
    ...
    */
    ...
    /*!
      ...
      Allowed values are RADIOLIB_SHAPING_0_5 or RADIOLIB_SHAPING_1_0. Set to
      RADIOLIB_SHAPING_NONE to disable data shaping. \param sh Time-bandwidth
      product of Gaussian filter to be set. \returns \ref status_codes
    */
    int16_t setDataShaping(uint8_t sh) override;

    /*!
      \brief Sets FSK/FLRC sync word in the form of array of up to 5 bytes (FSK).
      For FLRC modem, the sync word must be exactly 4 bytes long \param syncWord
      Sync word to be set. \param len Sync word length in bytes. \returns \ref
      status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param len CRC length in bytes, Allowed values are 1, 2 or 3, set to 0 to
      disable CRC. \param initial Initial CRC value. Defaults to 0x1D0F (CCIT
      CRC), not available for LoRa modem. \param polynomial Polynomial for CRC
      calculation. Defaults to 0x1021 (CCIT CRC), not available for LoRa or BLE
      modem. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param gain Use 0 for automatic gain, 1 for minimum gain and up to 13 for
      maximum gain. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      available for LoRa or ranging modem. \returns SNR of the last received
      packet in dB.
    */
    ...
    */
    ...
    /*!
      ...
      offset. \param update Update received packet length. Will return cached
      value when set to false. \param offset Pointer to variable to store the Rx
      buffer offset. \returns Length of last received packet in bytes.
    */
    ...
    */
    ...
    /*!
      \brief Set modem in fixed packet length mode. Available in GFSK and FLRC
      modes only. \param len Packet length. \returns \ref status_codes
    */
    int16_t fixedPacketLengthMode(uint8_t len = RADIOLIB_SX128X_MAX_PACKET_LENGTH);

    /*!
      \brief Set modem in variable packet length mode. Available in GFSK and FLRC
      modes only. \param maxLen Maximum packet length. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Sets transmission encoding. Serves only as alias for PhysicalLayer
      compatibility. \param encoding Encoding to be used. Set to 0 for NRZ, and 2
      for whitening. \returns \ref status_codes
    */
...
protected:
    ...
    int16_t setPacketParamsGFSK(uint8_t preambleLen, uint8_t syncLen, uint8_t syncMatch, uint8_t crcLen, uint8_t whiten,
                                uint8_t hdrType, uint8_t payLen = 0xFF);
    int16_t setPacketParamsBLE(uint8_t connState, uint8_t crcLen, uint8_t bleTest, uint8_t whiten);
    int16_t setPacketParamsLoRa(uint8_t preambleLen, uint8_t hdrType, uint8_t payLen, uint8_t crc,
                                uint8_t invIQ = RADIOLIB_SX128X_LORA_IQ_STANDARD);
    int16_t setDioIrqParams(uint16_t irqMask, uint16_t dio1Mask, uint16_t dio2Mask = RADIOLIB_SX128X_IRQ_NONE,
    ...
    // protected so that it can be accessed from SX1280 class during reinit after
    // ranging is complete
