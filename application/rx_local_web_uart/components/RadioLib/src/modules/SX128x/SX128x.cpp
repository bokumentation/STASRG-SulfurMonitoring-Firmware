...
int16_t SX128x::begin(float freq, float bw, uint8_t sf, uint8_t cr, uint8_t syncWord, int8_t pwr,
...
int16_t SX128x::beginFLRC(float freq, uint16_t br, uint8_t cr, int8_t pwr, uint16_t preambleLength,
                          uint8_t dataShaping)
{
...
{
    // run the reset sequence - same as SX126x, as SX128x docs don't seem to
    // mention this
...
int16_t SX128x::receive(uint8_t *data, size_t len, RadioLibTime_t timeout)
{
    ...
    // for most other modules, it is 500%, however, the overall datarates of
    // SX128x are higher so we use higher value for the default timeout
...
int16_t SX128x::setSpreadingFactor(uint8_t sf)
{
    ...
    // update mystery register in LoRa mode - SX1280 datasheet rev 3.2
    // section 14.4.1
    if (modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) {
        uint8_t data = 0;
...
int16_t SX128x::setPreambleLength(size_t preambleLength)
{
    ...
    if ((modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) || (modem == RADIOLIB_SX128X_PACKET_TYPE_RANGING)) {
        // LoRa or ranging
        // the actual limit is 491520, however, some platforms (notably AVR) limit
        // size_t to 16 bits
        ...
        // calculate exponent and mantissa values (use the next longer preamble if
...
int16_t SX128x::setFrequencyDeviation(float freqDev)
{
    ...
    // override for the lowest possible frequency deviation - required for some
...
int16_t SX128x::stageMode(RadioModeType_t mode, RadioModeConfig_t *cfg)
{
    ...
    case (RADIOLIB_RADIO_MODE_TX): {
        ...
        uint8_t modem = getPacketType();
        if (modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) {
            state = setPacketParamsLoRa(this->preambleLengthLoRa, this->headerType, cfg->transmit.len, this->crcLoRa,
                                        this->invertIQEnabled);
        } else if ((modem == RADIOLIB_SX128X_PACKET_TYPE_GFSK) || (modem == RADIOLIB_SX128X_PACKET_TYPE_FLRC)) {
            state = setPacketParamsGFSK(this->preambleLengthGFSK, this->syncWordLen, this->syncWordMatch, this->crcGFSK,
                                        this->whitening, this->packetType, cfg->transmit.len);
        } else if (modem == RADIOLIB_SX128X_PACKET_TYPE_BLE) {
            state = setPacketParamsBLE(this->connectionState, this->crcBLE, this->bleTestPayload, this->whitening);
        } else {
            return (RADIOLIB_ERR_WRONG_MODEM);
...
int16_t SX128x::setPacketParamsGFSK(uint8_t preambleLen, uint8_t syncLen, uint8_t syncMatch, uint8_t crcLen,
                                    uint8_t whiten, uint8_t hdrType, uint8_t payLen)
{
...
bool SX128x::findChip(const char *verStr)
{
    ...
    while ((i < 10) && !flagFound) {
        ...
        if (strncmp(verStr, version, 6) == 0) {
            ...
            RADIOLIB_DEBUG_BASIC_PRINTLN("SX128x not found! (%d of 10 tries) "
