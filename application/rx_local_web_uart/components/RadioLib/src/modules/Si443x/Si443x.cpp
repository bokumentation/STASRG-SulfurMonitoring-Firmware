...
int16_t Si443x::receive(uint8_t *data, size_t len, RadioLibTime_t timeout)
{
    ...
    if (!timeoutInternal) {
        // calculate timeout (500 ms + 400 full max-length packets at current bit
        // rate)
...
int16_t Si443x::transmitDirect(uint32_t frf)
{
    ...
    if (frf != 0) {
        ...
        // update registers
        this->mod->SPIwriteRegister(RADIOLIB_SI443X_REG_FREQUENCY_BAND_SELECT,
                                    RADIOLIB_SI443X_SIDE_BAND_SELECT_LOW | bandSelect | freqBand);
        this->mod->SPIwriteRegister(RADIOLIB_SI443X_REG_NOM_CARRIER_FREQUENCY_1,
                                    (uint8_t)((freqCarrier & 0xFF00) >> 8));
        this->mod->SPIwriteRegister(RADIOLIB_SI443X_REG_NOM_CARRIER_FREQUENCY_0, (uint8_t)(freqCarrier & 0xFF));

        // start direct transmission
        directMode();
        this->mod->SPIwriteRegister(RADIOLIB_SI443X_REG_OP_FUNC_CONTROL_1,
                                    RADIOLIB_SI443X_TX_ON | RADIOLIB_SI443X_XTAL_ON);
...
int16_t Si443x::packetMode()
{
    ...
    return (this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_MODULATION_MODE_CONTROL_2,
                                      RADIOLIB_SI443X_TX_DATA_SOURCE_FIFO, 5, 4));
}
...
int16_t Si443x::startReceive()
{
    ...
    // set interrupt mapping
    uint8_t irq = this->crcEnabled ? (RADIOLIB_SI443X_VALID_PACKET_RECEIVED_ENABLED | RADIOLIB_SI443X_CRC_ERROR_ENABLED)
                  : RADIOLIB_SI443X_VALID_PACKET_RECEIVED_ENABLED;
...
int16_t Si443x::readData(uint8_t *data, size_t len)
{
    ...
    // reading the FIFO will just repeat the first byte (see
    // https://github.com/jgromes/RadioLib/issues/1430)
...
int16_t Si443x::setFrequencyDeviation(float freqDev)
{
    ...
    // update registers
    int16_t state =
        this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_MODULATION_MODE_CONTROL_2, (uint8_t)((fdev & 0x0100) >> 6), 2, 2);
...
int16_t Si443x::setRxBandwidth(float rxBw)
{
    ...
    // decide which approximation to use for decimation rate and filter tap
    // calculation
    ...
    if ((rxBw >= 2.6f) && (rxBw <= 4.5f)) {
        ...
        // this is the "Lord help thee who tread 'ere" section - no way to
        // approximate this mess
...
int16_t Si443x::setPreambleLength(size_t preambleLen)
{
    ...
    // set default preamble detection threshold to 5/8 of preamble length (in
    // units of 4 bits)
...
int16_t Si443x::setEncoding(uint8_t encoding)
{
    ...
    case RADIOLIB_ENCODING_NRZ:
        return (this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_MODULATION_MODE_CONTROL_1,
                                          RADIOLIB_SI443X_MANCHESTER_OFF | RADIOLIB_SI443X_WHITENING_OFF, 2, 0));
    case RADIOLIB_ENCODING_MANCHESTER:
        return (this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_MODULATION_MODE_CONTROL_1,
                                          RADIOLIB_SI443X_MANCHESTER_ON | RADIOLIB_SI443X_WHITENING_OFF, 2, 0));
    case RADIOLIB_ENCODING_WHITENING:
        return (this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_MODULATION_MODE_CONTROL_1,
                                          RADIOLIB_SI443X_MANCHESTER_OFF | RADIOLIB_SI443X_WHITENING_ON, 2, 0));
    default:
...
int16_t Si443x::setDataShaping(uint8_t sh)
{
    ...
    case RADIOLIB_SHAPING_NONE:
        return (this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_MODULATION_MODE_CONTROL_1,
                                          RADIOLIB_SI443X_MANCHESTER_INVERTED_OFF | RADIOLIB_SI443X_MANCHESTER_OFF |
                                          RADIOLIB_SI443X_WHITENING_OFF,
                                          2, 0));
...
int16_t Si443x::setFrequencyRaw(float newFreq)
{
    ...
    state |=
        this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_NOM_CARRIER_FREQUENCY_1, (uint8_t)((freqCarrier & 0xFF00) >> 8));
...
bool Si443x::findChip()
{
    ...
    while ((i < 10) && !flagFound) {
        ...
        if (version == RADIOLIB_SI443X_DEVICE_VERSION) {
            ...
            RADIOLIB_DEBUG_BASIC_PRINTLN("Si443x not found! (%d of 10 tries) "
                                         "RADIOLIB_SI443X_REG_DEVICE_VERSION == 0x%02X, expected 0x0%X",
                                         i + 1, version, RADIOLIB_SI443X_DEVICE_VERSION);
            this->mod->hal->delay(10);
            i++;
        }
    }
...
int16_t Si443x::config()
{
    ...
    // enable AGC
    state = this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_AGC_OVERRIDE_1,
                                      RADIOLIB_SI443X_AGC_GAIN_INCREASE_ON | RADIOLIB_SI443X_AGC_ON, 6, 5);
    RADIOLIB_ASSERT(state);

    // disable packet header
    state = this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_HEADER_CONTROL_2,
                                      RADIOLIB_SI443X_SYNC_WORD_TIMEOUT_OFF | RADIOLIB_SI443X_HEADER_LENGTH_HEADER_NONE,
                                      7, 4);
    RADIOLIB_ASSERT(state);
    ...
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_HEADER_CONTROL_1, RADIOLIB_SI443X_BROADCAST_ADDR_CHECK_NONE |
...
int16_t Si443x::updateClockRecovery()
{
    ...
    RADIOLIB_DEBUG_BASIC_PRINTLN("\t%d\t%X" RADIOLIB_LINE_FEED "%lu\t%lX" RADIOLIB_LINE_FEED "%d\t%X", rxOsr_fixed,
                                 rxOsr_fixed, (long unsigned int)ncoOff, (long unsigned int)ncoOff, crGain, crGain);

    // update oversampling ratio
    int16_t state =
        this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_CLOCK_REC_OFFSET_2, (uint8_t)((rxOsr_fixed & 0x0700) >> 3), 7, 5);
    RADIOLIB_ASSERT(state);
    ...
    // update NCO offset
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_CLOCK_REC_OFFSET_2, (uint8_t)((ncoOff & 0x0F0000) >> 16), 3, 0);
    RADIOLIB_ASSERT(state);
    ...
    state = this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_CLOCK_REC_TIMING_LOOP_GAIN_1,
                                      (uint8_t)((crGain & 0x0700) >> 8), 2, 0);
    RADIOLIB_ASSERT(state);
...
{
    int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SI443X_REG_MODULATION_MODE_CONTROL_2,
                                              RADIOLIB_SI443X_TX_DATA_SOURCE_GPIO, 5, 4);
    RADIOLIB_ASSERT(state);
