...
int16_t LR11x0::cryptoProcessJoinAccept(uint8_t decKeyId, uint8_t verKeyId, uint8_t lwVer, const uint8_t *header,
                                        const uint8_t *dataIn, size_t len, uint8_t *dataOut)
{
    ...
    int16_t state =
        this->SPIcommand(RADIOLIB_LR11X0_CMD_CRYPTO_PROCESS_JOIN_ACCEPT, false, rplBuff, rplLen, reqBuff, reqLen);
#if !RADIOLIB_STATIC_ONLY
    delete[] reqBuff;
#endif
...
int16_t LR11x0::cryptoComputeAesCmac(uint8_t keyId, const uint8_t *data, size_t len, uint32_t *mic)
{
    ...
    int16_t state =
        this->SPIcommand(RADIOLIB_LR11X0_CMD_CRYPTO_COMPUTE_AES_CMAC, false, rplBuff, sizeof(rplBuff), reqBuff, reqLen);
#if !RADIOLIB_STATIC_ONLY
    delete[] reqBuff;
#endif
    ...
    if (mic) {
        *mic = ((uint32_t)(rplBuff[1]) << 24) | ((uint32_t)(rplBuff[2]) << 16) | ((uint32_t)(rplBuff[3]) << 8) |
               (uint32_t)rplBuff[4];
    }
    return (state);
}
...
int16_t LR11x0::cryptoVerifyAesCmac(uint8_t keyId, uint32_t micExp, const uint8_t *data, size_t len, bool *result)
{
    ...
    int16_t state =
        this->SPIcommand(RADIOLIB_LR11X0_CMD_CRYPTO_VERIFY_AES_CMAC, false, rplBuff, sizeof(rplBuff), reqBuff, reqLen);
#if !RADIOLIB_STATIC_ONLY
    delete[] reqBuff;
#endif
...
{
    uint8_t buff[5] = {id, (uint8_t)((value >> 24) & 0xFF), (uint8_t)((value >> 16) & 0xFF),
...
int16_t LR11x0::cryptoGetParam(uint8_t id, uint32_t *value)
{
    ...
    uint8_t rplBuff[4] = {0};
    int16_t state = this->SPIcommand(RADIOLIB_LR11X0_CMD_CRYPTO_GET_PARAM, false, rplBuff, sizeof(rplBuff), reqBuff,
                                     sizeof(reqBuff));
    RADIOLIB_ASSERT(state);
    if (value) {
        *value = ((uint32_t)(rplBuff[0]) << 24) | ((uint32_t)(rplBuff[1]) << 16) | ((uint32_t)(rplBuff[2]) << 8) |
                 (uint32_t)rplBuff[3];
    }
    return (state);
}
...
int16_t LR11x0::cryptoCheckEncryptedFirmwareImage(uint32_t offset, const uint32_t *data, size_t len, bool nonvolatile)
{
    ...
    return (
               this->writeCommon(RADIOLIB_LR11X0_CMD_CRYPTO_CHECK_ENCRYPTED_FIRMWARE_IMAGE, offset, data, len, nonvolatile));
}
...
int16_t LR11x0::cryptoCheckEncryptedFirmwareImageResult(bool *result)
{
    uint8_t buff[1] = {0};
    int16_t state =
        this->SPIcommand(RADIOLIB_LR11X0_CMD_CRYPTO_CHECK_ENCRYPTED_FIRMWARE_IMAGE_RESULT, false, buff, sizeof(buff));
