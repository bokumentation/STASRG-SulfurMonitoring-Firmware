...
int16_t LR1120::begin(float freq, float bw, uint8_t sf, uint8_t cr, uint8_t syncWord, int8_t power,
...
int16_t LR1120::beginGFSK(float freq, float br, float freqDev, float rxBw, int8_t power, uint16_t preambleLength,
                          float tcxoVoltage)
{
    // execute common part
...
int16_t LR1120::setFrequency(float freq, bool skipCalibration, float band)
{
    ...
    if (!(((freq >= 150.0f) && (freq <= 960.0f)) || ((freq >= 1900.0f) && (freq <= 2200.0f)) ||
...
int16_t LR1120::setOutputPower(int8_t power, bool forceHighPower, uint32_t rampTimeUs)
{
    ...
    state = LR11x0::setOutputPower(power, paSel, paSupply, 0x04, 0x07, rampTimeUs);
    return (state);
}
