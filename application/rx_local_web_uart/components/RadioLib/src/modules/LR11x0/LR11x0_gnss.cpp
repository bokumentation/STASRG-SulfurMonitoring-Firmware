...
int16_t LR11x0::getGnssAlmanacStatus(LR11x0GnssAlmanacStatus_t *stat)
{
    ...
    stat->gps.timeUntilSubframe =
        ((uint32_t)(raw[1]) << 24) | ((uint32_t)(raw[2]) << 16) | ((uint32_t)(raw[3]) << 8) | (uint32_t)raw[4];
    ...
    stat->gps.flagsUpdateNeeded[0] =
        ((uint32_t)(raw[10]) << 24) | ((uint32_t)(raw[11]) << 16) | ((uint32_t)(raw[12]) << 8) | (uint32_t)raw[13];
    stat->gps.flagsActive[0] =
        ((uint32_t)(raw[14]) << 24) | ((uint32_t)(raw[15]) << 16) | ((uint32_t)(raw[16]) << 8) | (uint32_t)raw[17];
    stat->beidou.status = (int8_t)raw[18];
    stat->beidou.timeUntilSubframe =
        ((uint32_t)(raw[19]) << 24) | ((uint32_t)(raw[20]) << 16) | ((uint32_t)(raw[21]) << 8) | (uint32_t)raw[22];
    ...
    stat->beidou.flagsUpdateNeeded[0] =
        ((uint32_t)(raw[28]) << 24) | ((uint32_t)(raw[29]) << 16) | ((uint32_t)(raw[30]) << 8) | (uint32_t)raw[31];
    stat->beidou.flagsUpdateNeeded[1] =
        ((uint32_t)(raw[32]) << 24) | ((uint32_t)(raw[33]) << 16) | ((uint32_t)(raw[34]) << 8) | (uint32_t)raw[35];
    stat->beidou.flagsActive[0] =
        ((uint32_t)(raw[36]) << 24) | ((uint32_t)(raw[37]) << 16) | ((uint32_t)(raw[38]) << 8) | (uint32_t)raw[39];
    stat->beidou.flagsActive[1] =
        ((uint32_t)(raw[40]) << 24) | ((uint32_t)(raw[41]) << 16) | ((uint32_t)(raw[42]) << 8) | (uint32_t)raw[43];
    stat->beidouSvNoAlmanacFlags[0] =
        ((uint32_t)(raw[44]) << 24) | ((uint32_t)(raw[45]) << 16) | ((uint32_t)(raw[46]) << 8) | (uint32_t)raw[47];
    stat->beidouSvNoAlmanacFlags[1] =
        ((uint32_t)(raw[18]) << 24) | ((uint32_t)(raw[49]) << 16) | ((uint32_t)(raw[50]) << 8) | (uint32_t)raw[51];
...
int16_t LR11x0::updateGnssAlmanac(uint8_t constellation)
{
    ...
    // wait for scan finished or timeout, assumes 2 subframes and up to 2.3s
    // pre-roll
    ...
    RADIOLIB_DEBUG_BASIC_PRINTLN("GPS almanac update done in %lu ms",
                                 (long unsigned int)(this->mod->hal->millis() - start));
...
int16_t LR11x0::getGnssPosition(LR11x0GnssPosition_t *pos, bool filtered)
{
    ...
    int16_t state;
    if (filtered) {
        state = this->gnssReadDopplerSolverRes(&error, &pos->numSatsUsed, NULL, NULL, NULL, NULL, &pos->latitude,
                                               &pos->longitude, &pos->accuracy, NULL);
    } else {
        state = this->gnssReadDopplerSolverRes(&error, &pos->numSatsUsed, &pos->latitude, &pos->longitude,
                                               &pos->accuracy, NULL, NULL, NULL, NULL, NULL);
    }
    RADIOLIB_ASSERT(state);
...
int16_t LR11x0::gnssReadRssi(int8_t *rssi)
{
    ...
    uint8_t rplBuff[2] = {0};
    int16_t state =
        this->SPIcommand(RADIOLIB_LR11X0_CMD_GNSS_READ_RSSI, false, rplBuff, sizeof(rplBuff), reqBuff, sizeof(reqBuff));
    RADIOLIB_ASSERT(state);
    if (rssi) {
        *rssi = rplBuff[1];
    }
...
{
    uint8_t buff[7] = {(uint8_t)((gpsTime >> 24) & 0xFF),
                       (uint8_t)((gpsTime >> 16) & 0xFF),
                       (uint8_t)((gpsTime >> 8) & 0xFF),
                       (uint8_t)(gpsTime & 0xFF),
                       RADIOLIB_LR11X0_GNSS_AUTO_EFFORT_MODE,
                       resMask,
                       nbSvMask
                      };
...
int16_t LR11x0::gnssSetAssistancePosition(float lat, float lon)
{
    ...
    uint8_t buff[4] = {
        (uint8_t)((latRaw >> 8) & 0xFF),
        (uint8_t)(latRaw & 0xFF),
        (uint8_t)((lonRaw >> 8) & 0xFF),
        (uint8_t)(lonRaw & 0xFF),
    };
...
int16_t LR11x0::gnssGetContextStatus(uint8_t *fwVersion, uint32_t *almanacCrc, uint8_t *errCode, uint8_t *almUpdMask,
                                     uint8_t *freqSpace)
{
    // send the command - datasheet here shows extra bytes being sent in the
    // request but doing that fails so treat it like any other read command
    uint8_t buff[9] = {0};
    ...
    if (almanacCrc) {
        *almanacCrc =
            ((uint32_t)(buff[3]) << 24) | ((uint32_t)(buff[4]) << 16) | ((uint32_t)(buff[5]) << 8) | (uint32_t)buff[6];
    }
    if (errCode) {
        *errCode = (buff[7] & 0xF0) >> 4;
    }
    if (almUpdMask) {
        *almUpdMask = (buff[7] & 0x0E) >> 1;
    }
    if (freqSpace) {
        *freqSpace = ((buff[7] & 0x01) << 1) | ((buff[8] & 0x80) >> 7);
    }
...
int16_t LR11x0::gnssGetSvDetected(uint8_t *svId, uint8_t *snr, int16_t *doppler, size_t nbSv)
{
    ...
    if (state == RADIOLIB_ERR_NONE) {
        for (size_t i = 0; i < nbSv; i++) {
            ...
            if (snr) {
                snr[i] = dataBuff[4 * i + 1];
            }
            if (doppler) {
                doppler[i] = ((uint16_t)(dataBuff[4 * i + 2]) << 8) | (uint16_t)dataBuff[4 * i + 3];
            }
        }
    }
...
int16_t LR11x0::gnssGetConsumption(uint32_t *cpu, uint32_t *radio)
{
    ...
    if (cpu) {
        *cpu =
            ((uint32_t)(buff[0]) << 24) | ((uint32_t)(buff[1]) << 16) | ((uint32_t)(buff[2]) << 8) | (uint32_t)buff[3];
    }
    if (radio) {
        *radio =
            ((uint32_t)(buff[4]) << 24) | ((uint32_t)(buff[5]) << 16) | ((uint32_t)(buff[6]) << 8) | (uint32_t)buff[7];
    }
...
int16_t LR11x0::gnssGetResultSize(uint16_t *size)
{
    ...
    // pass the replies
    if (size) {
        *size = ((uint16_t)(buff[0]) << 8) | (uint16_t)buff[1];
    }
...
int16_t LR11x0::gnssAlmanacFullUpdateHeader(uint16_t date, uint32_t globalCrc)
{
    uint8_t buff[RADIOLIB_LR11X0_GNSS_ALMANAC_BLOCK_SIZE] = {
        ...
        (uint8_t)((date >> 8) & 0xFF),
        (uint8_t)(date & 0xFF),
        (uint8_t)((globalCrc >> 24) & 0xFF),
        (uint8_t)((globalCrc >> 16) & 0xFF),
        (uint8_t)((globalCrc >> 8) & 0xFF),
        (uint8_t)(globalCrc & 0xFF),
    };
...
int16_t LR11x0::gnssAlmanacReadAddrSize(uint32_t *addr, uint16_t *size)
{
    ...
    if (size) {
        *size = ((uint16_t)(buff[4]) << 8) | (uint16_t)buff[5];
    }
...
{
    uint8_t reqBuff[2] = {svId, 0x01}; // in theory multiple SV entries can be read at
    // the same time, but we don't need that
    int16_t state = this->SPIcommand(RADIOLIB_LR11X0_CMD_GNSS_READ_ALMANAC_PER_SATELLITE, false, almanac, 22, reqBuff,
                                     sizeof(reqBuff));
    RADIOLIB_ASSERT(state);
    return (state);
}
...
int16_t LR11x0::gnssGetNbSvVisible(uint32_t time, float lat, float lon, uint8_t constellation, uint8_t *nbSv)
{
    ...
    int16_t lonRaw = (lon * 2048.0f) / 180.0f + 0.5f;
    uint8_t reqBuff[9] = {
        (uint8_t)((time >> 24) & 0xFF), (uint8_t)((time >> 16) & 0xFF), (uint8_t)((time >> 8) & 0xFF),
        (uint8_t)(time & 0xFF), (uint8_t)((latRaw >> 8) & 0xFF), (uint8_t)(latRaw & 0xFF),
        (uint8_t)((lonRaw >> 8) & 0xFF), (uint8_t)(lonRaw & 0xFF),        constellation,
    };
...
int16_t LR11x0::gnssGetSvVisible(uint8_t nbSv, uint8_t **svId, int16_t **doppler, int16_t **dopplerErr)
{
    ...
    for (uint8_t i = 0; i < nbSv; i++) {
        ...
        if (doppler && doppler[i]) {
            *doppler[i] = ((uint16_t)(buff[i * 12 + 1]) << 8) | (uint16_t)buff[i * 12 + 2];
        }
        if (dopplerErr && dopplerErr[i]) {
            *dopplerErr[i] = ((uint16_t)(buff[i * 12 + 3]) << 8) | (uint16_t)buff[i * 12 + 4];
        }
    }
...
int16_t LR11x0::gnssPerformScan(uint8_t effort, uint8_t resMask, uint8_t nbSvMax)
{
    ...
    // call the SPI write stream directly to skip waiting for BUSY - it will be
    // set to high once the scan starts
...
int16_t LR11x0::gnssReadTime(uint8_t *err, uint32_t *time, uint32_t *nbUs, uint32_t *timeAccuracy)
{
    ...
    if (time) {
        *time =
            ((uint32_t)(buff[1]) << 24) | ((uint32_t)(buff[2]) << 16) | ((uint32_t)(buff[3]) << 8) | (uint32_t)buff[4];
    ...
    if (timeAccuracy) {
        *timeAccuracy = ((uint32_t)(buff[8]) << 24) | ((uint32_t)(buff[9]) << 16) | ((uint32_t)(buff[10]) << 8) |
                        (uint32_t)buff[11];
        *timeAccuracy /= 16;
    }
...
int16_t LR11x0::gnssReadCumulTiming(uint32_t *timing, uint8_t *constDemod)
{
    ...
    if (timing) {
        for (size_t i = 0; i < 31; i++) {
            timing[i] =
                ((uint32_t)rplBuff[i * sizeof(uint32_t)] << 24) | ((uint32_t)rplBuff[1 + i * sizeof(uint32_t)] << 16) |
                ((uint32_t)rplBuff[2 + i * sizeof(uint32_t)] << 8) | (uint32_t)rplBuff[3 + i * sizeof(uint32_t)];
        }
    }

    if (constDemod) {
        *constDemod = rplBuff[124];
    }
...
int16_t LR11x0::gnssSetTime(uint32_t time, uint16_t accuracy)
{
    uint8_t buff[6] = {
        (uint8_t)((time >> 24) & 0xFF), (uint8_t)((time >> 16) & 0xFF), (uint8_t)((time >> 8) & 0xFF),
        (uint8_t)(time & 0xFF), (uint8_t)((accuracy >> 8) & 0xFF), (uint8_t)(accuracy & 0xFF),
    };
    return (this->SPIcommand(RADIOLIB_LR11X0_CMD_GNSS_SET_TIME, true, buff, sizeof(buff)));
}

int16_t LR11x0::gnssReadDopplerSolverRes(uint8_t *error, uint8_t *nbSvUsed, float *lat, float *lon, uint16_t *accuracy,
                                         ...
                                         uint16_t *xtalFilt)
{
    ...
    if (accuracy) {
        *accuracy = ((uint16_t)(buff[6]) << 8) | (uint16_t)buff[7];
    }
    if (xtal) {
        *xtal = ((uint16_t)(buff[8]) << 8) | (uint16_t)buff[9];
    }
    if (latFilt) {
    ...
    if (accuracyFilt) {
        *accuracyFilt = ((uint16_t)(buff[14]) << 8) | (uint16_t)buff[15];
    }
    if (xtalFilt) {
        *xtalFilt = ((uint16_t)(buff[16]) << 8) | (uint16_t)buff[17];
    }
...
int16_t LR11x0::gnssReadDelayResetAP(uint32_t *delay)
{
    ...
    if (delay) {
        *delay = ((uint32_t)(buff[0]) << 16) | ((uint32_t)(buff[1]) << 8) | (uint32_t)buff[2];
    }
...
int16_t LR11x0::gnssAlmanacUpdateFromSat(uint8_t effort, uint8_t bitMask)
{
    ...
    // set to high once the scan starts
    return (
               this->mod->SPIwriteStream(RADIOLIB_LR11X0_CMD_GNSS_ALMANAC_UPDATE_FROM_SAT, buff, sizeof(buff), false, false));
}
...
int16_t LR11x0::gnssReadKeepSyncStatus(uint8_t mask, uint8_t *nbSvVisible, uint32_t *elapsed)
{
    ...
    uint8_t rplBuff[5] = {0};
    int16_t state = this->SPIcommand(RADIOLIB_LR11X0_CMD_GNSS_READ_KEEP_SYNC_STATUS, false, rplBuff, sizeof(rplBuff),
                                     reqBuff, sizeof(reqBuff));
    RADIOLIB_ASSERT(state);
    if (nbSvVisible) {
        *nbSvVisible = rplBuff[0];
    }
    if (elapsed) {
        *elapsed = ((uint32_t)(rplBuff[1]) << 24) | ((uint32_t)(rplBuff[2]) << 16) | ((uint32_t)(rplBuff[3]) << 8) |
                   (uint32_t)rplBuff[4];
    }
    return (state);
}
...
int16_t LR11x0::gnssReadAlmanacUpdatePeriod(uint8_t bitMask, uint8_t svType, uint16_t *period)
{
    ...
    uint8_t rplBuff[2] = {0};
    int16_t state = this->SPIcommand(RADIOLIB_LR11X0_CMD_GNSS_READ_ALMANAC_UPDATE_PERIOD, false, rplBuff,
                                     sizeof(rplBuff), reqBuff, sizeof(reqBuff));
    RADIOLIB_ASSERT(state);

    if (period) {
        *period = ((uint16_t)(rplBuff[0]) << 8) | (uint16_t)rplBuff[1];
    }
...
int16_t LR11x0::gnssGetSvWarmStart(uint8_t bitMask, uint8_t *sv, uint8_t nbVisSat)
{
    ...
    return (
               this->SPIcommand(RADIOLIB_LR11X0_CMD_GNSS_GET_SV_WARM_START, false, sv, nbVisSat, reqBuff, sizeof(reqBuff)));
}
...
int16_t LR11x0::gnssReadWarmStartStatus(uint8_t bitMask, uint8_t *nbVisSat, uint32_t *timeElapsed)
{
    uint8_t reqBuff[1] = {bitMask};
    uint8_t rplBuff[5] = {0};
    int16_t state = this->SPIcommand(RADIOLIB_LR11X0_CMD_GNSS_READ_WARM_START_STATUS, false, rplBuff, sizeof(rplBuff),
                                     reqBuff, sizeof(reqBuff));
    RADIOLIB_ASSERT(state);
    ...
    if (timeElapsed) {
        *timeElapsed = ((uint32_t)(rplBuff[1]) << 24) | ((uint32_t)(rplBuff[2]) << 16) | ((uint32_t)(rplBuff[3]) << 8) |
                       (uint32_t)rplBuff[4];
    }
...
int16_t LR11x0::gnssWriteBitMaskSatActivated(uint8_t bitMask, uint32_t *bitMaskActivated0,
                                             uint32_t *bitMaskActivated1)
{
    uint8_t reqBuff[1] = {bitMask};
    uint8_t rplBuff[8] = {0};
    size_t rplLen = (bitMask & 0x01) ? 8 : 4; // GPS only has the first bit mask
    int16_t state = this->SPIcommand(RADIOLIB_LR11X0_CMD_GNSS_WRITE_BIT_MASK_SAT_ACTIVATED, false, rplBuff, rplLen,
                                     reqBuff, sizeof(reqBuff));
    RADIOLIB_ASSERT(state);

    if (bitMaskActivated0) {
        *bitMaskActivated0 = ((uint32_t)(rplBuff[0]) << 24) | ((uint32_t)(rplBuff[1]) << 16) |
                             ((uint32_t)(rplBuff[2]) << 8) | (uint32_t)rplBuff[3];
    }
    if (bitMaskActivated1) {
        *bitMaskActivated1 = ((uint32_t)(rplBuff[4]) << 24) | ((uint32_t)(rplBuff[5]) << 16) |
                             ((uint32_t)(rplBuff[6]) << 8) | (uint32_t)rplBuff[7];
    }
...
void LR11x0::gnssAbort()
{
    ...
    // we need to call the most basic overload of the SPI write method otherwise
