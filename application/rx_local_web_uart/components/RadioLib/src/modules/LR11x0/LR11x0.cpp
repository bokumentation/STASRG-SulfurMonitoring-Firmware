...
int16_t LR11x0::beginLRFHSS(uint8_t bw, uint8_t cr, bool narrowGrid, float tcxoVoltage)
{
    ...
    return (setModulationParamsLrFhss(RADIOLIB_LR11X0_LR_FHSS_BIT_RATE_RAW,
                                      RADIOLIB_LR11X0_LR_FHSS_SHAPING_GAUSSIAN_BT_1_0));
}

int16_t LR11x0::beginGNSS(uint8_t constellations, float tcxoVoltage)
{
    // set module properties and perform initial setup - packet type does not
    // matter
    int16_t state = this->modSetup(tcxoVoltage, RADIOLIB_LR11X0_PACKET_TYPE_LORA);
    RADIOLIB_ASSERT(state);
...
int16_t LR11x0::receive(uint8_t *data, size_t len, RadioLibTime_t timeout)
{
    ...
    if (!timeoutInternal) {
        ...
        if ((modem == RADIOLIB_LR11X0_PACKET_TYPE_LORA) || (modem == RADIOLIB_LR11X0_PACKET_TYPE_GFSK)) {
            ...
            size_t maxLen = len;
            if (len == 0) {
                maxLen = RADIOLIB_LR11X0_MAX_PACKET_LENGTH;
            }
...
int16_t LR11x0::sleep(bool retainConfig, uint32_t sleepTime)
{
    ...
    uint8_t buff[] = {
        ...
        (uint8_t)((sleepTime >> 24) & 0xFF),
        (uint8_t)((sleepTime >> 16) & 0xFF),
        (uint8_t)((sleepTime >> 16) & 0xFF),
        (uint8_t)(sleepTime & 0xFF),
    };
    if (sleepTime) {
...
{
    return (this->startReceive(RADIOLIB_LR11X0_RX_TIMEOUT_INF, RADIOLIB_IRQ_RX_DEFAULT_FLAGS,
                               RADIOLIB_IRQ_RX_DEFAULT_MASK, 0));
}

uint32_t LR11x0::getIrqStatus()
{
    // there is no dedicated "get IRQ" command, the IRQ bits are sent after the
    // status bytes
    uint8_t buff[6] = {0};
    ...
    uint32_t irq =
        ((uint32_t)(buff[2]) << 24) | ((uint32_t)(buff[3]) << 16) | ((uint32_t)(buff[4]) << 8) | (uint32_t)buff[5];
    return (irq);
}
...
int16_t LR11x0::readData(uint8_t *data, size_t len)
{
    ...
    if ((irq & RADIOLIB_LR11X0_IRQ_CRC_ERR) ||
            ((irq & RADIOLIB_LR11X0_IRQ_HEADER_ERR) && !(irq & RADIOLIB_LR11X0_IRQ_SYNC_WORD_HEADER_VALID))) {
        crcState = RADIOLIB_ERR_CRC_MISMATCH;
    }

    // get packet length
    // the offset is needed since LR11x0 seems to move the buffer base by 4 bytes
    // on every packet
    uint8_t offset = 0;
...
int16_t LR11x0::startChannelScan(const ChannelScanConfig_t &config)
{
    ...
    // set DIO pin mapping
    uint16_t irqFlags = (config.cad.irqFlags == RADIOLIB_IRQ_NOT_SUPPORTED)
                        ? RADIOLIB_LR11X0_IRQ_CAD_DETECTED | RADIOLIB_LR11X0_IRQ_CAD_DONE
                        : config.cad.irqFlags;
    ...
    // set mode to CAD
    return (
               startCad(config.cad.symNum, config.cad.detPeak, config.cad.detMin, config.cad.exitMode, config.cad.timeout));
}
...
int16_t LR11x0::setSpreadingFactor(uint8_t sf, bool legacy)
{
    ...
    if (legacy && (sf == 6)) {
        // this->mod->SPIsetRegValue(RADIOLIB_LR11X0_REG_SF6_SX127X_COMPAT,
        // RADIOLIB_LR11X0_SF6_SX127X, 18, 18);
    }
...
int16_t LR11x0::setFrequencyDeviation(float freqDev)
{
    ...
    // set frequency deviation to lowest available setting (required for
    // digimodes)
...
int16_t LR11x0::setSyncWord(uint8_t *syncWord, size_t len)
{
    ...
    if (type == RADIOLIB_LR11X0_PACKET_TYPE_GFSK) {
        ...
        state = setPacketParamsGFSK(this->preambleLengthGFSK, this->preambleDetLength, this->syncWordLength,
                                    this->addrComp, this->packetType, RADIOLIB_LR11X0_MAX_PACKET_LENGTH,
        ...
        // with length set to 1 and LoRa modem active, assume it is the LoRa sync
        // word
        if (len > 1) {
            return (RADIOLIB_ERR_INVALID_SYNC_WORD);
        }
        ...
        // with length set to 4 and LR-FHSS modem active, assume it is the LR-FHSS
        // sync word
...
int16_t LR11x0::setNodeAddress(uint8_t nodeAddr)
{
    ...
    state =
        setPacketParamsGFSK(this->preambleLengthGFSK, this->preambleDetLength, this->syncWordLength, this->addrComp,
                            this->packetType, RADIOLIB_LR11X0_MAX_PACKET_LENGTH, this->crcTypeGFSK, this->whitening);
    RADIOLIB_ASSERT(state);
...
int16_t LR11x0::disableAddressFiltering()
{
    ...
    return (setPacketParamsGFSK(this->preambleLengthGFSK, this->preambleDetLength, this->syncWordLength, this->addrComp,
                                this->packetType, RADIOLIB_LR11X0_MAX_PACKET_LENGTH, this->crcTypeGFSK,
                                this->whitening));
}
...
int16_t LR11x0::setDataRate(DataRate_t dr, ModemType_t modem)
{
    ...
    if (modem == RADIOLIB_MODEM_FSK) {
        ...
        // set hopping grid
        this->lrFhssGrid =
            dr.lrFhss.narrowGrid ? RADIOLIB_LR11X0_LR_FHSS_GRID_STEP_NON_FCC : RADIOLIB_LR11X0_LR_FHSS_GRID_STEP_FCC;
    }
...
int16_t LR11x0::setPreambleLength(size_t preambleLength)
{
    ...
    if (type == RADIOLIB_LR11X0_PACKET_TYPE_LORA) {
        ...
        return (setPacketParamsLoRa(this->preambleLengthLoRa, this->headerType, this->implicitLen, this->crcTypeLoRa,
        ...
        return (setPacketParamsGFSK(this->preambleLengthGFSK, this->preambleDetLength, this->syncWordLength,
                                    this->addrComp, this->packetType, RADIOLIB_LR11X0_MAX_PACKET_LENGTH,
                                    this->crcTypeGFSK, this->whitening));
    }
...
int16_t LR11x0::setCRC(uint8_t len, uint32_t initial, uint32_t polynomial, bool inverted)
{
    ...
    if (type == RADIOLIB_LR11X0_PACKET_TYPE_LORA) {
        ...
        state = setPacketParamsLoRa(this->preambleLengthLoRa, this->headerType, this->implicitLen, this->crcTypeLoRa,
                                    (uint8_t)this->invertIQEnabled);
...
RadioLibTime_t LR11x0::getTimeOnAir(size_t len)
{
    ...
    case ModemType_t::RADIOLIB_MODEM_FSK: {
        ...
        uint8_t crcLen = 0;
        if (this->crcTypeGFSK == RADIOLIB_LR11X0_GFSK_CRC_1_BYTE ||
                this->crcTypeGFSK == RADIOLIB_LR11X0_GFSK_CRC_1_BYTE_INV) {
            crcLen = 1;
        } else if (this->crcTypeGFSK == RADIOLIB_LR11X0_GFSK_CRC_2_BYTE ||
                   this->crcTypeGFSK == RADIOLIB_LR11X0_GFSK_CRC_2_BYTE_INV) {
            crcLen = 2;
        }
...
int16_t LR11x0::setOutputPower(int8_t power, uint8_t paSel, uint8_t regPaSupply, uint8_t paDutyCycle, uint8_t paHpSel,
                               uint32_t rampTimeUs)
{
    // set PA config
...
int16_t LR11x0::setLrFhssConfig(uint8_t bw, uint8_t cr, uint8_t hdrCount, uint16_t hopSeed)
{
    ...
    RADIOLIB_CHECK_RANGE((int8_t)cr, (int8_t)RADIOLIB_LR11X0_LR_FHSS_CR_5_6, (int8_t)RADIOLIB_LR11X0_LR_FHSS_CR_1_3,
                         RADIOLIB_ERR_INVALID_CODING_RATE);
    this->lrFhssCr = cr;
    RADIOLIB_CHECK_RANGE((int8_t)bw, (int8_t)RADIOLIB_LR11X0_LR_FHSS_BW_39_06,
                         (int8_t)RADIOLIB_LR11X0_LR_FHSS_BW_1574_2, RADIOLIB_ERR_INVALID_BANDWIDTH);
    this->lrFhssBw = bw;
...
int16_t LR11x0::updateFirmware(const uint32_t *image, size_t size, bool nonvolatile)
{
    ...
    if (device != RADIOLIB_LR11X0_DEVICE_BOOT) {
        RADIOLIB_DEBUG_BASIC_PRINTLN("Failed to put device to bootloader mode, %02x != %02x", (unsigned int)device,
                                     (unsigned int)RADIOLIB_LR11X0_DEVICE_BOOT);
    ...
    RADIOLIB_DEBUG_BASIC_PRINTLN("Writing image in %lu chunks, last chunk size is %lu words", (unsigned long)numWrites,
                                 (unsigned long)rem);
    for (size_t i = 0; i < numWrites; i++) {
        uint32_t offset = i * maxLen;
        uint32_t len = (i == (numWrites - 1)) ? rem : maxLen;
        RADIOLIB_DEBUG_BASIC_PRINTLN("Writing chunk %d at offset %08lx (%u words)", (int)i, (unsigned long)offset,
                                     (unsigned int)len);
        this->bootWriteFlashEncrypted(offset * sizeof(uint32_t), const_cast<uint32_t *>(&image[offset]), len,
                                      nonvolatile);
    }
    ...
    RADIOLIB_ASSERT(state);
    if (device == RADIOLIB_LR11X0_DEVICE_BOOT) {
        RADIOLIB_DEBUG_BASIC_PRINTLN("Failed to kick device from bootloader mode, %02x == %02x", (unsigned int)device,
                                     (unsigned int)RADIOLIB_LR11X0_DEVICE_BOOT);
        return (RADIOLIB_ERR_CHIP_NOT_FOUND);
    }
...
int16_t LR11x0::stageMode(RadioModeType_t mode, RadioModeConfig_t *cfg)
{
    ...
    case (RADIOLIB_RADIO_MODE_RX): {
        ...
        if ((this->headerType == RADIOLIB_LR11X0_LORA_HEADER_IMPLICIT) && (modem == RADIOLIB_LR11X0_PACKET_TYPE_LORA)) {
            state = setPacketParamsLoRa(this->preambleLengthLoRa, this->headerType, this->implicitLen,
    ...
    case (RADIOLIB_RADIO_MODE_TX): {
        ...
        if ((this->addrComp != RADIOLIB_LR11X0_GFSK_ADDR_FILTER_DISABLED) &&
                (cfg->transmit.len > RADIOLIB_LR11X0_MAX_PACKET_LENGTH - 1)) {
            return (RADIOLIB_ERR_PACKET_TOO_LONG);
        }
        ...
        if (modem == RADIOLIB_LR11X0_PACKET_TYPE_LORA) {
            ...
            state = setPacketParamsGFSK(this->preambleLengthGFSK, this->preambleDetLength, this->syncWordLength,
                                        this->addrComp, this->packetType, cfg->transmit.len, this->crcTypeGFSK,
                                        this->whitening);
        ...
        if (modem == RADIOLIB_LR11X0_PACKET_TYPE_LR_FHSS) {
            ...
            state = lrFhssBuildFrame(this->lrFhssHdrCount, this->lrFhssCr, this->lrFhssGrid, true, this->lrFhssBw,
                                     this->lrFhssHopSeq, 0, cfg->transmit.data, cfg->transmit.len);
            RADIOLIB_ASSERT(state);
...
int16_t LR11x0::workaroundGFSK()
{
    ...
    if ((this->bitRate == 1200) && (this->frequencyDev == 5000) &&
            (this->rxBandwidth == RADIOLIB_LR11X0_GFSK_RX_BW_19_5)) {
        // workaround for 1.2 kbps
        valFix2 = 0x04;

    } else if ((this->bitRate == 600) && (this->frequencyDev == 800) &&
               (this->rxBandwidth == RADIOLIB_LR11X0_GFSK_RX_BW_4_8)) {
...
{
    // the status check command doesn't return status in the same place as other
    // read commands, but only as the first byte (as with any other command),
    // hence LR11x0::SPIcommand can't be used it also seems to ignore the actual
    // command, and just sending in bunch of NOPs will work
    uint8_t buff[6] = {0};
...
int16_t LR11x0::SPIcommand(uint16_t cmd, bool write, uint8_t *data, size_t len, const uint8_t *out, size_t outLen)
{
    int16_t state = RADIOLIB_ERR_UNKNOWN;
    if (!write) {
        // the SPI interface of LR11x0 requires two separate transactions for
        // reading send the 16-bit command
    ...
    while ((i < 10) && !flagFound) {
        ...
        if ((info.device == ver) || (info.device == RADIOLIB_LR11X0_DEVICE_BOOT)) {
            ...
            if (info.device == RADIOLIB_LR11X0_DEVICE_BOOT) {
                RADIOLIB_DEBUG_BASIC_PRINTLN("Warning: device is in bootloader mode! "
                                             "Only FW update is possible now.");
            }
            flagFound = true;
        } else {
            RADIOLIB_DEBUG_BASIC_PRINTLN("LR11x0 not found! (%d of 10 tries) "
                                         "RADIOLIB_LR11X0_CMD_GET_VERSION = 0x%02x",
                                         i + 1, info.device);
...
int16_t LR11x0::setPacketMode(uint8_t mode, uint8_t len)
{
    ...
    state = setPacketParamsGFSK(this->preambleLengthGFSK, this->preambleDetLength, this->syncWordLength, this->addrComp,
                                mode, len, this->crcTypeGFSK, this->whitening);
    RADIOLIB_ASSERT(state);
...
int16_t LR11x0::startCad(uint8_t symbolNum, uint8_t detPeak, uint8_t detMin, uint8_t exitMode, RadioLibTime_t timeout)
{
    ...
    // TODO the magic numbers are based on Semtech examples, this is probably
    // suboptimal
