...
void CC1101::reset()
{
    // just send the command, the reset sequence as described in datasheet seems
...
int16_t CC1101::receiveDirect(bool sync)
{
    ...
    // enable promiscuous mode - needed for protocols that decode in software
    // (e.g. PagerClient)
...
{
    int16_t state = SPIsetRegValue(
                        RADIOLIB_CC1101_REG_PKTCTRL1,
                        RADIOLIB_CC1101_CRC_AUTOFLUSH_OFF | RADIOLIB_CC1101_APPEND_STATUS_ON | RADIOLIB_CC1101_ADR_CHK_NONE, 3, 0);
    state |= SPIsetRegValue(RADIOLIB_CC1101_REG_PKTCTRL0,
                            RADIOLIB_CC1101_WHITE_DATA_OFF | RADIOLIB_CC1101_PKT_FORMAT_NORMAL, 6, 4);
...
int16_t CC1101::startTransmit(const uint8_t *data, size_t len, uint8_t addr)
{
    ...
    while (dataSent < len) {
        ...
        // Check number of bytes on FIFO twice due to the CC1101 errata. Block until
        // two reads are equal.
        do {
        ...
        if (fifoBytes < RADIOLIB_CC1101_FIFO_SIZE) {
            uint8_t bytesToWrite =
                RADIOLIB_MIN((uint8_t)(RADIOLIB_CC1101_FIFO_SIZE - fifoBytes), (uint8_t)(len - dataSent));
...
int16_t CC1101::startReceive()
{
    ...
    // RADIOLIB_CC1101_GDOX_SYNC_WORD_SENT_OR_PKT_RECEIVED gets triggered by both
    // packet received as well as packet discarded,
    // RADIOLIB_CC1101_GDOX_PKT_RECEIVED_CRC_OK does not get triggered with CRC
    // disabled
...
int16_t CC1101::readData(uint8_t *data, size_t len)
{
    ...
    // check if status bytes are enabled (default:
    // RADIOLIB_CC1101_APPEND_STATUS_ON)
    bool isAppendStatus = SPIgetRegValue(RADIOLIB_CC1101_REG_PKTCTRL1, 2, 2) == RADIOLIB_CC1101_APPEND_STATUS_ON;

    // if status byte is enabled at least 2 bytes (2 status bytes + any following
    // packet) will remain in FIFO.
    int16_t state = RADIOLIB_ERR_NONE;
    if (isAppendStatus) {
        // read RSSI byte
    ...
    // flush then standby according to RXOFF_MODE (default:
    // RADIOLIB_CC1101_RXOFF_IDLE)
...
int16_t CC1101::setFrequency(float freq)
{
    ...
    if (!(((freq >= 300.0f) && (freq <= 348.0f)) || ((freq >= 387.0f) && (freq <= 464.0f)) ||
    ...
    // Update the TX power accordingly to new freq. (PA values depend on chosen
    // freq)
...
int16_t CC1101::checkOutputPower(int8_t power, int8_t *clipped, uint8_t *raw)
{
    ...
    uint8_t paTable[8][4] = {{0x12, 0x12, 0x03, 0x03}, {0x0D, 0x0E, 0x0F, 0x0E}, {0x1C, 0x1D, 0x1E, 0x1E},
...
int16_t CC1101::enableSyncWordFiltering(uint8_t maxErrBits, bool requireCarrierSense)
{
    ...
    case 0:
        ...
        state |= SPIsetRegValue(
                     RADIOLIB_CC1101_REG_MDMCFG2,
                     (requireCarrierSense ? RADIOLIB_CC1101_SYNC_MODE_16_16_THR : RADIOLIB_CC1101_SYNC_MODE_16_16), 2, 0);
        break;
    case 1:
        // in 16 bit sync word, expect at least 15 bits
        state |= SPIsetRegValue(
                     RADIOLIB_CC1101_REG_MDMCFG2,
                     (requireCarrierSense ? RADIOLIB_CC1101_SYNC_MODE_15_16_THR : RADIOLIB_CC1101_SYNC_MODE_15_16), 2, 0);
        break;
    default:
...
int16_t CC1101::disableSyncWordFiltering(bool requireCarrierSense)
{
    int16_t state = SPIsetRegValue(
                        RADIOLIB_CC1101_REG_MDMCFG2,
                        (requireCarrierSense ? RADIOLIB_CC1101_SYNC_MODE_NONE_THR : RADIOLIB_CC1101_SYNC_MODE_NONE), 2, 0);
    return (state);
}
...
int16_t CC1101::setPromiscuousMode(bool enable, bool requireCarrierSense)
{
    ...
    if (enable) {
        ...
        // We have to set the length to set PQT, but it should get disabled with
        // disableSyncWordFiltering()
        state = setPreambleLength(16, 0);
        RADIOLIB_ASSERT(state);
        // disable sync word filtering and insertion
        // this also disables preamble
        // Can enable Sync Mode with carriersense when promiscuous is enabled.
        // Default is false: Sync Mode None
...
int16_t CC1101::beginCommon(float freq, float br, float freqDev, float rxBw, int8_t pwr, uint8_t preambleLength)
{
    ...
    while ((i < 10) && !flagFound) {
        ...
        if ((version == RADIOLIB_CC1101_VERSION_CURRENT) || (version == RADIOLIB_CC1101_VERSION_LEGACY) ||
                (version == RADIOLIB_CC1101_VERSION_CLONE)) {
            flagFound = true;
        } else {
            RADIOLIB_DEBUG_BASIC_PRINTLN("CC1101 not found! (%d of 10 tries) RADIOLIB_CC1101_REG_VERSION == "
                                         "0x%04X, expected 0x0004/0x0014",
                                         i + 1, version);
            this->mod->hal->delay(10);
            i++;
        }
...
int16_t CC1101::config()
{
    ...
    // set GDOs to Hi-Z so that it doesn't output clock on startup (might confuse
    // GDO0 action)
...
void CC1101::getExpMant(float target, uint16_t mantOffset, uint8_t divExp, uint8_t expMax, uint8_t &exp,
                        uint8_t &mant)
{
