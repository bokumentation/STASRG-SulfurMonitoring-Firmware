...
int16_t SX1233::begin(float freq, float br, float freqDev, float rxBw, int8_t power, uint8_t preambleLen)
{
    ...
    while ((i < 10) && !flagFound) {
        ...
        if ((version == RADIOLIB_SX123X_CHIP_REVISION_2_A) || (version == RADIOLIB_SX123X_CHIP_REVISION_2_B) ||
                (version == RADIOLIB_SX123X_CHIP_REVISION_2_C) || (version == RADIOLIB_SX123X_CHIP_REVISION_2_D)) {
            flagFound = true;
            this->chipRevision = version;
        } else {
            RADIOLIB_DEBUG_BASIC_PRINTLN("SX1231 not found! (%d of 10 tries) RF69_REG_VERSION == 0x%04X, "
                                         "expected 0x0021 / 0x0022 / 0x0023",
                                         i + 1, version);
            mod->hal->delay(10);
            i++;
        }
    ...
    // default sync word values 0x2D01 is the same as the default in LowPowerLab
    // RFM69 library
