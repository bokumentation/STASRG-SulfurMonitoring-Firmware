...
int16_t nRF24::receive(uint8_t *data, size_t len, RadioLibTime_t timeout)
{
    ...
    if (!timeoutInternal) {
        // calculate timeout (15 retries * 4ms (max Tx time as per datasheet) + 10
        // ms)
...
int16_t nRF24::setCrcFiltering(bool crcOn)
{
    ...
    // Disable CRC
    return this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_CONFIG,
                                     (crcOn ? RADIOLIB_NRF24_CRC_ON : RADIOLIB_NRF24_CRC_OFF), 3, 3);
}

int16_t nRF24::setAutoAck(bool autoAckOn)
{
    return this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_EN_AA,
                                     (autoAckOn ? RADIOLIB_NRF24_AA_ALL_ON : RADIOLIB_NRF24_AA_ALL_OFF), 5, 0);
}
...
int16_t nRF24::setAutoAck(uint8_t pipeNum, bool autoAckOn)
{
    ...
    case 0:
        return this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_EN_AA,
                                         (autoAckOn ? RADIOLIB_NRF24_AA_P0_ON : RADIOLIB_NRF24_AA_P0_OFF), 0, 0);
        break;
    case 1:
        return this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_EN_AA,
                                         (autoAckOn ? RADIOLIB_NRF24_AA_P1_ON : RADIOLIB_NRF24_AA_P1_OFF), 1, 1);
        break;
    case 2:
        return this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_EN_AA,
                                         (autoAckOn ? RADIOLIB_NRF24_AA_P2_ON : RADIOLIB_NRF24_AA_P2_OFF), 2, 2);
        break;
    case 3:
        return this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_EN_AA,
                                         (autoAckOn ? RADIOLIB_NRF24_AA_P3_ON : RADIOLIB_NRF24_AA_P3_OFF), 3, 3);
        break;
    case 4:
        return this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_EN_AA,
                                         (autoAckOn ? RADIOLIB_NRF24_AA_P4_ON : RADIOLIB_NRF24_AA_P4_OFF), 4, 4);
        break;
    case 5:
        return this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_EN_AA,
                                         (autoAckOn ? RADIOLIB_NRF24_AA_P5_ON : RADIOLIB_NRF24_AA_P5_OFF), 5, 5);
        break;
    default:
        return (RADIOLIB_ERR_INVALID_PIPE_NUMBER);
    }
...
{
    return (this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_RF_SETUP,
                                      enable ? RADIOLIB_NRF24_RF_LNA_ON : RADIOLIB_NRF24_RF_LNA_OFF, 0, 0));
}

void nRF24::clearIRQ()
{
    // clear status bits
    this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_STATUS,
                              RADIOLIB_NRF24_RX_DR | RADIOLIB_NRF24_TX_DS | RADIOLIB_NRF24_MAX_RT, 6, 4);

    // disable interrupts
    this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_CONFIG,
                              RADIOLIB_NRF24_MASK_RX_DR_IRQ_OFF | RADIOLIB_NRF24_MASK_TX_DS_IRQ_OFF |
                              RADIOLIB_NRF24_MASK_MAX_RT_IRQ_OFF,
                              6, 4);
}
...
int16_t nRF24::config()
{
    // enable 16-bit CRC
    int16_t state =
        this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_CONFIG, RADIOLIB_NRF24_CRC_ON | RADIOLIB_NRF24_CRC_16, 3, 2);
    RADIOLIB_ASSERT(state);

    // set 15 retries and delay 1500 (5*250) us
    this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_SETUP_RETR, (5 << 4) | 5);

    // set features: dynamic payload on, payload with ACK packets off, dynamic ACK
    // off
    state = this->mod->SPIsetRegValue(RADIOLIB_NRF24_REG_FEATURE,
                                      RADIOLIB_NRF24_DPL_ON | RADIOLIB_NRF24_ACK_PAY_OFF | RADIOLIB_NRF24_DYN_ACK_OFF,
                                      2, 0);
    RADIOLIB_ASSERT(state);
