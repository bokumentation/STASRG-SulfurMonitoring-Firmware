...
#define RADIOLIB_NRF24_MASK_RX_DR_IRQ_ON 0b00000000  //  6     6   RX_DR will be reflected on IRQ pin as active low
//  (default)
#define RADIOLIB_NRF24_MASK_TX_DS_IRQ_OFF 0b00100000 //  5     5   TX_DS will not be reflected on IRQ pin
#define RADIOLIB_NRF24_MASK_TX_DS_IRQ_ON 0b00000000  //  5     5   TX_DS will be reflected on IRQ pin as active low
//  (default)
#define RADIOLIB_NRF24_MASK_MAX_RT_IRQ_OFF 0b00010000 //  4     4   MAX_RT will not be reflected on IRQ pin
#define RADIOLIB_NRF24_MASK_MAX_RT_IRQ_ON 0b00000000  //  4     4   MAX_RT will be reflected on IRQ pin as active low
//  (default)
...
#define RADIOLIB_NRF24_ARD 0b00000000 //  7     4   auto retransmit delay: t[us] = (NRF24_ARD + 1) * 250
//  us
#define RADIOLIB_NRF24_ARC_OFF 0b00000000 //  3     0   auto retransmit count: auto retransmit disabled
#define RADIOLIB_NRF24_ARC 0b00000011     //  3     0                          up to 3 retransmits on AA fail
//  (default)
...
#define RADIOLIB_NRF24_MAX_RT 0b00010000 //  4     4   maximum number of retransmits reached (must be
//  cleared to continue)
...
#define RADIOLIB_NRF24_RP_BELOW_64_DBM 0b00000000 //  0     0   received power in the current channel: less than -64
//  dBm
#define RADIOLIB_NRF24_RP_ABOVE_64_DBM 0b00000001 //  0     0                                            more than
//  -64 dBm
...
public:
    ...
    */
    int16_t begin(int16_t freq = RADIOLIB_NRF24_DEFAULT_FREQ, int16_t dr = RADIOLIB_NRF24_DEFAULT_DR,
                  int8_t pwr = RADIOLIB_NRF24_DEFAULT_POWER, uint8_t addrWidth = RADIOLIB_NRF24_DEFAULT_ADDRWIDTH);
    ...
    */
    ...
    /*!
      \brief Dummy direct mode reception method, to ensure PhysicalLayer
      compatibility. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Interrupt-driven binary transmit method. IRQ will be activated when
      full packet is transmitted. Overloads for string-based transmissions are
      implemented in PhysicalLayer. \param data Binary data to be sent. \param len
      Number of bytes to send. \param addr Dummy address parameter, to ensure
    ...
    */
    ...
    /*!
      ...
      binary data. \param len Number of bytes that will be received. Must be known
      in advance for binary transmissions. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Sets carrier frequency. Allowed values range from 2400 MHz to 2525
    ...
    */
    ...
    /*!
      \brief Sets address width of transmit and receive pipes in bytes. Allowed
      values are 3, 4 or 5 bytes. \param addrWidth Address width to be set in
      bytes. \returns \ref status_codes
    */
    int16_t setAddressWidth(uint8_t addrWidth);

    /*!
      \brief Sets address of transmit pipe. The address width must be the same as
      the same as the configured in setAddressWidth. \param addr Address to which
      the next packet shall be transmitted. \returns \ref status_codes
    */
    int16_t setTransmitPipe(const uint8_t *addr);

    /*!
      \brief Sets address of receive pipes 0 or 1. The address width must be the
      same as the same as the configured in setAddressWidth. \param pipeNum Number
      of pipe to which the address shall be set. Either 0 or 1, other pipes are
      handled using overloaded method. \param addr Address from which %nRF24 shall
      receive new packets on the specified pipe. \returns \ref status_codes
    */
    int16_t setReceivePipe(uint8_t pipeNum, const uint8_t *addr);

    /*!
      \brief Sets address of receive pipes 2 - 5. The first 2 - 4 address bytes
      for these pipes are the same as for address pipe 1, only the last byte can
      be set. \param pipeNum Number of pipe to which the address shall be set.
      Allowed values range from 2 to 5. \param addrByte LSB of address from which
      %nRF24 shall receive new packets on the specified pipe. \returns \ref
      status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param freqDev Dummy frequency deviation parameter, no configuration will be
      changed. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Dummy data shaping configuration method, to ensure PhysicalLayer
      compatibility. \param sh Ignored. \returns \ref status_codes
    */
    int16_t setDataShaping(uint8_t sh) override;

    /*!
      \brief Dummy encoding configuration method, to ensure PhysicalLayer
      compatibility. \param encoding Ignored. \returns \ref status_codes
    */
...
protected:
    ...
    void SPItransfer(uint8_t cmd, bool write = false, const uint8_t *dataOut = NULL, uint8_t *dataIn = NULL,
                     uint8_t numBytes = 0);
