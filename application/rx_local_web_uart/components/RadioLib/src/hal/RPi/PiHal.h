...
public:
    ...
    PiHal(uint8_t spiChannel, uint32_t spiSpeed = 2000000, uint8_t spiDevice = 0, uint8_t gpioDevice = 0)
        : RadioLibHal(PI_INPUT, PI_OUTPUT, LG_LOW, LG_HIGH, PI_RISING, PI_FALLING), _gpioDevice(gpioDevice),
          _spiDevice(spiDevice), _spiSpeed(spiSpeed), _spiChannel(spiChannel)
    {
    }
    ...
    void attachInterrupt(uint32_t interruptNum, void (*interruptCb)(void), uint32_t mode) override
    {
        ...
        // lpgio reports the value of level after an interrupt, not the actual
        // direction
