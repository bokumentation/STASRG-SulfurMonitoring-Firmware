...
int16_t APRSClient::sendPosition(char *destCallsign, uint8_t destSSID, const char *lat, const char *lon,
                                 const char *msg, const char *time)
{
...
int16_t APRSClient::sendMicE(float lat, float lon, uint16_t heading, uint16_t speed, uint8_t type, const uint8_t *telem,
                             size_t telemLen, const char *grid, const char *status, int32_t alt)
{
    // sanity checks first
    ...
    // the following is based on APRS Mic-E implementation by
    // https://github.com/omegat as discussed in
    // https://github.com/jgromes/RadioLib/issues/430
...
int16_t APRSClient::sendFrame(char *destCallsign, uint8_t destSSID, char *info)
{
    ...
    if (this->axClient != nullptr) {
        ...
        AX25Frame frameUI(destCallsign, destSSID, srcCallsign, axClient->getSSID(),
                          RADIOLIB_AX25_CONTROL_UNNUMBERED_FRAME, RADIOLIB_AX25_PID_NO_LAYER_3,
        ...
        snprintf(buff, len, RADIOLIB_APRS_LORA_HEADER "%s-%d>%s,WIDE%d-%d:%s", this->src, this->id, destCallsign,
                 destSSID, destSSID, info);
