...
//                        - Morse code representation is saved LSb first, using
//                        additional bit as guard
//                        - position in array corresponds ASCII code minus
//                        RADIOLIB_MORSE_ASCII_OFFSET
//                        - ASCII characters marked RADIOLIB_MORSE_UNSUPPORTED
//                        do not have ITU-R M.1677-1 equivalent
...
/*!
  \class MorseClient
  \brief Client for Morse Code communication. The public interface is the same
  as Arduino Serial.
*/
...
public:
    ...
    */
    ...
    /*!
      ...
      \param base Base RF frequency to be used in MHz (in 2-FSK mode), or the tone
      frequency in Hz (in AFSK mode) \param speed Coding speed in words per
      minute. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param low Low threshold for decision limit (dot length, pause length etc.),
      defaults to 0.75. \param high High threshold for decision limit (dot length,
      pause length etc.), defaults to 1.25. \returns 0 if not enough symbols were
      decoded, 1 if inter-character space was detected, 2 if inter-word space was
      detected.
    */
#if !RADIOLIB_EXCLUDE_AFSK
    ...
    /*!
      \brief Write one byte. Implementation of interface of the
      RadioLibPrint/Print class. \param b Byte to write. \returns 1 if the byte
