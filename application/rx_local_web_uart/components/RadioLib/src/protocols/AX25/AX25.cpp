...
AX25Frame::AX25Frame(const char *destCallsign, uint8_t destSSID, const char *srcCallsign, uint8_t srcSSID,
                     uint8_t control)
...
AX25Frame::AX25Frame(const char *destCallsign, uint8_t destSSID, const char *srcCallsign, uint8_t srcSSID,
                     uint8_t control, uint8_t protocolID, const char *info)
    : AX25Frame(destCallsign, destSSID, srcCallsign, srcSSID, control, protocolID,
                reinterpret_cast<uint8_t *>(const_cast<char *>(info)), strlen(info))
{
}

AX25Frame::AX25Frame(const char *destCallsign, uint8_t destSSID, const char *srcCallsign, uint8_t srcSSID,
                     uint8_t control, uint8_t protocolID, const uint8_t *info, uint16_t infoLen)
{
...
AX25Frame::AX25Frame(const AX25Frame &frame)
    : destSSID(frame.destSSID), srcSSID(frame.srcSSID), numRepeaters(frame.numRepeaters), control(frame.control),
      protocolID(frame.protocolID), infoLen(frame.infoLen), rcvSeqNumber(frame.rcvSeqNumber),
...
#if !RADIOLIB_EXCLUDE_AFSK
AX25Client::AX25Client(AFSKClient *aud) : audio(aud)
{
...
AX25Client::AX25Client(const AX25Client &ax25)
    : phyLayer(ax25.phyLayer), sourceSSID(ax25.sourceSSID), preambleLen(ax25.preambleLen)
{
...
int16_t AX25Client::transmit(const char *str, const char *destCallsign, uint8_t destSSID)
{
    ...
    // build the frame
    AX25Frame frame(destCallsign, destSSID, sourceCallsign, sourceSSID, controlField, RADIOLIB_AX25_PID_NO_LAYER_3,
...
int16_t AX25Client::sendFrame(AX25Frame *frame)
{
    ...
    // calculate frame length without FCS (destination address, source address,
    // repeater addresses, control, PID, info)
    ...
    // set destination callsign - all address field bytes are shifted by one bit
    // to make room for HDLC address extension bit
    ...
    *(frameBuffPtr++) = RADIOLIB_AX25_SSID_RESPONSE_DEST | RADIOLIB_AX25_SSID_RESERVED_BITS |
                        (frame->destSSID & 0x0F) << 1 | RADIOLIB_AX25_SSID_HDLC_EXTENSION_CONTINUE;

    // set source callsign - all address field bytes are shifted by one bit to
    // make room for HDLC address extension bit
    memset(frameBuffPtr, ' ' << 1, RADIOLIB_AX25_MAX_CALLSIGN_LEN);
    ...
    // set source SSID
    *(frameBuffPtr++) = RADIOLIB_AX25_SSID_COMMAND_SOURCE | RADIOLIB_AX25_SSID_RESERVED_BITS |
                        (frame->srcSSID & 0x0F) << 1 | RADIOLIB_AX25_SSID_HDLC_EXTENSION_CONTINUE;
    ...
    for (uint16_t i = 0; i < frame->numRepeaters; i++) {
        ...
        frameBuffPtr += RADIOLIB_AX25_MAX_CALLSIGN_LEN;
        *(frameBuffPtr++) = RADIOLIB_AX25_SSID_HAS_NOT_BEEN_REPEATED | RADIOLIB_AX25_SSID_RESERVED_BITS |
                            (frame->repeaterSSIDs[i] & 0x0F) << 1 | RADIOLIB_AX25_SSID_HDLC_EXTENSION_CONTINUE;
    }
    ...
    *(frameBuffPtr++) = (uint8_t)(fcs & 0xFF);
    ...
    // prepare buffer for the final frame (stuffed, with added preamble + flags
    // and NRZI-encoded)
#if !RADIOLIB_STATIC_ONLY
    // worst-case scenario: sequence of 1s, will have 120% of the original length,
    // stuffed frame also includes both flags
    ...
    // set end flag field (may be split into two bytes due to misalignment caused
    // by extra stuffing bits)
    if (trailingLen != 0) {
        stuffedFrameBuffLen++;
