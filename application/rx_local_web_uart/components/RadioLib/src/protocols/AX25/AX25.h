...
#define RADIOLIB_AX25_SSID_HAS_NOT_BEEN_REPEATED                                                                       \
    0b00000000 //  7     7                 not repeated yet (set in repeater SSID)
...
#define RADIOLIB_AX25_SSID_HDLC_EXTENSION_CONTINUE                                                                     \
    0b00000000 //  0     0     HDLC extension bit: next octet contains more
//  address information
#define RADIOLIB_AX25_SSID_HDLC_EXTENSION_END 0b00000001 //  0     0                         address field end

// control field
#define RADIOLIB_AX25_CONTROL_U_SET_ASYNC_BAL_MODE                                                                     \
    0b01101100 //  7     2     U frame type: set asynchronous balanced mode
//  (connect request)
#define RADIOLIB_AX25_CONTROL_U_SET_ASYNC_BAL_MODE_EXT                                                                 \
    0b00101100 //  7     2                   set asynchronous balanced mode
//  extended (connect request with module 128)
#define RADIOLIB_AX25_CONTROL_U_DISCONNECT 0b01000000      //  7     2                   disconnect request
#define RADIOLIB_AX25_CONTROL_U_DISCONNECT_MODE 0b00001100 //  7     2                   disconnect mode (system busy or
//  disconnected)
...
#define RADIOLIB_AX25_CONTROL_S_RECEIVE_READY 0b00000000 //  3     2     S frame type: receive ready (system ready to
//  receive)
#define RADIOLIB_AX25_CONTROL_S_RECEIVE_NOT_READY                                                                      \
    0b00000100                                    //  3     2                   receive not ready (TNC buffer full)
#define RADIOLIB_AX25_CONTROL_S_REJECT 0b00001000 //  3     2                   reject (out of sequence or duplicate)
#define RADIOLIB_AX25_CONTROL_S_SELECTIVE_REJECT                                                                       \
    0b00001100 //  3     2                   selective reject (single frame repeat
//  request)
...
public:
    ...
    */
    ...
    /*!
      ...
      \param srcSSID SSID of the source station.
      \param control The control field.
      \param protocolID The protocol identifier (PID) field. Set to zero if the
      frame doesn't have this field. \param info Information field, in the form of
      null-terminated C-string.
    */
    AX25Frame(const char *destCallsign, uint8_t destSSID, const char *srcCallsign, uint8_t srcSSID, uint8_t control,
              uint8_t protocolID, const char *info);
    ...
    /*!
      ...
      \param protocolID The protocol identifier (PID) field. Set to zero if the
      frame doesn't have this field. \param info Information field, in the form of
      arbitrary binary buffer. \param infoLen Number of bytes in the information
      field.
    */
    AX25Frame(const char *destCallsign, uint8_t destSSID, const char *srcCallsign, uint8_t srcSSID, uint8_t control,
              uint8_t protocolID, const uint8_t *info, uint16_t infoLen);
    ...
    */
    ...
    /*!
      ...
      \param repeaterCallsigns Array of repeater callsigns in the form of
      null-terminated C-strings. \param repeaterSSIDs Array of repeater SSIDs.
...
public:
    /*!
      ...
      \param phy Pointer to the wireless module providing PhysicalLayer
      communication.
    */
    ...
    */
    ...
    /*!
      \brief Set AFSK tone correction offset. On some platforms, this is required
      to get the audio produced by the setup to match the expected 1200/2200 Hz
      tones. \param mark Positive or negative correction offset for mark audio
      frequency in Hz. \param space Positive or negative correction offset for
      space audio frequency in Hz. \param length Audio tone length modifier,
      defaults to 1.0. \returns \ref status_codes
    */
    ...
    /*!
      ...
      \param srcCallsign Callsign of the source station.
      \param srcSSID 4-bit SSID of the source station (in case there are more
      stations with the same callsign). Defaults to 0. \param preLen Number of
      "preamble" bytes (RADIOLIB_AX25_FLAG) sent ahead of the actual AX.25 frame.
      Does not include the first RADIOLIB_AX25_FLAG byte, which is considered part
      of the frame. Defaults to 8. \returns \ref status_codes
    */
    ...
    /*!
      ...
      \param destCallsign Callsign of the destination station.
      \param destSSID 4-bit SSID of the destination station (in case there are
      more stations with the same callsign). Defaults to 0. \returns \ref
      status_codes
    */
