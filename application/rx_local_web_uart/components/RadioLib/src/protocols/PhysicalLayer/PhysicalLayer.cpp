...
int16_t PhysicalLayer::startReceive(uint32_t timeout, RadioLibIrqFlags_t irqFlags, RadioLibIrqFlags_t irqMask,
                                    size_t len)
{
    RadioModeConfig_t cfg = {.receive = {
            .timeout = timeout,
            .irqFlags = irqFlags,
            .irqMask = irqMask,
            .len = len,
...
{
    RadioModeConfig_t cfg = {.transmit = {
            .data = data,
            .len = len,
            .addr = addr,
        }
...
int16_t PhysicalLayer::readData(String &str, size_t len)
{
    ...
    if ((len < length) && (len != 0)) {
        // user requested less bytes than were received, this is allowed (but
        // frowned upon) requests for more data than were received will only return
        // the number of actually received bytes (unlike PhysicalLayer::receive())
        length = len;
    }
...
int32_t PhysicalLayer::random(int32_t max)
{
    ...
    int32_t randNum = ((int32_t)randBuff[0] << 24) | ((int32_t)randBuff[1] << 16) | ((int32_t)randBuff[2] << 8) |
                      ((int32_t)randBuff[3]);
    if (randNum < 0) {
        randNum *= -1;
    }
