...
#define RADIOLIB_LORAWAN_FPORT_RESERVED (0xE0 << 0)    //  7     0     fPort values equal to and larger than this are
//  reserved
...
#define RADIOLIB_LORAWAN_JOIN_ACCEPT_CFLIST_TYPE_POS                                                                   \
    (RADIOLIB_LORAWAN_JOIN_ACCEPT_CFLIST_POS + RADIOLIB_LORAWAN_JOIN_ACCEPT_CFLIST_LEN - 1)
...
#define RADIOLIB_LORAWAN_MAC_COMMAND_NONE                                                                              \
    { .cid = 0, .lenDn = 0, .lenUp = 0, .persist = false, .user = false }
...
#define RADIOLIB_LORAWAN_PACKAGE_NONE                                                                                  \
    { .packId = 0, .packFPort = 0, .isAppPack = false, .fixedFPort = false, .enabled = false, .callback = NULL }
...
enum LoRaWANSchemeSession_t {
    ...
    RADIOLIB_LORAWAN_SESSION_SNWK_SINT_KEY =
        RADIOLIB_LORAWAN_SESSION_FNWK_SINT_KEY + RADIOLIB_AES128_KEY_SIZE,                                 // 16 bytes
    ...
    RADIOLIB_LORAWAN_SESSION_DL_CHANNELS =
        RADIOLIB_LORAWAN_SESSION_UL_CHANNELS + RADIOLIB_LORAWAN_MAX_NUM_DYNAMIC_CHANNELS * 5, // 16*4 bytes
    RADIOLIB_LORAWAN_SESSION_AVAILABLE_CHANNELS =
        RADIOLIB_LORAWAN_SESSION_DL_CHANNELS + RADIOLIB_LORAWAN_MAX_NUM_DYNAMIC_CHANNELS * 4, // 2 bytes
    RADIOLIB_LORAWAN_SESSION_MAC_QUEUE = RADIOLIB_LORAWAN_SESSION_AVAILABLE_CHANNELS +
                                         RADIOLIB_LORAWAN_MAX_NUM_SUBBANDS, // 12 bytes                   // 15
    // bytes
    RADIOLIB_LORAWAN_SESSION_MAC_QUEUE_LEN =
        RADIOLIB_LORAWAN_SESSION_MAC_QUEUE + RADIOLIB_LORAWAN_FHDR_FOPTS_MAX_LEN,                  // 1 byte
...
#define RADIOLIB_LORAWAN_CHANNEL_NONE                                                                                  \
    { .idx = 0, .freq = 0, .drMin = 0, .drMax = 0, .dr = RADIOLIB_LORAWAN_DATA_RATE_UNUSED }
...
struct LoRaWANChannelSpan_t {
    ...
    /*! \brief Center frequency of the first channel in span (coded in 100 Hz
     * steps) */
    uint32_t freqStart;
...
#define RADIOLIB_LORAWAN_CHANNEL_SPAN_NONE                                                                             \
    {                                                                                                                  \
        .numChannels = 0, .freqStart = 0, .freqStep = 0, .drMin = 0, .drMax = 0,                                       \
...
#define RADIOLIB_DATARATE_NONE                                                                                         \
    {                                                                                                                  \
        .modem = RADIOLIB_MODEM_NONE, .dr = {.lora = {0, 0, 0}}, .pc = {.lora = {8, 0, 0, 0} }                         \
    }
...
struct LoRaWANBand_t {
    ...
    /*! \brief Whether the channels are fixed per specification, or dynamically
     * allocated through the network (plus defaults) */
    uint8_t bandType;
    ...
    /*! \brief Array of allowed maximum application payload lengths for each data
     * rate (N-value) */
    ...
    /*! \brief Default uplink (TX) channel spans for fixed bands, including
    ...
    /*! \brief The corresponding datarates, bandwidths and coding rates for DR
     * index */
...
struct LoRaWANEvent_t {
    ...
    /*! \brief Whether the event is confirmed or not (e.g., confirmed uplink sent
     * by user application) */
    bool confirmed;
    ...
    /*! \brief The appropriate frame counter - for different events, different
     * frame counters will be reported! */
    uint32_t fCnt;
...
public:
    ...
    */
    ...
    /*!
      \brief Returns the pointer to the internal buffer that holds the LW base
      parameters \returns Pointer to uint8_t array of size
      RADIOLIB_LORAWAN_NONCES_BUF_SIZE
    */
    uint8_t *getBufferNonces();

    /*!
      \brief Fill the internal buffer that holds the LW base parameters with a
      supplied buffer \param persistentBuffer Buffer that should match the
      internal format (previously extracted using getBufferNonces) \returns \ref
      status_codes
    */
    int16_t setBufferNonces(const uint8_t *persistentBuffer);

    /*!
      \brief Clear an active session. This requires the device to rejoin the
      network.
    */
    void clearSession();

    /*!
      \brief Returns the pointer to the internal buffer that holds the LW session
      parameters \returns Pointer to uint8_t array of size
    ...
    */
    ...
    /*!
      \brief Fill the internal buffer that holds the LW session parameters with a
      supplied buffer \param persistentBuffer Buffer that should match the
      internal format (previously extracted using getBufferSession) \returns \ref
      status_codes
    */
    ...
    */
    ...
    /*!
      ...
      \param fNwkSIntKey Pointer to the Forwarding network session (LoRaWAN 1.1),
      NULL for LoRaWAN 1.0. \param sNwkSIntKey Pointer to the Serving network
      session (LoRaWAN 1.1), NULL for LoRaWAN 1.0. \param nwkSEncKey Pointer to
      the MAC command network session key [NwkSEncKey] (LoRaWAN 1.1) or network
      session AES-128 key [NwkSKey] (LoRaWAN 1.0). \param appSKey Pointer to the
      application session AES-128 key. \returns \ref status_codes
    */
    int16_t beginABP(uint32_t addr, const uint8_t *fNwkSIntKey, const uint8_t *sNwkSIntKey, const uint8_t *nwkSEncKey,
                     const uint8_t *appSKey);

    /*!
      \brief Join network by restoring OTAA session or performing over-the-air
      activation. By this procedure, the device will perform an exchange with the
      network server and set all necessary configuration. \returns \ref
      status_codes
    */
    virtual int16_t activateOTAA(LoRaWANJoinEvent_t *joinEvent = NULL);

    /*!
      \brief Join network by restoring ABP session or performing over-the-air
      activation. In this procedure, all necessary configuration must be provided
      by the user. \returns \ref status_codes
    */
    ...
    /*! \brief Configure class (RADIOLIB_LORAWAN_CLASS_A or
    ...
    /*!
      ...
      Default = 0 uses Rx2 frequency. \param mcDr The datarate used for the
      Multicast downlinks. Default = 0 uses Rx2 datarate. \returns \ref
      status_codes
    */
    ...
    /*!
      \brief Send a message to the server and wait for a downlink during Rx1
      and/or Rx2 window. \param strUp Address of Arduino String that will be
      transmitted. \param fPort Port number to send the message to. \param strDown
      Address of Arduino String to save the received data. \param isConfirmed
      Whether to send a confirmed uplink or not. \param eventUp Pointer to a
      structure to store extra information about the uplink event (fPort, frame
      counter, etc.). If set to NULL, no extra information will be passed to the
      user. \param eventDown Pointer to a structure to store extra information
      about the downlink event (fPort, frame counter, etc.). If set to NULL, no
      extra information will be passed to the user. \returns Window number > 0 if
      downlink was received, 0 is no downlink was received, otherwise \ref
      status_codes
    */
    virtual int16_t sendReceive(const String &strUp, uint8_t fPort, String &strDown, bool isConfirmed = false,
                                LoRaWANEvent_t *eventUp = NULL, LoRaWANEvent_t *eventDown = NULL);
#endif

    /*!
      \brief Send a message to the server and wait for a downlink during Rx1
      and/or Rx2 window. \param strUp C-string that will be transmitted. \param
      fPort Port number to send the message to. \param isConfirmed Whether to send
      a confirmed uplink or not. \param eventUp Pointer to a structure to store
      extra information about the uplink event (fPort, frame counter, etc.). If
      set to NULL, no extra information will be passed to the user. \param
      eventDown Pointer to a structure to store extra information about the
      downlink event (fPort, frame counter, etc.). If set to NULL, no extra
      information will be passed to the user. \returns Window number > 0 if
      downlink was received, 0 is no downlink was received, otherwise \ref
      status_codes
    */
    virtual int16_t sendReceive(const char *strUp, uint8_t fPort, bool isConfirmed = false,
                                LoRaWANEvent_t *eventUp = NULL, LoRaWANEvent_t *eventDown = NULL);

    /*!
      \brief Send a message to the server and wait for a downlink during Rx1
      and/or Rx2 window. \param strUp C-string that will be transmitted. \param
      fPort Port number to send the message to. \param dataDown Buffer to save
      received data into. \param lenDown Pointer to variable that will be used to
      save the number of received bytes. \param isConfirmed Whether to send a
      confirmed uplink or not. \param eventUp Pointer to a structure to store
      extra information about the uplink event (fPort, frame counter, etc.). If
      set to NULL, no extra information will be passed to the user. \param
      eventDown Pointer to a structure to store extra information about the
      downlink event (fPort, frame counter, etc.). If set to NULL, no extra
      ...
      status_codes
    */
    virtual int16_t sendReceive(const char *strUp, uint8_t fPort, uint8_t *dataDown, size_t *lenDown,
                                bool isConfirmed = false, LoRaWANEvent_t *eventUp = NULL,
                                LoRaWANEvent_t *eventDown = NULL);

    /*!
      \brief Send a message to the server and wait for a downlink but don't bother
      the user with downlink contents \param dataUp Data to send. \param lenUp
      Length of the data. \param fPort Port number to send the message to. \param
      isConfirmed Whether to send a confirmed uplink or not. \param eventUp
      Pointer to a structure to store extra information about the uplink event
      (fPort, frame counter, etc.). If set to NULL, no extra information will be
      passed to the user. \param eventDown Pointer to a structure to store extra
      information about the downlink event (fPort, frame counter, etc.). If set to
      NULL, no extra information will be passed to the user. \returns Window
      number > 0 if downlink was received, 0 is no downlink was received,
      otherwise \ref status_codes
    */
    virtual int16_t sendReceive(const uint8_t *dataUp, size_t lenUp, uint8_t fPort = 1, bool isConfirmed = false,
                                LoRaWANEvent_t *eventUp = NULL, LoRaWANEvent_t *eventDown = NULL);

    /*!
      \brief Send a message to the server and wait for a downlink during Rx1
      and/or Rx2 window. \param dataUp Data to send. \param lenUp Length of the
      data. \param fPort Port number to send the message to. \param dataDown
      Buffer to save received data into. \param lenDown Pointer to variable that
      will be used to save the number of received bytes. \param isConfirmed
      Whether to send a confirmed uplink or not. \param eventUp Pointer to a
      structure to store extra information about the uplink event (fPort, frame
      counter, etc.). If set to NULL, no extra information will be passed to the
      user. \param eventDown Pointer to a structure to store extra information
      ...
      status_codes
    */
    virtual int16_t sendReceive(const uint8_t *dataUp, size_t lenUp, uint8_t fPort, uint8_t *dataDown, size_t *lenDown,
                                bool isConfirmed = false, LoRaWANEvent_t *eventUp = NULL,
                                LoRaWANEvent_t *eventDown = NULL);
    ...
    /*!
      ...
      \param lenDown Pointer to variable that will be used to save the number of
      received bytes. \param eventDown Pointer to a structure to store extra
      information about the downlink event (fPort, frame counter, etc.). If set to
      NULL, no extra information will be passed to the user. \returns Window
      number > 0 if downlink was received, 0 is no downlink was received,
      otherwise \ref status_codes
    ...
    */
    ...
    /*!
      \brief Returns the quality of connectivity after requesting a LinkCheck MAC
      command. \param margin Link margin in dB of LinkCheckReq demodulation at
      gateway side. \param gwCnt Number of gateways that received the
      LinkCheckReq. \returns \ref status_codes
    */
    ...
    /*!
      ...
      This time is supposed to be <100ms accurate, but may be accurate to 1
      second. \param timestamp Number of seconds since GPS epoch (Jan. 6th 1980).
      \param milliseconds Milliseconds on top of the timestamp.
      \param returnUnix If true, returns Unix timestamp instead of GPS (default
      true). \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      This should not be needed for LoRaWAN 1.1 as it is configured through the
      first downlink. \param dr The datarate to be used for listening for
      downlinks in Rx2. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      \brief Set or disable uplink dwell time limitation; enabled by default if
      mandatory. \param enable Whether to adhere to dwellTime limits or not
      (default true). \param msPerUplink The maximum allowed Time-on-Air per
      uplink in milliseconds (default 0 = band default value); make sure you
      follow regulations/law!
    */
    ...
    /*!
      ...
      \param maxChanges Maximum number of channel hops if channel is used (default
      4). \param backoffMax Num of BO slots to be decremented after DIFS phase. 0
      to disable BO (default). \param difsSlots Num of CADs to estimate a clear CH
      (default 2).
    */
    ...
    /*!
      ...
      \param battLevel Battery level to set. 0 for external power source, 1 for
      lowest battery, 254 for highest battery, 255 for unable to measure.
    */
    void setDeviceStatus(uint8_t battLevel);

    /*!
      \brief Set the exact time a transmission should occur. Note: this is the
      internal clock time. On Arduino platforms, this is the usual time supplied
      by millis(). If the supplied time is larger than the current time,
      sendReceive() or uplink() will delay until the scheduled time. \param
    ...
    */
    ...
    /*!
      ...
      \returns (RadioLibTime_t) time-on-air (ToA) of last uplink message (in
      milliseconds).
    */
    ...
    /*!
      ...
      This interval is based on the ToA of one uplink and does not actually keep
      track of total airtime. \param msPerHour The maximum allowed duty cycle (in
      milliseconds per hour). \param airtime The airtime of the uplink. \returns
    ...
    */
    ...
    /*! \brief Returns time in milliseconds until next uplink is available under
     * dutyCycle limits */
    RadioLibTime_t timeUntilUplink();

    /*!
      \brief Returns the maximum allowed uplink payload size given the current MAC
      state. Most importantly, this includes dwell time limitations and ADR.
    */
    ...
    /*!
      \brief Set custom delay/sleep function callback. If set, LoRaWAN node will
      call this function to wait for periods of time longer than
      RADIOLIB_LORAWAN_DELAY_SLEEP_THRESHOLD. This can be used to lower the power
      consumption by putting the host microcontroller to sleep. NOTE: Since this
      method will call a user-provided function, it is up to the user to ensure
      that the time duration spent in that sleep function is accurate to at least
      1 ms!
    */
    void setSleepFunction(SleepCb_t cb);

    /*!
      \brief Add a LoRaWAN Application Package as defined in one of the TSxxx
      documents. Any downlinks that occur on the corresponding FPort will be
      redirected to a supplied callback that implements this package. These
      downlink contents will be hidden from the user as the downlink buffer will
      be empty and the length zero. The package may need to overrule the behaviour
      of your device - refer to the examples. Advanced users only! \param
      packageId The ID of the package (one of RADIOLIB_LORAWAN_PACKAGE_TSxxx).
      \param callback The downlink handler for this package of type (uint8_t*
      dataDown, size_t lenDown). \returns \ref status_codes
    */
    ...
    /*!
      ...
      packageId The ID of the package (one of RADIOLIB_LORAWAN_PACKAGE_TSxxx).
      \param callback The downlink handler for this package of type (uint8_t*
      dataDown, size_t lenDown). \param fPort A custom FPort for packages that
      have a default FPort < 224. \returns \ref status_codes
    */
    ...
    */
    ...
    /*!
      ...
      according to the spec, the Rx window must be at least enough time to
      effectively detect a preamble but we pad it a bit on both sides (start and
      end) to make sure it is wide enough The larger this number the more power
      will be consumed! So be careful about changing it. For debugging purposes 50
      is a reasonable start, but for production devices it should be as low as
      possible. 0 is a valid time.

      500 is the **maximum** value, but it is not a good idea to go anywhere near
      that. If you have to go above 50 you probably have a bug somewhere. Check
      your device timing.
    */
...
protected:
    ...
    // a buffer that holds all LW base parameters that should persist at all
    // times!
    uint8_t bufferNonces[RADIOLIB_LORAWAN_NONCES_BUF_SIZE] = {0};

    // a buffer that holds all LW session parameters that preferably persist, but
    // can be afforded to get lost
    ...
    // duty cycle is set upon initialization and activated in regions that impose
    // this
    bool dutyCycleEnabled = false;
    uint32_t dutyCycle = 0;

    // dwell time is set upon initialization and activated in regions that impose
    // this
    ...
    // A random BO avoids collisions in the case where two or more nodes start the
    // CSMA process at the same time.
    ...
    RadioLibTime_t rxDelays[4] = {0, RADIOLIB_LORAWAN_RECEIVE_DELAY_1_MS, RADIOLIB_LORAWAN_RECEIVE_DELAY_2_MS, 0};
    ...
    // create an encrypted uplink buffer, composing metadata, user data and MAC
    // data
    ...
    // handle one of the Class A receive windows with a given channel and certain
    // timestamps
    int16_t receiveClassA(uint8_t dir, const LoRaWANChannel_t *dlChannel, uint8_t window, const RadioLibTime_t dlDelay,
                          RadioLibTime_t tReference);

    // handle a Class C receive window with timeout (between Class A windows) or
    // without (between uplinks)
    ...
    // execute mac command, return the number of processed bytes for sequential
    // processing
    ...
    // possible override for additional MAC commands that are not in the base
    // specification
    virtual bool derivedMacHandler(uint8_t cid, uint8_t *optIn, uint8_t lenIn, uint8_t *optOut);

    // pre-process a (set of) LinkAdrReq commands into one super-channel-mask +
    // Tx/Dr/NbTrans fields
    ...
    // in downlink direction, the user-accessible MAC commands remain available
    // for retrieval
    ...
    // perform a single CAD operation for the under SF/CH combination. Returns
    // either busy or otherwise.
    ...
    void processAES(const uint8_t *in, size_t len, uint8_t *key, uint8_t *out, uint32_t addr, uint32_t fCnt,
    ...
    // 16-bit checksum method that takes a uint8_t array of even length and
    // calculates the checksum
    static uint16_t checkSum16(const uint8_t *key, uint16_t keyLen);

    // check the integrity of a buffer using a 16-bit checksum located in the last
    // two bytes of the buffer
    static int16_t checkBufferCommon(const uint8_t *buffer, uint16_t size);

    // network-to-host conversion method - takes data from network packet and
    // converts it to the host endians
    template <typename T> static T ntoh(const uint8_t *buff, size_t size = 0);

    // host-to-network conversion method - takes data from host variable and and
    // converts it to network packet endians
    template <typename T> static void hton(uint8_t *buff, T val, size_t size = 0);
};

template <typename T> T LoRaWANNode::ntoh(const uint8_t *buff, size_t size)
{
...
template <typename T> void LoRaWANNode::hton(uint8_t *buff, T val, size_t size)
{
