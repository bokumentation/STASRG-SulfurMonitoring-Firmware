...
int16_t LoRaWANNode::sendReceive(const String &strUp, uint8_t fPort, String &strDown, bool isConfirmed,
                                 LoRaWANEvent_t *eventUp, LoRaWANEvent_t *eventDown)
{
    ...
    state = this->sendReceive(reinterpret_cast<const uint8_t *>(dataUp), strlen(dataUp), fPort, dataDown, &lenDown,
                              isConfirmed, eventUp, eventDown);
...
int16_t LoRaWANNode::sendReceive(const char *strUp, uint8_t fPort, bool isConfirmed, LoRaWANEvent_t *eventUp,
                                 LoRaWANEvent_t *eventDown)
{
    ...
    return (this->sendReceive(reinterpret_cast<uint8_t *>(const_cast<char *>(strUp)), strlen(strUp), fPort, dataDown,
                              &lenDown, isConfirmed, eventUp, eventDown));
}

int16_t LoRaWANNode::sendReceive(const char *strUp, uint8_t fPort, uint8_t *dataDown, size_t *lenDown, bool isConfirmed,
                                 LoRaWANEvent_t *eventUp, LoRaWANEvent_t *eventDown)
{
    return (this->sendReceive(reinterpret_cast<uint8_t *>(const_cast<char *>(strUp)), strlen(strUp), fPort, dataDown,
                              lenDown, isConfirmed, eventUp, eventDown));
...
int16_t LoRaWANNode::sendReceive(const uint8_t *dataUp, size_t lenUp, uint8_t fPort, bool isConfirmed,
                                 LoRaWANEvent_t *eventUp, LoRaWANEvent_t *eventDown)
{
    // build a temporary buffer
    // LoRaWAN downlinks can have 250 bytes at most with 1 extra byte for NULL
...
int16_t LoRaWANNode::sendReceive(const uint8_t *dataUp, size_t lenUp, uint8_t fPort, uint8_t *dataDown, size_t *lenDown,
                                 bool isConfirmed, LoRaWANEvent_t *eventUp, LoRaWANEvent_t *eventDown)
{
    ...
    // if after (at) ADR_ACK_LIMIT frames no RekeyConf was received, revert to
    // Join state
    if (this->fCntUp == (1UL << this->adrLimitExp)) {
        state = this->getMacPayload(RADIOLIB_LORAWAN_MAC_REKEY, this->fOptsUp, this->fOptsUpLen, NULL,
    ...
    // if the payload consists of piggybacked MAC only, move this to the
    // FRMPayload
    ...
    for (; trans < this->nbTrans; trans++) {
        ...
        do {
            ...
            // if CSMA is enabled, repeat channel selection & encryption up to numHops
            // times
        ...
        state = this->transmitUplink(&this->channels[RADIOLIB_LORAWAN_UPLINK],
                                     ...
                                     (uint8_t)(uplinkMsgLen - RADIOLIB_LORAWAN_FHDR_LEN_START_OFFS), trans > 0);
        if (state != RADIOLIB_ERR_NONE) {
            // sometimes, a spurious error can occur even though the uplink was
            // transmitted therefore, just to be safe, increase frame counter by one
            // for the next uplink
            this->fCntUp += 1;
        ...
        // When an end-device has requested an ACK from the Network but has not yet
        // received it, it SHALL wait RETRANSMIT_TIMEOUT seconds after
        // RECEIVE_DELAY2 seconds have elapsed after the end of the previous uplink
        // transmission before sending a new uplink (repetition or new frame). The
        // RETRANSMIT_TIMEOUT delay is not required between unconfirmed uplinks, or
        // after the ACK has been successfully demodulated by the end-device.
        if (isConfirmed) {
    ...
    // note: if an error occurred, it may still be the case that a transmission
    // occurred therefore, we act as if a transmission occurred before throwing
    // the actual error this feels to be the best way to comply to spec
    ...
    if (rxWindow == 0) {
        ...
        // remove only non-persistent MAC commands, the other commands should be
        // re-sent until downlink is received
...
uint8_t *LoRaWANNode::getBufferNonces()
{
    ...
    LoRaWANNode::hton<uint16_t>(&this->bufferNonces[RADIOLIB_LORAWAN_NONCES_VERSION],
                                RADIOLIB_LORAWAN_NONCES_VERSION_VAL);
    ...
    // generate the signature of the Nonces buffer, and store it in the last two
    // bytes of the Nonces buffer
...
int16_t LoRaWANNode::setBufferNonces(const uint8_t *persistentBuffer)
{
    ...
    // uint8_t nvm_table_version =
    // this->bufferNonces[RADIOLIB_LORAWAN_NONCES_VERSION]; if
    // (RADIOLIB_LORAWAN_NONCES_VERSION_VAL > nvm_table_version) {
    ...
    bool isSameKeys =
        LoRaWANNode::ntoh<uint16_t>(&persistentBuffer[RADIOLIB_LORAWAN_NONCES_CHECKSUM]) == this->keyCheckSum;
    bool isSameMode = LoRaWANNode::ntoh<uint16_t>(&persistentBuffer[RADIOLIB_LORAWAN_NONCES_MODE]) == this->lwMode;
    bool isSamePlan =
        LoRaWANNode::ntoh<uint8_t>(&persistentBuffer[RADIOLIB_LORAWAN_NONCES_PLAN]) == this->band->bandNum;

    // check if Nonces buffer matches the current configuration
    if (!isSameKeys || !isSameMode || !isSamePlan) {
        // if configuration did not match, discard whatever is currently in the
        // buffers and start fresh
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("Configuration mismatch (keys: %d, mode: %d, plan: %d)", isSameKeys, isSameMode,
                                        isSamePlan);
...
void LoRaWANNode::createSession()
{
    ...
    case (16):
        maxEIRPRaw = 5;
        break;
    case (19): // this option does not exist for the TxParamSetupReq but will be
...
uint8_t *LoRaWANNode::getBufferSession()
{
    ...
    memcpy(&this->bufferSession[RADIOLIB_LORAWAN_SESSION_MAC_QUEUE], this->fOptsUp,
           RADIOLIB_LORAWAN_FHDR_FOPTS_MAX_LEN);
    ...
    memcpy(&this->bufferSession[RADIOLIB_LORAWAN_SESSION_AVAILABLE_CHANNELS], this->channelFlags,
           sizeof(this->channelFlags));

    // store the session status
    LoRaWANNode::hton<uint8_t>(&this->bufferSession[RADIOLIB_LORAWAN_SESSION_STATUS], this->sessionStatus);

    // generate the signature of the Session buffer, and store it in the last two
    // bytes of the Session buffer
...
int16_t LoRaWANNode::setBufferSession(const uint8_t *persistentBuffer)
{
    ...
    // the Nonces buffer holds a checksum signature - compare this to the
    // signature that is in the session buffer
    uint16_t signatureNonces = LoRaWANNode::ntoh<uint16_t>(&this->bufferNonces[RADIOLIB_LORAWAN_NONCES_SIGNATURE]);
    uint16_t signatureInSession =
        LoRaWANNode::ntoh<uint16_t>(&persistentBuffer[RADIOLIB_LORAWAN_SESSION_NONCES_SIGNATURE]);
    ...
    // always restore the channels, so as to adhere to channel hopping between
    // JoinRequests
    memcpy(this->channelFlags, &this->bufferSession[RADIOLIB_LORAWAN_SESSION_AVAILABLE_CHANNELS],
           RADIOLIB_LORAWAN_MAX_NUM_SUBBANDS);
    ...
    const uint8_t cids[6] = {RADIOLIB_LORAWAN_MAC_DUTY_CYCLE,      RADIOLIB_LORAWAN_MAC_RX_PARAM_SETUP,
    ...
    const uint16_t locs[6] = {RADIOLIB_LORAWAN_SESSION_DUTY_CYCLE,      RADIOLIB_LORAWAN_SESSION_RX_PARAM_SETUP,
    ...
    memcpy(this->fOptsUp, &this->bufferSession[RADIOLIB_LORAWAN_SESSION_MAC_QUEUE],
           RADIOLIB_LORAWAN_FHDR_FOPTS_MAX_LEN);
...
int16_t LoRaWANNode::beginABP(uint32_t addr, const uint8_t *fNwkSIntKey, const uint8_t *sNwkSIntKey,
                              const uint8_t *nwkSEncKey, const uint8_t *appSKey)
{
    ...
    if (fNwkSIntKey) {
        this->keyCheckSum ^= LoRaWANNode::checkSum16(fNwkSIntKey, RADIOLIB_AES128_KEY_SIZE);
    }
    if (sNwkSIntKey) {
        this->keyCheckSum ^= LoRaWANNode::checkSum16(sNwkSIntKey, RADIOLIB_AES128_KEY_SIZE);
    }
...
int16_t LoRaWANNode::processJoinAccept(LoRaWANJoinEvent_t *joinEvent)
{
    ...
    if ((lenRx != RADIOLIB_LORAWAN_JOIN_ACCEPT_MAX_LEN) &&
            (lenRx != RADIOLIB_LORAWAN_JOIN_ACCEPT_MAX_LEN - RADIOLIB_LORAWAN_JOIN_ACCEPT_CFLIST_LEN)) {
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("JoinAccept reply length mismatch, expected %dB got %luB",
                                        RADIOLIB_LORAWAN_JOIN_ACCEPT_MAX_LEN, (unsigned long)lenRx);
    ...
    if ((joinAcceptMsgEnc[0] & RADIOLIB_LORAWAN_MHDR_MTYPE_MASK) != RADIOLIB_LORAWAN_MHDR_MTYPE_JOIN_ACCEPT) {
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("JoinAccept reply message type invalid, expected 0x%02x got 0x%02x",
                                        RADIOLIB_LORAWAN_MHDR_MTYPE_JOIN_ACCEPT, joinAcceptMsgEnc[0]);
        return (RADIOLIB_ERR_DOWNLINK_MALFORMED);
    }
    ...
    RadioLibAES128Instance.encryptECB(&joinAcceptMsgEnc[1], RADIOLIB_LORAWAN_JOIN_ACCEPT_MAX_LEN - 1,
                                      &joinAcceptMsg[1]);
    ...
    RADIOLIB_DEBUG_PROTOCOL_PRINTLN("JoinAccept (JoinNonce = %lu, previously %lu):", (unsigned long)joinNonceNew,
                                    (unsigned long)this->joinNonce);
    RADIOLIB_DEBUG_PROTOCOL_HEXDUMP(joinAcceptMsg, lenRx);

    if (this->rev == 1) {
        // for v1.1, the JoinNonce received must be greater than the last joinNonce
        // heard, else error
        if ((this->joinNonce > 0) && (joinNonceNew <= this->joinNonce)) {
            return (RADIOLIB_ERR_JOIN_NONCE_INVALID);
        }
    } else {
        // for v1.0.4, the JoinNonce is simply a non-repeating value (we only check
        // the last value)
        if ((this->joinNonce > 0) && (joinNonceNew == this->joinNonce)) {
            return (RADIOLIB_ERR_JOIN_NONCE_INVALID);
        }
    ...
    if (lenRx == RADIOLIB_LORAWAN_JOIN_ACCEPT_MAX_LEN &&
            joinAcceptMsg[RADIOLIB_LORAWAN_JOIN_ACCEPT_CFLIST_TYPE_POS] == this->band->bandType) {
        this->processCFList(&joinAcceptMsg[RADIOLIB_LORAWAN_JOIN_ACCEPT_CFLIST_POS]);
    }
    // if no (valid) CFList was received, default or subband are already setup so
    // don't need to do anything else

    uint8_t keyDerivationBuff[RADIOLIB_AES128_BLOCK_SIZE] = {0};
    LoRaWANNode::hton<uint32_t>(&keyDerivationBuff[RADIOLIB_LORAWAN_JOIN_ACCEPT_AES_JOIN_NONCE_POS], this->joinNonce,
                                3);
    ...
    if (this->rev == 1) {
        ...
        LoRaWANNode::hton<uint16_t>(&keyDerivationBuff[RADIOLIB_LORAWAN_JOIN_ACCEPT_AES_DEV_NONCE_POS],
                                    this->devNonce - 1);
        ...
        LoRaWANNode::hton<uint32_t>(&keyDerivationBuff[RADIOLIB_LORAWAN_JOIN_ACCEPT_HOME_NET_ID_POS], this->homeNetId,
                                    3);
...
int16_t LoRaWANNode::activateOTAA(LoRaWANJoinEvent_t *joinEvent)
{
    ...
    state = this->transmitUplink(&this->channels[RADIOLIB_LORAWAN_UPLINK], joinRequestMsg,
    ...
    // generate the signature of the Nonces buffer, and store it in the last two
    // bytes of the Nonces buffer also store this signature in the Session buffer
    // to make sure these buffers match
    uint16_t signature = LoRaWANNode::checkSum16(this->bufferNonces, RADIOLIB_LORAWAN_NONCES_BUF_SIZE - 2);
    LoRaWANNode::hton<uint16_t>(&this->bufferNonces[RADIOLIB_LORAWAN_NONCES_SIGNATURE], signature);
    LoRaWANNode::hton<uint16_t>(&this->bufferSession[RADIOLIB_LORAWAN_SESSION_NONCES_SIGNATURE], signature);

    // configure Rx1 and Rx2 delay for JoinAccept message - these are
    // re-configured once a valid JoinAccept is received
...
int16_t LoRaWANNode::activateABP()
{
    ...
    if (this->rev == 1) {
        LoRaWANNode::pushMacCommand(RADIOLIB_LORAWAN_MAC_RESET, &this->rev, this->fOptsUp, &this->fOptsUpLen,
                                    RADIOLIB_LORAWAN_UPLINK);
    }
...
void LoRaWANNode::processCFList(const uint8_t *cfList)
{
    ...
    if (this->band->bandType == RADIOLIB_LORAWAN_BAND_DYNAMIC) {
        ...
        for (uint8_t i = 0; i < 5; i++, num++) {
            // if the frequency fields are all zero, there are no more channels in the
            // CFList
...
int16_t LoRaWANNode::setClass(uint8_t cls)
{
    ...
    uint8_t cOct = cls;
    int16_t state = LoRaWANNode::pushMacCommand(RADIOLIB_LORAWAN_MAC_DEVICE_MODE, &cOct, this->fOptsUp,
                                                &this->fOptsUpLen, RADIOLIB_LORAWAN_UPLINK);
    return (state);
}

int16_t LoRaWANNode::startMulticastSession(uint8_t cls, uint32_t mcAddr, const uint8_t *mcAppSKey,
                                           const uint8_t *mcNwkSKey, uint32_t mcFCntMin, uint32_t mcFCntMax,
                                           uint32_t mcFreq, uint8_t mcDr)
{
...
void LoRaWANNode::adrBackoff()
{
    ...
    // as there is nothing more to do, set ADR counter to maximum value to
    // indicate that we've tried everything
...
void LoRaWANNode::composeUplink(const uint8_t *in, uint8_t lenIn, uint8_t *out, uint8_t fPort, bool isConfirmed)
{
    ...
    // if the ConfirmFCnt is set, there is a downlink to acknowledge, so set the
    // ACK bit
    ...
    if (this->fOptsUpLen > 0) {
        ...
        if (this->rev == 1) {
            ...
            processAES(this->fOptsUp, this->fOptsUpLen, this->nwkSEncKey, &out[RADIOLIB_LORAWAN_FHDR_FOPTS_POS],
                       this->devAddr, this->fCntUp, RADIOLIB_LORAWAN_UPLINK, 0x01, true);
        } else {
    ...
    processAES(in, lenIn, encKey, &out[RADIOLIB_LORAWAN_FRAME_PAYLOAD_POS(this->fOptsUpLen)], this->devAddr,
               this->fCntUp, RADIOLIB_LORAWAN_UPLINK, 0x00, true);
}
...
void LoRaWANNode::micUplink(uint8_t *inOut, size_t lenInOut)
{
    ...
    if (this->rev == 1) {
        uint32_t mic = ((uint32_t)(micF & 0x0000FF00) << 16) | ((uint32_t)(micF & 0x0000000FF) << 16) |
                       ((uint32_t)(micS & 0x0000FF00) >> 0) | ((uint32_t)(micS & 0x0000000FF) >> 0);
...
int16_t LoRaWANNode::transmitUplink(const LoRaWANChannel_t *chnl, uint8_t *in, uint8_t len, bool retrans)
{
    ...
    // if dutycycle is enabled and the time since last uplink + interval has not
    // elapsed, return an error but: don't check this for retransmissions
    ...
    if (this->dwellTimeUp) {
        if (toa > this->dwellTimeUp) {
            RADIOLIB_DEBUG_PROTOCOL_PRINTLN("Dwell time exceeded: ToA = %lu, max = %d", (unsigned long)toa,
                                            this->dwellTimeUp);
    ...
    state = this->setPhyProperties(chnl, RADIOLIB_LORAWAN_UPLINK, this->txPowerMax - 2 * this->txPowerSteps);
    RADIOLIB_ASSERT(state);
    ...
    if (this->tUplink > tNow + this->launchDuration) {
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("Delaying transmission by %lu ms",
                                        (unsigned long)(this->tUplink - tNow - this->launchDuration));
        tNow = mod->hal->millis();
        if (this->tUplink > tNow + this->launchDuration) {
    ...
    // start transmission, and time the duration of launchMode() to offset window
    // timing
...
// flag to indicate whether there was some action during Rx mode (timeout or
// downlink)
...
int16_t LoRaWANNode::receiveClassA(uint8_t dir, const LoRaWANChannel_t *dlChannel, uint8_t window,
                                   const RadioLibTime_t dlDelay, RadioLibTime_t tReference)
{
    Module *mod = this->phyLayer->getMod();
    ...
    if (dlDelay > 0 && tReference > 0) {
        ...
        // - the launch of Rx window takes a few milliseconds, so shorten the
        // waitLen a bit (launchDuration)
        // - the Rx window is padded using scanGuard, so shorten the waitLen a bit
        // (scanGuard / 2)
    ...
    // use a small additional delay in case the RxTimeout interrupt is slow to
    // fire
...
{
    // only open RxC if the device is Unicast-C or Multicast-C, otherwise ignore
    // without error
    ...
    if (timeout) {
        ...
        // Any frame received by an end-device containing a MACPayload greater than
        // the specified maximum length M over the data rate used to receive the
...
int16_t LoRaWANNode::receiveDownlink()
{
    ...
    RadioLibTime_t timeoutClassC =
        this->tUplinkEnd + this->rxDelays[RADIOLIB_LORAWAN_RX1] - mod->hal->millis() - 5 * this->scanGuard;
    int16_t state = this->receiveClassC(timeoutClassC);
    RADIOLIB_ASSERT(state);

    // open Rx1 window
    state = this->receiveClassA(RADIOLIB_LORAWAN_DOWNLINK, &this->channels[RADIOLIB_LORAWAN_RX1], RADIOLIB_LORAWAN_RX1,
                                this->rxDelays[RADIOLIB_LORAWAN_RX1], this->tUplinkEnd);
    RADIOLIB_ASSERT(state);

    // for LoRaWAN v1.1 Class C, there is no Rx2 window: it keeps RxC open
    // uninterrupted
    ...
    timeoutClassC = this->tUplinkEnd + this->rxDelays[RADIOLIB_LORAWAN_RX2] - mod->hal->millis() - 5 * this->scanGuard;
    state = this->receiveClassC(timeoutClassC);
    RADIOLIB_ASSERT(state);

    // open Rx2 window
    state = this->receiveClassA(RADIOLIB_LORAWAN_DOWNLINK, &this->channels[RADIOLIB_LORAWAN_RX2], RADIOLIB_LORAWAN_RX2,
                                this->rxDelays[RADIOLIB_LORAWAN_RX2], this->tUplinkEnd);
    RADIOLIB_ASSERT(state);
...
int16_t LoRaWANNode::parseDownlink(uint8_t *data, size_t *len, uint8_t window, LoRaWANEvent_t *event)
{
    ...
    *len = 0;
    ...
    // in LoRaWAN v1.1, a frame is a Network frame if there is no Application
    // payload i.e.: either no payload at all (empty frame or FOpts only), or MAC
    // only payload
    ...
    if (payLen > 0) {
        ...
        // LoRaWAN v1.0.4 only: A Class B/C downlink SHALL NOT transport any MAC
        // command.
        // (...) it SHALL silently discard the entire frame.
        // However, we also enforce this for LoRaWAN v1.1 (TTS does not allow this
        // anyway).
        if (fPort == RADIOLIB_LORAWAN_FPORT_MAC_COMMAND) {
    ...
    if (fPort == RADIOLIB_LORAWAN_FPORT_MAC_COMMAND && payLen > 0) {
        if (fOptsLen > 0) {
            // MAC commands SHALL NOT be present in the payload field and the frame
            // options field simultaneously. Should this occur, the end-device SHALL
    ...
    // (...) it SHALL silently discard the entire frame.
    // However, we also enforce this for LoRaWAN v1.1 (TTS does not allow this
    // anyway). Note: we check Device Class == A because Relay also uses a third
    // Rx window
    ...
    // (MAX_FCNT_GAP is deprecated for 1.0.4 / 1.1, TTS and CS both apply a 16-bit
    // rollover)
    ...
    // check if the ACK bit is set, indicating this frame acknowledges the
    // previous uplink
    ...
    // if this downlink is confirming an uplink, the MIC was generated with the
    // least-significant 16 bits of that fCntUp (LoRaWAN v1.1 only)
    ...
    // check the MIC
    // (if a rollover was more than 16-bit, this will always result in MIC
    // mismatch)
    ...
    // do some housekeeping for normal Class A downlinks (not allowed for Class
    // B/C)
    if (window < RADIOLIB_LORAWAN_RX_BC) {
        // if this is a confirmed frame, save the downlink number (only app frames
        // can be confirmed)
        ...
        // a Class A downlink was received, so restart the ADR counter with the next
        // uplink
    ...
    if (fOptsLen > 0 && isPiggyBacking) {
        ...
        // in LoRaWAN v1.1, the piggy-backed FOpts are encrypted using the
        // NwkSEncKey
        if (this->rev == 1) {
            uint8_t ctrId = 0x01 + isAppDownlink; // see LoRaWAN v1.1 errata
            processAES(fOptsPtr, (size_t)fOptsLen, this->nwkSEncKey, fOptsPtr, this->devAddr, devFCnt32,
                       RADIOLIB_LORAWAN_DOWNLINK, ctrId, true);
        }
        ...
        processAES(fOptsPtr, (size_t)fOptsLen, this->nwkSEncKey, fOptsPtr, this->devAddr, devFCnt32,
                   RADIOLIB_LORAWAN_DOWNLINK, 0x00, true);
    }
    ...
    // decrypt the frame payload (in-place to allow a fully decrypted hex-dump
    // next)
    ...
    if (fOptsLen > 0) {
        ...
        while (procLen < fOptsLen) {
            ...
            if (procLen + fLen > fOptsLen) {
                RADIOLIB_DEBUG_PROTOCOL_PRINTLN("WARNING: Incomplete MAC command %02x (%d bytes, expected %d)", cid,
                                                fOptsLen - procLen, fLen);
                RADIOLIB_DEBUG_PROTOCOL_PRINTLN("WARNING: Skipping remaining MAC payload");
                fOptsLen = procLen; // truncate to last processed MAC command
                break;
            }
            ...
            // if this is a LinkAdr MAC command, pre-process contiguous commands into
            // one atomic block
            if (cid == RADIOLIB_LORAWAN_MAC_LINK_ADR) {
                // if there was any LinkAdr command before, set NACK and continue
                // without processing
                if (mAdr) {
                    reply = true;
                    ...
                    // pre-process them into a single complete channel mask (stored in
                    // mAdrOpt)
                    ...
                    // in LoRaWAN v1.0.x, all ACK bytes should have equal status - fix in
                    // post-processing
                    if (this->rev == 0) {
                        LoRaWANNode::postprocessMacLinkAdr(&fOptsRe[fOptsReLen], fLen);

                        // in LoRaWAN v1.1, just provide one ACK, so no post-processing but
                        // cut off reply length
                    } else {
                        fLenRe = 2;
                    }
                }
        ...
        // remove all MAC commands except those whose payload can be requested by
        // the user (which are LinkCheck and DeviceTime)
        ...
        // if fOptsLen for the next uplink is larger than can be piggybacked onto an
        // uplink, send separate uplink
...
{
    // only allow if the device is Unicast-C or Multicast-C, otherwise ignore
    // without error
    if (this->lwClass != RADIOLIB_LORAWAN_CLASS_C && this->multicast != RADIOLIB_LORAWAN_CLASS_C) {
        return (RADIOLIB_ERR_NONE);
    }
    ...
    if (downlinkAction) {
        ...
        if (state == RADIOLIB_ERR_NONE) {
            ...
            // otherwise, if device is acting as Multicast on top of the same Unicast
            // class, try decrypting it as a Unicast downlink by temporarily disabling
            // Multicast
...
bool LoRaWANNode::execMacCommand(uint8_t cid, uint8_t *optIn, uint8_t lenIn, uint8_t *optOut)
{
    ...
    case (RADIOLIB_LORAWAN_MAC_RESET): {
        ...
        if (srvVersion != this->rev) {
            ...
            RADIOLIB_DEBUG_PROTOCOL_PRINTLN("ERROR! Please disable your device and "
                                            "consult supported LoRaWAN versions");
            (void)LoRaWANNode::pushMacCommand(cid, &this->rev, this->fOptsUp, &this->fOptsUpLen,
                                              RADIOLIB_LORAWAN_UPLINK);
        }
        return (false);
    } break;
    ...
    case (RADIOLIB_LORAWAN_MAC_LINK_ADR): {
        ...
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("LinkAdrReq: dataRate = %d, txSteps = %d, nbTrans = %d", macDrUp, macTxSteps,
                                        lenIn > 1 ? optIn[13] : 0);
        ...
        // only apply channel mask if present (internal Dr/Tx commands do not set
        // channel mask)
        chMaskAck = true;
        if (lenIn > 1) {
        ...
        if (this->band->dataRates[macDrUp].modem != RADIOLIB_MODEM_NONE) {
            ...
            state =
                this->phyLayer->checkDataRate(this->band->dataRates[macDrUp].dr, this->band->dataRates[macDrUp].modem);
            ...
            if (state == RADIOLIB_ERR_NONE) {
                ...
                RADIOLIB_DEBUG_PROTOCOL_PRINTLN("ADR: hardware failure configurating datarate %d, code %d", macDrUp,
                                                state);
            }
        }
        ...
        // only allow TxPower if less than / equal to the maximum number of defined
        // steps
        if (macTxSteps <= this->band->powerNumSteps) {
            ...
            // only acknowledge if the radio is able to operate at or below the
        ...
        if (lenIn > 1) {
            ...
            if (macNbTrans) {
                ...
                // for LoRaWAN v1.0.4, if NbTrans == 0, the end-device SHALL use the
                // default value (being 1)
                if (this->rev == 0) {
                    this->nbTrans = 1;
                }
                // for LoRaWAN v1.1, if NbTrans == 0, the end-device SHALL keep the
                // current NbTrans value unchanged so, don't do anything
            }
        }

        // save to the ADR MAC location
        // but first re-set the Dr/Tx/NbTrans field to make sure they're not set to
        // 0xF
    ...
    case (RADIOLIB_LORAWAN_MAC_DUTY_CYCLE): {
        ...
        if (maxDutyCycle == 0) {
            ...
            this->dutyCycle =
                (RadioLibTime_t)60 * (RadioLibTime_t)60 * (RadioLibTime_t)1000 / (RadioLibTime_t)(1UL << maxDutyCycle);
        }
    ...
    case (RADIOLIB_LORAWAN_MAC_RX_PARAM_SETUP): {
        ...
        if (rx1Dr != RADIOLIB_LORAWAN_DATA_RATE_UNUSED) {
            int16_t state =
                this->phyLayer->checkDataRate(this->band->dataRates[rx1Dr].dr, this->band->dataRates[rx1Dr].modem);
            if (state == RADIOLIB_ERR_NONE) {
                rx1DrOsAck = 1;
            }
        }
        ...
        // if not fully acknowledged, return now without applying the requested
        // configuration
        if (optOut[0] != 0x07) {
            return (true);
        }
    ...
    case (RADIOLIB_LORAWAN_MAC_NEW_CHANNEL): {
        ...
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("NewChannelReq: index = %d, freq = %7.3f MHz, DR %d-%d", macChIndex,
                                        macFreq / 10000.0, macDrMin, macDrMax);

        uint8_t drAck = 0;
        uint8_t freqAck = 0;

        // the default channels shall not be modified, so check if this is a default
        // channel if the channel index is set, this channel is defined, so return a
        // NACK
        ...
        // check if the outermost datarates are defined and if the device supports
        // them
        if (this->band->dataRates[macDrMin].modem != RADIOLIB_MODEM_NONE &&
                this->band->dataRates[macDrMax].modem != RADIOLIB_MODEM_NONE) {
            if (this->phyLayer->checkDataRate(this->band->dataRates[macDrMin].dr,
                                              this->band->dataRates[macDrMin].modem) == RADIOLIB_ERR_NONE) {
                if (this->phyLayer->checkDataRate(this->band->dataRates[macDrMax].dr,
                                                  this->band->dataRates[macDrMax].modem) == RADIOLIB_ERR_NONE) {
                    drAck = 1;
                }
            }
        }
        ...
        if (macFreq >= this->band->freqMin && macFreq <= this->band->freqMax) {
            ...
            // otherwise, if frequency is 0, disable the channel which is also a valid
            // option
        } else if (macFreq == 0) {
            freqAck = 1;
        }
        ...
        if (macFreq > 0) {
            ...
            this->dynamicChannels[RADIOLIB_LORAWAN_DOWNLINK][macChIndex] =
                this->dynamicChannels[RADIOLIB_LORAWAN_UPLINK][macChIndex];
    ...
    case (RADIOLIB_LORAWAN_MAC_TX_PARAM_SETUP): {
        ...
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("TxParamSetupReq: dlDwell = %d, ulDwell = %d, maxEirp = %d dBm", dlDwell,
                                        ulDwell, eirpEncoding[maxEirpRaw]);
    ...
    case (RADIOLIB_LORAWAN_MAC_REKEY): {
        ...
        // If the server's version is invalid the device SHALL discard the RekeyConf
        // command and retransmit the RekeyInd in the next uplink frame
        if (srvVersion != this->rev) {
            RADIOLIB_DEBUG_PROTOCOL_PRINTLN("ERROR! Please disable your device and "
                                            "consult supported LoRaWAN versions");
            (void)LoRaWANNode::pushMacCommand(cid, &this->rev, this->fOptsUp, &this->fOptsUpLen,
    ...
    case (RADIOLIB_LORAWAN_MAC_ADR_PARAM_SETUP): {
        ...
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("ADRParamSetupReq: limitExp = %d, delayExp = %d", this->adrLimitExp,
                                        this->adrDelayExp);
    ...
    case (RADIOLIB_LORAWAN_MAC_FORCE_REJOIN): {
        ...
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("ForceRejoinReq: period = %d, maxRetries = "
                                        "%d, rejoinType = %d, dr = %d",
                                        period, maxRetries, rejoinType, dr);
        (void)period;
        (void)maxRetries;
        (void)rejoinType;
        (void)dr;
    ...
    for (uint8_t opt = 0; opt < numOpts; opt++) {
        ...
        case 5:
            ...
            if (this->band->bandNum == BandCN470) {
                ...
                // for all other bands, the first 10 bits enable banks of 8 125kHz
                // channels
            } else {
        ...
        case 6:
            ...
            // for fixed bands:   all default 125kHz channels ON, channel mask similar
            // to ChMaskCntl = 4 except for CN470:  all default 125kHz channels ON
            ...
            break;
        case 7:
            // for fixed bands:   all 125kHz channels OFF, channel mask similar to
            // ChMaskCntl = 4 except for CN470:  RFU
...
int16_t LoRaWANNode::getMacCommand(uint8_t cid, LoRaWANMacCommand_t *cmd)
{
    for (size_t i = 0; i < RADIOLIB_LORAWAN_NUM_MAC_COMMANDS; i++) {
        if (MacTable[i].cid == cid) {
            memcpy(reinterpret_cast<void *>(cmd),
                   reinterpret_cast<void *>(const_cast<LoRaWANMacCommand_t *>(&MacTable[i])),
                   sizeof(LoRaWANMacCommand_t));
            return (RADIOLIB_ERR_NONE);
        }
    }
...
int16_t LoRaWANNode::sendMacCommandReq(uint8_t cid)
{
    ...
    // if there are already 15 MAC bytes in the uplink queue, we can't add a new
    // one
...
{
    uint8_t payload[2] = {0};
    int16_t state = this->getMacPayload(RADIOLIB_LORAWAN_MAC_LINK_CHECK, this->fOptsDown, this->fOptsDownLen, payload,
                                        RADIOLIB_LORAWAN_DOWNLINK);
    RADIOLIB_ASSERT(state);
...
{
    uint8_t payload[5] = {0};
    int16_t state = this->getMacPayload(RADIOLIB_LORAWAN_MAC_DEVICE_TIME, this->fOptsDown, this->fOptsDownLen, payload,
                                        RADIOLIB_LORAWAN_DOWNLINK);
    RADIOLIB_ASSERT(state);
...
{
    // if this MAC command doesn't exist, it wouldn't even get into the queue, so
    ...
    if (dir == RADIOLIB_LORAWAN_UPLINK) {
        ...
        // in the downlink direction, MAC payload should persist if it is
        // user-accessible which is the case for LinkCheck and DeviceTime
    } else {
        return (cmd.user);
    }
    return (false);
...
void LoRaWANNode::clearMacCommands(uint8_t *inOut, uint8_t *lenInOut, uint8_t dir)
{
    ...
    while (i < *lenInOut) {
        ...
        // only clear MAC command if it should not persist until a downlink is
        // received
...
int16_t LoRaWANNode::setTxPower(int8_t txPower)
{
    ...
    // the selected value is rounded down to nearest multiple of two away from
    // txPowerMax e.g. on EU868, max is 16; if 13 is selected then we set to 12
...
// A user may enable CSMA to provide frames an additional layer of protection
// from interference.
...
int16_t LoRaWANNode::setPhyProperties(const LoRaWANChannel_t *chnl, uint8_t dir, int8_t pwr, size_t pre)
{
    ...
    // at this point, assume that Tx power value is already checked, so ignore the
    // return value this call is only used to clip a value that is higher than the
    // module supports
    ...
    case (ModemType_t::RADIOLIB_MODEM_FSK): {
        ...
        syncWordLen = 3;
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("[FSK] BR = %4.1f, FD = %4.1f kHz", (double)dr->fsk.bitRate,
    ...
    case (ModemType_t::RADIOLIB_MODEM_LORA): {
        ...
        syncWordLen = 1;
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("[LoRa] SF = %d, BW = %5.1f kHz, CR = 4/%d, IQ: %c", dr->lora.spreadingFactor,
                                        (double)dr->lora.bandwidth, dr->lora.codingRate, dir ? 'D' : 'U');
    } break;
    ...
    case (ModemType_t::RADIOLIB_MODEM_LRFHSS): {
        ...
        syncWordLen = 4;
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("[LR-FHSS] BW = 0x%02x, CR = 0x%02x kHz, grid = %c", dr->lrFhss.bw,
                                        dr->lrFhss.cr, dr->lrFhss.narrowGrid ? 'N' : 'W');
    } break;
...
bool LoRaWANNode::calculateChannelFlags()
{
    ...
    if (this->band->bandType == RADIOLIB_LORAWAN_BAND_DYNAMIC) {
        ...
        // check first frequency span to see if the datarate is allowed and any
        ...
        // check second frequency span to see if the datarate is allowed and any
        // channel is available
...
int16_t LoRaWANNode::selectChannels()
{
    ...
    if (!this->isActivated() && this->band->bandType == RADIOLIB_LORAWAN_BAND_FIXED && this->subBand == 0) {
        ...
        if (bank < num125kHzBanks) {
            ...
            // if we selected the 500 kHz bank, select a random channel from this
            // whole span
        } else {
    ...
    if (this->band->bandType == RADIOLIB_LORAWAN_BAND_DYNAMIC) {
        ...
        // the downlink channel is the uplink channel ID `modulo` number of downlink
        // channels
...
bool LoRaWANNode::verifyMIC(uint8_t *msg, size_t len, uint8_t *key)
{
    ...
    if (micCalculated != micReceived) {
        RADIOLIB_DEBUG_PROTOCOL_PRINTLN("MIC mismatch, expected %08lx, got %08lx", (unsigned long)micCalculated,
                                        (unsigned long)micReceived);
        return (false);
    }
...
void LoRaWANNode::processAES(const uint8_t *in, size_t len, uint8_t *key, uint8_t *out, uint32_t addr, uint32_t fCnt,
                             uint8_t dir, uint8_t ctrId, bool counter)
{
    if (len == 0) {
        return;
    }
    ...
    // on downlink frames, this has a decryption effect because server actually
