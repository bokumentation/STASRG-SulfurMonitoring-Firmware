...
//                  Hellschreiber use 7x7 characters, but this simplified font
//                  uses only 5x5 the extra bytes aren't stored
...
public:
    ...
    */
    ...
    /*!
      ...
      frequency in Hz (in AFSK mode). \param rate Baud rate to be used during
      transmission. Defaults to 122.5 ("Feld Hell")
    */
    int16_t begin(float base, float rate = 122.5);

    /*!
      \brief Method to "print" a buffer of pixels, this is exposed to allow users
      to send custom characters. \param buff Buffer of pixels to send, in a 7x7
      pixel array. \returns Always returns the number of printed glyphs (1).
    */
    size_t printGlyph(const uint8_t *buff);

    /*!
      \brief Invert text color.
      \param inv Whether to enable color inversion (white text on black
      background), or not (black text on white background)
    */
