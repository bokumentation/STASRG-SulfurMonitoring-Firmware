...
// this is a massive hack, but we need a global-scope ISR to manage the bit
// reading let's hope nobody ever tries running two POCSAG receivers at the same
// time
...
{
    return (PagerClient::transmit(reinterpret_cast<uint8_t *>(const_cast<char *>(str)), strlen(str), addr, encoding,
                                  function));
}
...
int16_t PagerClient::transmit(const uint8_t *data, size_t len, uint32_t addr, uint8_t encoding, uint8_t function)
{
    ...
    // get target position in batch (3 LSB from address determine frame position
    // in batch)
    ...
    if (len > 0) {
        ...
        for (size_t i = 0; i < numDataBlocks + numBatches - 1; i++) {
            ...
            // set all message symbols until we overflow to the next code word or run
            // out of message symbols
...
int16_t PagerClient::readData(String &str, size_t len, uint32_t *addr)
{
    ...
    // determine the message length, based on user input or the amount of received
    // data
    size_t length = len;
    if (length == 0) {
...
int16_t PagerClient::readData(uint8_t *data, size_t *len, uint32_t *addr)
{
    ...
    while (!match && phyLayer->available()) {
        ...
        if (addressMatched(addr_found)) {
            ...
            if ((cw & RADIOLIB_PAGER_FUNCTION_BITS_MASK) >> RADIOLIB_PAGER_FUNC_BITS_POS ==
