...
Module::Module(uint32_t cs, uint32_t irq, uint32_t rst, uint32_t gpio)
    : csPin(cs), irqPin(irq), rstPin(rst), gpioPin(gpio)
{
    this->hal = new ArduinoHal();
}

Module::Module(uint32_t cs, uint32_t irq, uint32_t rst, uint32_t gpio, SPIClass &spi, SPISettings spiSettings)
    : csPin(cs), irqPin(irq), rstPin(rst), gpioPin(gpio)
{
...
int16_t Module::SPIsetRegValue(uint32_t reg, uint8_t value, uint8_t msb, uint8_t lsb, uint8_t checkInterval,
                               uint8_t checkMask, bool force)
{
    if ((msb > 7) || (lsb > 7) || (lsb > msb)) {
        return (RADIOLIB_ERR_INVALID_BIT_RANGE);
    }
    ...
    // some registers need a bit of time to process the change (e.g.
    // SX127X_REG_OP_MODE)
...
void Module::SPIreadRegisterBurst(uint32_t reg, size_t numBytes, uint8_t *inBytes)
{
    if (!this->spiConfig.stream) {
        ...
        SPItransferStream(cmd,
                          this->spiConfig.widths[RADIOLIB_MODULE_SPI_WIDTH_CMD] / 8 +
                          this->spiConfig.widths[RADIOLIB_MODULE_SPI_WIDTH_ADDR] / 8,
                          false, NULL, inBytes, numBytes, true);
    }
}
...
{
    // prepare the buffers
    size_t buffLen = this->spiConfig.widths[RADIOLIB_MODULE_SPI_WIDTH_CMD] / 8 +
                     this->spiConfig.widths[RADIOLIB_MODULE_SPI_WIDTH_ADDR] / 8 + numBytes;
...
int16_t Module::SPIreadStream(uint16_t cmd, uint8_t *data, size_t numBytes, bool waitForGpio, bool verify)
{
    ...
    return (this->SPIreadStream(cmdBuf, this->spiConfig.widths[RADIOLIB_MODULE_SPI_WIDTH_CMD] / 8, data, numBytes,
                                waitForGpio, verify));
}
...
int16_t Module::SPIwriteStream(uint16_t cmd, const uint8_t *data, size_t numBytes, bool waitForGpio, bool verify)
{
    ...
    return (this->SPIwriteStream(cmdBuf, this->spiConfig.widths[RADIOLIB_MODULE_SPI_WIDTH_CMD] / 8, data, numBytes,
                                 waitForGpio, verify));
}

int16_t Module::SPIwriteStream(const uint8_t *cmd, uint8_t cmdLen, const uint8_t *data, size_t numBytes,
                               bool waitForGpio, bool verify)
{
    // send the command
...
int16_t Module::SPIcheckStream()
{
    ...
    state = this->SPItransferStream(cmdBuf, this->spiConfig.widths[RADIOLIB_MODULE_SPI_WIDTH_CMD] / 8, false, NULL,
                                    &spiStatus, 1, true);
    RADIOLIB_ASSERT(state);
...
int16_t Module::SPItransferStream(const uint8_t *cmd, uint8_t cmdLen, bool write, const uint8_t *dataOut,
                                  uint8_t *dataIn, size_t numBytes, bool waitForGpio)
{
    ...
    if (write) {
        ...
        memset(buffOutPtr, this->spiConfig.cmds[RADIOLIB_MODULE_SPI_COMMAND_NOP],
               numBytes + (this->spiConfig.widths[RADIOLIB_MODULE_SPI_WIDTH_STATUS] / 8));
    }
