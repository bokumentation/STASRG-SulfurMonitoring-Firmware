...
// may be Serial port (on Arduino) or file like stdout or stderr (on generic
// platforms)
 ...
 * Every write to an SPI register using SPI set function will be verified by a
 * subsequent read operation. This improves reliability, but slightly slows down
 * communication. Note: Enabled by default.
 */
 ...
 * RadioLib will check provided parameters (such as frequency) against limits
 * determined by the device manufacturer. It is highly advised to keep this
 * macro defined, removing it will allow invalid values to be set, possibly
 * leading to bricked module and/or program crashing. Note: Enabled by default.
 */
 ...
 * Warning: SX127x errata fix has been reported to cause issues with LoRa
 * bandwidths lower than 62.5 kHz. It should only be enabled if you really are
 * observing some errata-related issue. Note: Disabled by default.
 */
...
/*
 * God mode enable - all methods and member variables in all classes will be
 * made public, thus making them accessible from Arduino code. Warning: Come on,
 * it's called GOD mode - obviously only use this if you know what you're doing.
 ...
 * This will make some hardware methods like SPI get/set accessible from the
 * user sketch - think of it as "god mode lite" Warning: RadioLib won't stop you
 * from writing invalid stuff into your device, so it's quite easy to brick your
 * module with this.
 */
...
/*
 * Enable static-only memory management: no dynamic allocation will be
 * performed. Warning: Large static arrays will be created in some methods. It
 * is not advised to send large packets in this mode.
 */
 ...
 * If the difference is e.g. 1014 milliseconds between the prints, set this
 * value to 14 Or, for more accuracy, wait for 100,000 milliseconds and divide
 * the total drift by 100
 */
 ...
 *
 * RADIOLIB_PLATFORM - platform name, used in debugging to quickly check the
 * correct platform is detected. RADIOLIB_NC - alias for unused pin, usually the
 * largest possible value of uint32_t. RADIOLIB_DEFAULT_SPI - default SPIClass
 * instance to use. RADIOLIB_NONVOLATILE - macro to place variable into program
 * storage (usually Flash). RADIOLIB_NONVOLATILE_READ_BYTE - function/macro to
 * read variables saved in program storage (usually Flash). RADIOLIB_TYPE_ALIAS
 * - construct to create an alias for a type, usually via the `using` keyword.
 * RADIOLIB_TONE_UNSUPPORTED - some platforms do not have tone()/noTone(), which
 * is required for AFSK.
 *
 * Users may also specify their own configuration by uncommenting the
 * RADIOLIB_CUSTOM_ARDUINO, and then specifying all platform parameters in the
 * section below. This will override automatic platform detection.
 */
...
// the following must be defined if the Arduino core does not support tone or
// yield function
// #define RADIOLIB_TONE_UNSUPPORTED
// #define RADIOLIB_YIELD_UNSUPPORTED

// in addition, the following macros may be defined if the Arduino core differs
// from the defaults
...
// you might also have to define these if the Arduino core has some uncommon pin
// mode/status types
...
// some of RadioLib drivers may be excluded, to prevent collisions with
// platforms (or to speed up build process) the following is a complete list of
// all possible exclusion macros, uncomment any of them to disable that driver
// NOTE: Some of the exclusion macros are dependent on each other. For example,
// it is not possible to exclude RF69
//       while keeping SX1231 (because RF69 is the base class for SX1231). The
//       dependency is always uni-directional, so excluding SX1231 and keeping
//       RF69 is valid.
// #define RADIOLIB_EXCLUDE_CC1101           (1)
// #define RADIOLIB_EXCLUDE_NRF24            (1)
// #define RADIOLIB_EXCLUDE_RF69             (1)
// #define RADIOLIB_EXCLUDE_SX1231           (1) // dependent on
// RADIOLIB_EXCLUDE_RF69 #define RADIOLIB_EXCLUDE_SI443X           (1) #define
// RADIOLIB_EXCLUDE_RFM2X            (1) // dependent on RADIOLIB_EXCLUDE_SI443X
// #define RADIOLIB_EXCLUDE_SX127X           (1)
// #define RADIOLIB_EXCLUDE_SX126X           (1)
// #define RADIOLIB_EXCLUDE_STM32WLX         (1) // dependent on
// RADIOLIB_EXCLUDE_SX126X #define RADIOLIB_EXCLUDE_SX128X           (1) #define
// RADIOLIB_EXCLUDE_AFSK             (1) #define RADIOLIB_EXCLUDE_AX25 (1)
...
// #define RADIOLIB_EXCLUDE_PAGER            (1)
    ...
    !(defined(ARDUINO_AVR_UNO_WIFI_REV2) || defined(ARDUINO_AVR_NANO_EVERY) || defined(ARDUINO_ARCH_MEGAAVR))
...
// ESP32 doesn't support tone(), but it can be emulated via LED control
// peripheral
#define RADIOLIB_TONE_UNSUPPORTED
...
// Arduino mbed OS boards have a really bad tone implementation which will crash
...
// Arduino STM32 core by Roger Clark
...
// ... and also defines pinMode() as a macro, which is by far the stupidest
// thing I have seen on Arduino
#undef pinMode
...
// ... AND as the (hopefully) final nail in the coffin, IT F*CKING DEFINES
// YIELD() AS A MACRO THAT DOES NOTHING!!!
#define RADIOLIB_YIELD_UNSUPPORTED
#if defined(yield)
#undef yield
#endif
...
// hmm, no yield either - weird on something like Raspberry PI, but sure, we can
// handle it
#define RADIOLIB_YIELD_UNSUPPORTED

// aight, getting to the juicy stuff - PGM_P seems missing, that's the first
// time
#define PGM_P const char *

// ... and for the grand finale, we have millis() and micros() DEFINED AS
// MACROS!
#if defined(millis)
#undef millis
inline RadioLibTime_t millis()
{
    return ((RadioLibTime_t)(STCV / 1000));
};
#endif

#if defined(micros)
#undef micros
inline RadioLibTime_t micros()
{
    return ((RadioLibTime_t)(STCV));
};
#endif
...
#else
// other Arduino platforms not covered by the above list - this may or may not
// work
...
// some Arduino platforms do not support printf("%f"), so it has to be done this
// way
#if defined(RADIOLIB_BUILD_ARDUINO)
#define RADIOLIB_DEBUG_PRINT_FLOAT(LEVEL, VAL, DECIMALS)                                                               \
    RADIOLIB_DEBUG_PRINT(LEVEL);                                                                                       \
    RADIOLIB_DEBUG_PORT.print(VAL, DECIMALS)
#else
...
#define RADIOLIB_INFO                                                                                                                       \
    "\r\nRadioLib Info\nVersion:  \"" RADIOLIB_VALUE(RADIOLIB_VERSION_MAJOR) "." RADIOLIB_VALUE(RADIOLIB_VERSION_MINOR) "." RADIOLIB_VALUE( \
        RADIOLIB_VERSION_PATCH) "." RADIOLIB_VALUE(RADIOLIB_VERSION_EXTRA) "\""                                                             \
                                                                           "\r"                                                             \
                                                                           "\n"                                                             \
                                                                           "Pla"                                                            \
                                                                           "tfo"                                                            \
                                                                           ...
                                                                           " " RADIOLIB_VALUE(                                              \
                                                                               RADIOLIB_PLATFORM) "\r"                                      \
                                                                                                  "\n" RADIOLIB_VALUE(                      \
                                                                                                      __DATE__) " " RADIOLIB_VALUE(__TIME__)

/*!
  \brief A simple assert macro, will return on error.
  If RADIOLIB_VERBOSE_ASSERT is enabled, the macro will also print out file and
  line number trace, at a significant program storage cost.
*/
#if RADIOLIB_VERBOSE_ASSERT
#define RADIOLIB_ASSERT(STATEVAR)                                                                                      \
    {                                                                                                                  \
        if ((STATEVAR) != RADIOLIB_ERR_NONE) {                                                                         \
            RADIOLIB_DEBUG_BASIC_PRINTLN("%d at %s:%d", STATEVAR, __FILE__, __LINE__);                                 \
            return (STATEVAR);                                                                                         \
        }                                                                                                              \
    }
#define RADIOLIB_ASSERT_PTR(PTR)                                                                                       \
    {                                                                                                                  \
        if ((PTR) == NULL) {                                                                                           \
            RADIOLIB_DEBUG_BASIC_PRINTLN("NULL at %s:%d", __FILE__, __LINE__);                                         \
...
/*!
  \brief Macro to check variable is within constraints - this is commonly used
  to check parameter ranges. Requires RADIOLIB_CHECK_RANGE to be enabled
*/
...
#define RADIOLIB_ERRATA_SX127X(...)                                                                                    \
    { errataFix(__VA_ARGS__); }
#else
#define RADIOLIB_ERRATA_SX127X(...)                                                                                    \
    {}
#endif

// these macros are usually defined by Arduino, but some platforms undef them,
// so its safer to use our own
...
#define RADIOLIB_VERSION                                                                                               \
    (((RADIOLIB_VERSION_MAJOR) << 24) | ((RADIOLIB_VERSION_MINOR) << 16) | ((RADIOLIB_VERSION_PATCH) << 8) |           \
