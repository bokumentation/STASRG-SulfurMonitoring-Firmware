...
/*!
  \def END_OF_MODE_TABLE Value to use as the last element in a mode table to
  indicate the end of the table. See \ref setRfSwitchTable for details.
*/
...
/*!
  \def RFSWITCH_PIN_FLAG Bit flag used to mark unused pins in RF switch pin map.
  This can be either unconnected pin marked with RADIOLIB_NC, or a pin
  controlled by the radio (e.g. DIOx pins on LR11x0), as opposed to an
  MCU-controlled GPIO pin.
*/
#define RFSWITCH_PIN_FLAG (0x01UL << 31)

/*!
  \defgroup module_spi_command_pos Position of commands in Module::spiConfig
  command array.
  \{
*/
...
/*!
  \defgroup module_spi_width_pos Position of bit field widths in
  Module::spiConfig width array.
  \{
*/
...
public:
    ...
    */
    struct RfSwitchMode_t {
        /*! \brief RF switching mode, one of \ref OpMode_t or a custom radio-defined
         * value. */
        uint8_t mode;
...
#if defined(RADIOLIB_BUILD_ARDUINO)
    /*!
      \brief Arduino Module constructor. Will use the default SPI interface and
      automatically initialize it. \param cs Arduino pin to be used as chip
      select. \param irq Arduino pin to be used as interrupt/GPIO. \param rst
      Arduino pin to be used as hardware reset for the module. \param gpio Arduino
      pin to be used as additional interrupt/GPIO.
    */
    Module(uint32_t cs, uint32_t irq, uint32_t rst, uint32_t gpio = RADIOLIB_NC);

    /*!
      \brief Arduino Module constructor. Will not attempt SPI interface
      initialization. \param cs Arduino pin to be used as chip select. \param irq
      Arduino pin to be used as interrupt/GPIO. \param rst Arduino pin to be used
      as hardware reset for the module. \param gpio Arduino pin to be used as
      additional interrupt/GPIO. \param spi SPI interface to be used, can also use
      software SPI implementations. \param spiSettings SPI interface settings.
    */
    ...
    /*!
      ...
      \param hal A Hardware abstraction layer instance. An ArduinoHal instance for
      example. \param cs Pin to be used as chip select. \param irq Pin to be used
      as interrupt/GPIO. \param rst Pin to be used as hardware reset for the
      module. \param gpio Pin to be used as additional interrupt/GPIO.
    */
    ...
    */
    struct SPIConfig_t {
        /*! \brief Whether the SPI module is stream-type (SX126x/8x) or registrer
         * access type (SX127x, CC1101 etc). */
        bool stream;

        /*! \brief Last recorded SPI error - only updated for modules that return
         * status during SPI transfers. */
        int16_t err;
    ...
    /*! \brief SPI configuration structure. The default configuration corresponds
     * to register-access modules, such as SX127x. */
    ...
    */
    ...
    /*!
      \brief Callback to timer interrupt setup function when running in interrupt
      timing control mode.
    */
    TimerSetupCb_t TimerSetupCb = nullptr;

    /*!
      \brief Timer flag variable to be controlled by a platform-dependent
      interrupt.
    */
    ...
    */
    ...
    /*!
      \brief SPI read method that automatically masks unused bits. This method is
      the preferred SPI read mechanism. \param reg Address of SPI register to
      read. \param msb Most significant bit of the register variable. Bits above
      this one will be masked out. \param lsb Least significant bit of the
      register variable. Bits below this one will be masked out. \returns Masked
      register value or status code.
    */
    int16_t SPIgetRegValue(uint32_t reg, uint8_t msb = 7, uint8_t lsb = 0);

    /*!
      \brief Overwrite-safe SPI write method with verification. This method is the
      preferred SPI write mechanism. \param reg Address of SPI register to write.
      \param value Single byte value that will be written to the SPI register.
      \param msb Most significant bit of the register variable. Bits above this
      one will not be affected by the write operation. \param lsb Least
      significant bit of the register variable. Bits below this one will not be
      affected by the write operation. \param checkInterval Number of milliseconds
      between register writing and verification reading. Some registers need up to
      10ms to process the change. \param checkMask Mask of bits to check, only
      bits set to 1 will be verified. \param force Write new value even if the old
      value is the same. \returns \ref status_codes
    */
    int16_t SPIsetRegValue(uint32_t reg, uint8_t value, uint8_t msb = 7, uint8_t lsb = 0, uint8_t checkInterval = 2,
                           uint8_t checkMask = 0xFF, bool force = false);
    ...
    */
    ...
    /*!
      \brief SPI basic read method. Use of this method is reserved for special
      cases, SPIgetRegValue should be used instead. \param reg Address of SPI
      register to read. \returns Value that was read from register.
    */
    ...
    */
    ...
    /*!
      \brief SPI basic write method. Use of this method is reserved for special
      cases, SPIsetRegValue should be used instead. \param reg Address of SPI
      register to write. \param data Value that will be written to the register.
    */
    ...
    */
    ...
    /*!
      ...
      \param numBytes Number of bytes to transfer.
      \param waitForGpio Whether to wait for some GPIO at the end of transfer
      (e.g. BUSY line on SX126x/SX128x). \param verify Whether to verify the
      result of the transaction after it is finished. \returns \ref status_codes
    */
    ...
    /*!
      ...
      result of the transaction after it is finished. \returns \ref status_codes
    */
    int16_t SPIreadStream(const uint8_t *cmd, uint8_t cmdLen, uint8_t *data, size_t numBytes, bool waitForGpio = true,
                          bool verify = true);
    ...
    /*!
      ...
      result of the transaction after it is finished. \returns \ref status_codes
    */
    int16_t SPIwriteStream(uint16_t cmd, const uint8_t *data, size_t numBytes, bool waitForGpio = true,
                           bool verify = true);
    ...
    /*!
      ...
      result of the transaction after it is finished. \returns \ref status_codes
    */
    int16_t SPIwriteStream(const uint8_t *cmd, uint8_t cmdLen, const uint8_t *data, size_t numBytes,
                           bool waitForGpio = true, bool verify = true);

    /*!
      \brief SPI single transfer method for modules with stream-type SPI interface
      (SX126x, SX128x etc.). \param cmd SPI operation command. \param cmdLen SPI
      command length in bytes. \param write Set to true for write commands, false
      for read commands. \param dataOut Data that will be transferred from master
      to slave. \param dataIn Data that was transferred from slave to master.
      \param numBytes Number of bytes to transfer.
      \param waitForGpio Whether to wait for some GPIO at the end of transfer
      (e.g. BUSY line on SX126x/SX128x). \returns \ref status_codes
    */
    int16_t SPItransferStream(const uint8_t *cmd, uint8_t cmdLen, bool write, const uint8_t *dataOut, uint8_t *dataIn,
                              size_t numBytes, bool waitForGpio);

    // pin number access methods
    // getCs is omitted on purpose, as it can interfere when accessing the SPI in
    // a concurrent environment so it is considered to be part of the SPI pins and
    // hence not accessible from outside see
    // https://github.com/jgromes/RadioLib/discussions/1364
    ...
    */
    ...
    /*!
      ...
      \param mode The mode to set. This must be one of the MODE_ constants, or a
      radio-specific constant.
    */
    void setRfSwitchState(uint8_t mode);

    /*!
      \brief Wait for time to elapse, either using the microsecond timer, or the
      TimerFlag. Note that in interrupt timing mode, it is up to the user to set
      up the timing interrupt!
