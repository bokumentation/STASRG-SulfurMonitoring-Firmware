...
} tcList[] = {{TC0, TC0_GCLK_ID, EVSYS_ID_USER_TC0_EVU}, {TC1, TC1_GCLK_ID, EVSYS_ID_USER_TC1_EVU},
    {TC2, TC2_GCLK_ID, EVSYS_ID_USER_TC2_EVU}, {TC3, TC3_GCLK_ID, EVSYS_ID_USER_TC3_EVU},
#if defined(TC4)
...
*/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, int8_t cs, int8_t dc, int8_t mosi, int8_t sck, int8_t rst,
                                 int8_t miso)
...
*/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, tftBusWidth busWidth, int8_t d0, int8_t wr, int8_t dc,
...
void Adafruit_SPITFT::initSPI(uint32_t freq, uint8_t spiMode)
{
    ...
    if (connection == TFT_HARD_SPI) {
        ...
        for (uint8_t i = 0; i < PINS_COUNT; i++) {
            if ((g_APinDescription[i].ulPort == portNum) && (g_APinDescription[i].ulPin >= dBit) &&
    ...
    if (((connection == TFT_HARD_SPI) || (connection == TFT_PARALLEL)) &&
        ...
        if (dptr = dma.addDescriptor(NULL, NULL, 42, DMA_BEAT_SIZE_BYTE, false, false)) {
            ...
            if ((pixelBuf[0] = (uint16_t *)malloc(maxFillLen * sizeof(uint16_t)))) {
                ...
                // is, so here we are.
                if ((descriptor = (DmacDescriptor *)memalign(16, numDescriptors * sizeof(DmacDescriptor)))) {
                    int dmac_id;
                    ...
                    if (connection == TFT_HARD_SPI) {
                        ...
                        for (int d = 0; d < numDescriptors; d++) {
                            ...
                            descriptor[d].BTCTRL.bit.BLOCKACT = DMA_BLOCK_ACTION_NOACT;
                            descriptor[d].BTCTRL.bit.BEATSIZE = tft8.wide ? DMA_BEAT_SIZE_HWORD : DMA_BEAT_SIZE_BYTE;
    ...
    if ((connection == TFT_HARD_SPI) || (connection == TFT_PARALLEL)) {
        ...
        if (!bigEndian) { // Normal little-endian situation...
            while (len) {
                ...
                descriptor[pixelBufIdx].SRCADDR.reg = (uint32_t)pixelBuf[pixelBufIdx] + count * 2;
...
#define TMPBUF_PIXELS (TMPBUF_LONGWORDS * 2)
        ...
        uint16_t bufLen = (len < TMPBUF_PIXELS) ? len : TMPBUF_PIXELS, xferLen, fillLen;
...
#undef QUICKPATH_MAX_LEN
    ...
    if (((connection == TFT_HARD_SPI) || (connection == TFT_PARALLEL)) &&
        ...
        if (hi == lo) { // If high & low bytes are same...
            ...
            uint32_t *pixelPtr = (uint32_t *)pixelBuf[0], twoPixels = __builtin_bswap16(color) * 0x00010001;
            ...
            if (color == lastFillColor) {
                ...
                if (len > lastFillLen) {
                    int fillStart = lastFillLen / 2, fillEnd = (((len < maxFillLen) ? len : maxFillLen) + 1) / 2;
