/*
SoftwareSerial.cpp - Implementation of the Arduino software serial for
...
void UARTBase::beginRx(bool hasPullUp, int bufCapacity, int isrBufCapacity)
{
    ...
    m_isrBuffer.reset(new circular_queue<uint32_t, UARTBase *>(
                          (isrBufCapacity > 0) ? isrBufCapacity
                          : m_buffer->capacity() * (2 + m_dataBits + static_cast<bool>(m_parityMode))));
    ...
    if (m_rxValid && on != m_rxEnabled) {
        if (on) {
            ...
            if (m_bitTicks >= microsToTicks(1000000UL) / 74880UL) {
                ...
                attachInterruptArg(digitalPinToInterrupt(m_rxPin), reinterpret_cast<void (*)(void *)>(rxBitSyncISR),
                                   this, m_invert ? RISING : FALLING);
            }
        } else {
...
void IRAM_ATTR UARTBase::writePeriod(uint32_t dutyCycle, uint32_t offCycle, bool withStopBit)
{
    preciseDelay();
    if (dutyCycle) {
#if defined(ESP8266)
