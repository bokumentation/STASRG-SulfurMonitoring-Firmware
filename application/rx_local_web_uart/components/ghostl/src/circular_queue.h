#pragma once
/*
circular_queue.h - Implementation of a lock-free circular queue for
EspSoftwareSerial. Copyright (c) 2019 Dirk O. Kaar. All rights reserved.
...
/*!
    @brief  Instance class for a single-producer, single-consumer circular queue
   / ring buffer (FIFO). This implementation is lock-free between producer and
   consumer for the available(), peek(), pop(), and push() type functions.
*/
template <typename T, typename ForEachArg = void> class circular_queue {
public:
    /*!
    ...
    */
    ...
    /*!
        @brief  Resize the queue. The available elements in the queue are
       preserved. This is not lock-free and concurrent producer or consumer access
    ...
    */
    size_t IRAM_ATTR available() const
    {
        ...
        if (avail < 0) {
            avail += static_cast<int>(m_bufSize);
        }
        return avail;
    }
    ...
    */
    ...
    /*!
        @brief  Peek at the next element pop will return without removing it from
       the queue.
        @return An rvalue copy of the next element that can be popped. If the
       queue is empty, return an rvalue copy of the element that is pending the
       next push.
    */
    T peek() const
    {
    ...
    */
    ...
    /*!
        @brief  Release the next pending input value, accessible by pushpeek(),
       into the queue.
...
#if defined(ESP8266) || defined(ESP32) || !defined(ARDUINO)
    /*!
        @brief  Pop multiple elements in ordered sequence from the queue to a
       buffer. If buffer is nullptr, simply discards up to size elements from the
       queue.
    ...
    */
    ...
    /*!
        @brief  In reverse order, iterate over, pop and optionally requeue each
       available element from the queue, calling back fun with a reference of
       every single element. Requeuing is dependent on the return boolean of the
       callback function. If it returns true, the requeue occurs.
    */
#if defined(ESP8266) || defined(ESP32) || !defined(ARDUINO)
...
template <typename T, typename ForEachArg> bool circular_queue<T, ForEachArg>::capacity(const size_t cap)
{
    if (cap + 1 == m_bufSize) {
        return true;
...
#if defined(ESP8266) || defined(ESP32) || !defined(ARDUINO)
template <typename T, typename ForEachArg> size_t circular_queue<T, ForEachArg>::push_n(const T *buffer, size_t size)
{
    const auto inPos = m_inPos.load(std::memory_order_acquire);
    const auto outPos = m_outPos.load(std::memory_order_relaxed);

    size_t blockSize = (outPos > inPos) ? outPos - 1 - inPos
                       : (outPos == 0)  ? m_bufSize - 1 - inPos
                       : m_bufSize - inPos;
...
template <typename T, typename ForEachArg> T circular_queue<T, ForEachArg>::pop()
{
...
#if defined(ESP8266) || defined(ESP32) || !defined(ARDUINO)
template <typename T, typename ForEachArg> size_t circular_queue<T, ForEachArg>::pop_n(T *buffer, size_t size)
{
...
#endif
{
    ...
    do {
        ...
        if (fun(val)) {
            ...
            if (outPos1 != pos) {
                circular_queue<T, ForEachArg>::m_buffer[outPos1] = std::move(val);
            }
        }
    } while (pos != outPos);
    std::atomic_thread_fence(std::memory_order_release);
