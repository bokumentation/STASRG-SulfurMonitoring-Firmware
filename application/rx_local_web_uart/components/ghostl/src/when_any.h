...
namespace ghostl {
template <typename T = void> struct when_any final {
    static auto continuation(T result, std::shared_ptr<std::atomic<bool>> completed,
    ...
    template <typename C> explicit when_any(C &&_tasks)
    {
        ...
        for (ghostl::task<T> &task : tasks) {
            ...
            runner.continue_with([completed, tcs](T result) {
                continuation(result, completed, tcs);
            });
...
template <> struct when_any<void> final {
    static auto continuation(std::shared_ptr<std::atomic<bool>> completed, ghostl::task_completion_source<> tcs)
    -> void
    {
        if (auto isCompleted = completed->exchange(true); !isCompleted) {
            tcs.set_value();
    ...
    template <typename C> explicit when_any(C &&_tasks)
    {
        ...
        for (ghostl::task<> &task : tasks) {
            ...
            runner.continue_with([completed, tcs]() {
