#pragma once
/*
cancellation_token.h - Implementation of a C++20 async coroutines cancellation
token. Copyright (c) 2023 Dirk O. Kaar. All rights reserved.
...
struct cancellation_state_type final {
    ...
    {
        if (!cancelled.load())
            list.for_each([](task_completion_source<bool> &&tcs) {
            auto _tcs = std::move(tcs);
            _tcs.set_value(false);
        });
    }
    void cancel()
    {
...
public:
    ...
    {
        if (tcs_node) {
            state->erase_tcs_list_node(tcs_node);
        }
    }
    ...
    /// or false if the cancellation source etc was deleted without
    /// cancellation.</returns>
    [[nodiscard]] auto cancellation_request() -> ghostl::task<bool>
    {
        if (!state || state->is_cancellation_requested()) {
            co_return true;
        }
        tcs_node = state->make_tcs_list_node();
        // if concurrently cancelled, re-cancel to force processing of the new
        // token.
