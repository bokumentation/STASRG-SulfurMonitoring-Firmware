...
// later - earlier >= 0 for both later >= earlier or (rolled over) later <=
// earlier Also, expiration should remain observable for a useful duration of
// time: now - (start + period) >= 0 for now - start >= 0 despite (start +
// period) >= now A well-balanced solution, not breaking on two's compliment
// signed arithmetic, is limiting durations to the maximum signed value of the
// same word size as the original unsigned word.
...
{
    if (!schedule_queue.available()) {
        return HALF_MAX_MICROS;
    }
