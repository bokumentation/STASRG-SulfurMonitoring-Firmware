// circular_mp_test.cpp : This file contains the 'main' function. Program
...
//     // The native POSIX board emulation does not advance system timeout
//     during code execution,
//     // which is incompatible with the FastScheduler comparing system time to
//     target times. k_sleep(K_USEC(schedDelay_us ? schedDelay_us : 1));
...
struct schedule : std::suspend_always {
    ...
    void await_suspend(std::coroutine_handle<> handle)
    {
        ...
        //     // The native POSIX board emulation does not advance system timeout
        //     during code execution,
        //     // which is incompatible with the FastScheduler comparing system time
        //     to target times. k_sleep(K_USEC(schedDelay_us ? schedDelay_us : 1));
        // #else // CONFIG_BOARD_NATIVE_POSIX
        //     k_sleep(K_USEC(schedDelay_us));
        // #endif // CONFIG_BOARD_NATIVE_POSIX
#else  //__ZEPHYR__
    ...
    for (unsigned i = 0; i < PRODUCER_THREAD_CNT; ++i) {
        producer_threads.push(std::thread([i]() {
            for (unsigned c = 0; c < MESSAGES; ++c) {
                ...
                // if (0 == c % 10000) while (std::chrono::system_clock::now() - start <
                // 400ms) {}
