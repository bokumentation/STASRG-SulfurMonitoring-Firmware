...
long unsigned micros() {
    return static_cast<long unsigned>(k_uptime_get() * 1000UL);
}
long unsigned millis() {
    return static_cast<long unsigned>(k_uptime_get());
}
} // namespace
...
struct co_delay : std::suspend_always {
    ...
    void await_suspend(std::coroutine_handle<> handle) {
        ...
        schedule_recurrent_function_us(
            ...
            delay_us, [this]() { return ct.is_cancellation_requested(); });
#else
        auto fn{[](decltype(micros()) delay_us, std::coroutine_handle<> handle, ghostl::cancellation_token ct) {
            std::this_thread::sleep_for(std::chrono::microseconds(delay_us));
            handle.resume();
        }};
        auto thr{std::thread(fn, delay_us, handle, ct)};
        thr.detach();
#endif
    }
};
...
ghostl::generator<std::uint64_t> fibonacci_sequence(unsigned n) {
    ...
    if (n > 94) {
        // throw std::runtime_error("Too big Fibonacci sequence. Elements would
        // overflow.");
    }
...
auto eventAsync(int id, ghostl::cancellation_token ct = {}) {
    ...
    if (id % 2) {
        ...
        struct local {
            static auto task(int id, ghostl::task_completion_source<std::string> tcs,
                             ghostl::cancellation_token ct = {}) -> ghostl::task<> {
...
auto run_tcs(ghostl::cancellation_token ct = {}) -> ghostl::task<> {
    ...
    schedule_recurrent_function_us(
        [posted_tcs]() {
            posted_tcs.set_value();
            return false;
        },
        DELAY / 2, [ct]() { return ct.is_cancellation_requested(); });
...
void main_tcs(ghostl::cancellation_token ct) {
    ...
    runner.continue_with([ct]() {
        schedule_function([ct]() {
            if (!ct.is_cancellation_requested()) main_tcs(ct);
        });
    });
    runner.resume();
}
...
ghostl::task<> make_when_all_tasks(ghostl::cancellation_token outer_ct = {}) {
    ...
    auto ct = cts.token();
    fwd_ct.continue_with([cts](bool cancelled) {
...
void main_when_all(ghostl::cancellation_token ct) {
    ...
    runner.continue_with([ct]() {
        schedule_function([ct]() {
            if (!ct.is_cancellation_requested()) main_when_all(ct);
        });
    });
    runner.resume();
}
...
void main_when_any(ghostl::cancellation_token ct) {
    ...
    runner.continue_with([ct]() {
        schedule_function([ct]() {
            if (!ct.is_cancellation_requested()) main_when_any(ct);
        });
    });
    runner.resume();
}
...
void loop() {
    if (!pending_restart && micros() - start > 4000000) {
        PRINTLN("Global cancel and restart after 2000 ms");
    ...
    // The native POSIX board emulation does not advance system timeout during
    // code execution, which is incompatible with the FastScheduler comparing
    // system time to target times.
...
#else  //__ZEPHYR__
    std::this_thread::sleep_for(schedDelay_us < 4000000 ? std::chrono::microseconds(schedDelay_us)
                                                        : std::chrono::microseconds(4000000));
#endif //__ZEPHYR__
